import type { DifficultyRating, ParsedSet } from './types.js';
 ...
 */
const DIFFICULTY_TOKENS: DifficultyRating[] = ['vh', 'mh', 'em', 'e', 'm', 'h'];
 ...
 */
export function parseSetCell(raw: string): ParsedSet | null {
  if (!raw) return null;
 ...
 */
export function parseWarmupNotes(notesRaw: string): { warmupSets: ParsedSet[]; remainingNotes: string } {
  if (!notesRaw || !notesRaw.trim()) {
    return { warmupSets: [], remainingNotes: '' };
  }

  const notes = notesRaw.trim();
  const warmupSets: ParsedSet[] = [];
 ...
 */
export function isSplitMarker(value: string): boolean {
