...
export const SHEET_CONFIGS: SheetConfig[] = [
  { gid: 484725860,   dayNumber: 1, focus: 'Leg Strength' },
  { gid: 1288547180,  dayNumber: 2, focus: 'Push Strength' },
  { gid: 954466183,   dayNumber: 3, focus: 'Pull Strength' },
  { gid: 1309240948,  dayNumber: 5, focus: 'Leg Hypertrophy' },
  { gid: 2074668672,  dayNumber: 6, focus: 'Push Hypertrophy' },
  { gid: 1273628631,  dayNumber: 7, focus: 'Pull Hypertrophy' },
];
...
export const DAY_TYPE: Record<number, 'Strength' | 'Hypertrophy'> = {
  1: 'Strength',
  2: 'Strength',
  3: 'Strength',
  5: 'Hypertrophy',
 ...
 */
export const STRENGTH_SCHEME: Record<number, { reps: number; sets: number }> = {
  1: { reps: 3, sets: 4 },
  2: { reps: 4, sets: 5 },
  3: { reps: 5, sets: 4 },
  4: { reps: 3, sets: 2 },
...
export const HYPERTROPHY_SCHEME: Record<number, { reps: number; sets: number }> = {
  1: { reps: 10, sets: 4 },
  2: { reps: 10, sets: 5 },
  3: { reps: 12, sets: 4 },
  4: { reps: 8, sets: 2 },
 ...
 */
export const CYCLE_2_EXERCISE_MAP: Record<number, Record<string, string | null>> = {
 ...
 */
export function getExerciseNameForCycle(
  headerName: string,
  dayNumber: number,
  cycleNumber: number,
): string {
  if (cycleNumber !== 2) return headerName; // Cycle 1 and 3 use header names

  const dayMap = CYCLE_2_EXERCISE_MAP[dayNumber];
  if (!dayMap) return headerName;

  const mapped = dayMap[headerName];
  if (mapped === undefined) return headerName; // not in map → keep original
  if (mapped === null) return headerName;       // explicitly same → keep original
  return mapped;
}
