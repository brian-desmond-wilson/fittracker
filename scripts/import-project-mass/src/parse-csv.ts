...
import type { SheetConfig, SheetData, ParsedRow, ParsedExercise, CycleMarker, SheetSegment } from './types.js';
...
/**
 * Structure of exercise columns in the CSV.
 * Each exercise has Set 1-7 + Notes columns.
 */
interface ExerciseColumnRange {
  name: string;
  setColumns: number[];  // indices for Set 1-7
  notesColumn: number;   // index for Notes column
}

/**
 * Parse the header rows (rows 1-4) to determine exercise layout
 */
function parseExerciseLayout(rows: string[][]): ExerciseColumnRange[] {
  if (rows.length < 4) return [];

  const exerciseNameRow = rows[2]; // Row 3: exercise names
  const setHeaderRow = rows[3];    // Row 4: "Set 1", "Set 2", etc.

  const exercises: ExerciseColumnRange[] = [];
  let currentExercise: ExerciseColumnRange | null = null;
...
function parseDataRow(
  row: string[],
  exerciseLayout: ExerciseColumnRange[],
  yearHint?: number,
): ParsedRow {
 ...
 */
function getCycleMarker(row: string[]): number | null {
 ...
 */
export function parseSheetCSV(csvText: string, config: SheetConfig): SheetData {
 ...
 */
export function detectSegments(sheetData: SheetData, knownInstances: { startDate: Date; endDate: Date | null }[]): SheetSegment[] {
  const segments: SheetSegment[] = [];
  let currentRows: ParsedRow[] = [];
  let currentCycleMarkers: CycleMarker[] = [];
