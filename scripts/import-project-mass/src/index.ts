...
import { correlateInstances, buildWorkouts, collectAllExerciseNames } from './correlate.js';
import { previewImport } from './dry-run.js';
import {
  upsertExercises,
  upsertProgramTemplate,
  upsertProgramCycles,
  upsertProgramWorkouts,
  upsertProgramWorkoutExercises,
  upsertProgressions,
  importInstance,
} from './database.js';
import { createEmptyStats } from './utils.js';
import { parseDate } from './parse-dates.js';
import type { SheetData, SheetSegment, ProgramInstanceData } from './types.js';
...
async function main() {
  ...
  const allSegments: SheetSegment[][] = [];
  ...
  await upsertProgramWorkoutExercises(
    supabase, programId, workoutMap, exerciseIdMap, instancesToImport, VERBOSE
  );
