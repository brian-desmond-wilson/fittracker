...
export function guessExerciseCategory(name) {
    ...
    if (compoundPatterns.some(p => lower.includes(p)))
        return 'Compound';
    if (isolationPatterns.some(p => lower.includes(p)))
        return 'Isolation';
    return 'Accessory';
}
/**
 ...
 */
export function guessMuscleGroups(name) {
    const lower = name.toLowerCase();
    const groups = [];
    ...
    if (lower.includes('lateral') || lower.includes('side') || lower.includes('front raise'))
    ...
    if (lower.includes('shrug'))
        groups.push('trapezius');
    if (lower.includes('hyperext') || lower.includes('back ext'))
        groups.push('lower back', 'glutes');
    // Deduplicate
