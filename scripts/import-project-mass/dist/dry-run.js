 ...
 */
export function previewImport(instances, allSheetData, allExerciseNames, verbose) {
    const stats = {
        exercisesCreated: 0,
        exercisesExisting: 0,
    ...
    for (const instance of instances) {
        ...
        const totalSets = workouts.reduce((sum, w) => sum + w.exercises.reduce((eSum, e) => eSum + e.workingSets.length + e.warmupSets.length, 0), 0);
        const workingSets = workouts.reduce((sum, w) => sum + w.exercises.reduce((eSum, e) => eSum + e.workingSets.length, 0), 0);
        const warmupSets = workouts.reduce((sum, w) => sum + w.exercises.reduce((eSum, e) => eSum + e.warmupSets.length, 0), 0);
        ...
        if (splitCount > 0)
            console.log(`      ✂️  ${splitCount} split workouts`);
