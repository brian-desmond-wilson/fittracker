import { DAY_TYPE, DAY_NAMES, STRENGTH_SCHEME, HYPERTROPHY_SCHEME } from './config.js';
import { formatDate, formatTimestamp } from './parse-dates.js';
import { slugify, normalizeExerciseName } from './utils.js';
import { buildWorkouts } from './correlate.js';
/**
 * Phase 1A: Create/upsert all exercises found across all instances
 */
export async function upsertExercises(supabase, allExerciseNames, stats, verbose) {
    const exerciseIdMap = new Map(); // name -> UUID
    for (const name of allExerciseNames) {
        ...
        if (existing) {
            ...
            if (verbose)
                console.log(`   📋 Exercise exists: "${name}" → ${existing.id}`);
            continue;
        }
        ...
        if (verbose)
            console.log(`   ✨ Created exercise: "${normalized}" (${slug}) → ${created.id}`);
    }
 ...
 */
export async function upsertProgramTemplate(supabase, userId, verbose) {
    ...
    if (existing) {
        if (verbose)
            console.log(`   📋 Program template exists: ${existing.id}`);
        return existing.id;
    }
    const { data, error } = await supabase
        ...
        .single();
    if (error)
        throw new Error(`Failed to create program template: ${error.message}`);
    if (verbose)
        console.log(`   ✨ Created program template: ${data.id}`);
    return data.id;
}
/**
 * Phase 1C: Create 3 program cycles
 */
export async function upsertProgramCycles(supabase, programId, verbose) {
    ...
    for (const cycle of cycleData) {
        ...
        if (existing) {
            ...
            if (verbose)
                console.log(`   📋 Cycle ${cycle.cycle_number} exists: ${existing.id}`);
            continue;
        }
        const { data, error } = await supabase
            .from('program_cycles')
            ...
            .single();
        if (error)
            throw new Error(`Failed to create cycle ${cycle.cycle_number}: ${error.message}`);
        cycleMap.set(cycle.cycle_number, data.id);
        if (verbose)
            console.log(`   ✨ Created cycle ${cycle.cycle_number}: ${data.id}`);
    }
    return cycleMap;
}
/**
 ...
 */
export async function upsertProgramWorkouts(supabase, programId, cycleMap, verbose) {
    const workoutMap = new Map(); // "weekNum-dayNum" -> UUID
    ...
    if (verbose)
        console.log(`   📋 ${workoutMap.size} program workouts ready`);
    return workoutMap;
}
/**
 ...
 */
export async function upsertProgramWorkoutExercises(supabase, programId, workoutMap, exerciseIdMap, instances, verbose) {
    ...
    for (const [key, exercises] of workoutExerciseMap) {
        ...
        for (const ex of exercises) {
            ...
            if (!exerciseId) {
                if (verbose)
                    console.warn(`   ⚠️ No exercise ID for "${ex.name}" in ${key}`);
                continue;
            }
    ...
    if (verbose)
        console.log(`   📋 PWEs: ${created} created, ${existing} existing`);
}
/**
 * Phase 1F: Create progressions for each program_workout_exercise
 */
export async function upsertProgressions(supabase, workoutMap, verbose) {
    let created = 0;
    let existing = 0;
    ...
    if (verbose)
        console.log(`   📋 Progressions: ${created} created, ${existing} existing`);
}
/**
 * Phase 2: Import a single program instance
 */
export async function importInstance(supabase, instance, programId, workoutMap, exerciseIdMap, userId, stats, verbose) {
    ...
    if (existingInstance) {
        ...
        if (verbose)
            console.log(`   📋 Instance exists: ${instanceName} → ${programInstanceId}`);
    }
    else {
        ...
        if (verbose)
            console.log(`   ✨ Created instance: ${instanceName} → ${programInstanceId}`);
    }
    // 2.2 Build workouts from parsed data (with cycle-remapped exercise names)
    const workouts = buildWorkouts(instance);
    if (verbose)
        console.log(`   📊 ${workouts.length} workouts to import`);
    let workoutsCompleted = 0;
    // 2.3 Import each workout
    for (const workout of workouts) {
        try {
            await importWorkout(supabase, workout, programInstanceId, programId, workoutMap, exerciseIdMap, userId, stats, verbose);
...
// Internal helpers
// ════════════════════════════════════════════════════════════════════
async function importWorkout(supabase, workout, programInstanceId, programId, workoutMap, exerciseIdMap, userId, stats, verbose) {
    ...
    const { data: existingWI } = await supabase
        ...
        .maybeSingle();
    let workoutInstanceId;
    if (existingWI) {
        workoutInstanceId = existingWI.id;
        if (verbose)
            console.log(`     📋 Workout exists: w${workout.weekNumber}d${workout.dayNumber}`);
    }
    else {
    ...
    for (const exerciseData of workout.exercises) {
        try {
            await importExerciseInstance(supabase, exerciseData, workoutInstanceId, programWorkoutId, exerciseIdMap, userId, workout, stats, verbose);
        }
        catch (err) {
...
async function importExerciseInstance(supabase, exerciseData, workoutInstanceId, programWorkoutId, exerciseIdMap, userId, workout, stats, verbose) {
...
async function importSetInstance(supabase, set, exerciseInstanceId, userId, setNumber, isWarmup, stats) {
    // Check if set exists
    const { data: existing } = await supabase
        .from('set_instances')
        .select('id')
