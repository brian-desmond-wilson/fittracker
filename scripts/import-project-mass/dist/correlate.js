import { KNOWN_INSTANCES, getExerciseNameForCycle } from './config.js';
 ...
 */
export function correlateInstances(allSegments) {
    const instances = [];
 ...
 */
export function buildWorkouts(instance) {
    const workouts = [];
    for (const segment of instance.segments) {
        ...
        for (const row of segment.rows) {
            ...
            const exercises = row.exercises.map((ex, idx) => ({
 ...
 */
export function collectAllExerciseNames(instances) {
    const names = new Set();
