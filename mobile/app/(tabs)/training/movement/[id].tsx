...
export default function MovementDetailPage() {
  ...
  const loadHierarchy = async () => {
    ...
    try {
      ...
      let currentParentId: string | null = movement.parent_exercise_id;
  ...
  return (
    <>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.metaSection}>
          {movement.movement_category?.name && (
            <View style={styles.metaItem}>
              ...
              <Text style={styles.metaValue} numberOfLines={1}>{movement.movement_category.name}</Text>
            </View>
          )}
          {movement.goal_type?.name && (
            <View style={styles.metaItem}>
              <Text style={styles.metaLabel}>Goal Type</Text>
              <Text style={styles.metaValue} numberOfLines={1}>{movement.goal_type.name}</Text>
            </View>
          )}
          {movement.skill_level && (
            <View style={styles.metaItem}>
              <Text style={styles.metaLabel}>Skill Level</Text>
              <Text style={styles.metaValue} numberOfLines={1}>{movement.skill_level}</Text>
            </View>
          )}
        </View>
        ...
        {/* Movement Hierarchy */}
        {!movement.is_core && hierarchyData.ancestors.length > 0 && (
          <View style={styles.section}>
              ...
              {hierarchyData.ancestors.map((ancestor, index) => (
                <View key={ancestor.id} style={index > 0 ? styles.hierarchyAncestorWrapper : undefined}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => router.push(`/(tabs)/training/movement/${ancestor.id}`)}
                    activeOpacity={0.7}
                  >
