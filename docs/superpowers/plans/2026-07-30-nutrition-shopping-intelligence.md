...
Tests:       309 passed, 309 total
```
(309 = unchanged from the count Task 4 left at — this task added no new tests, since it is a type/wrapper change with no new pure-lib logic to pin; the existing suite's silence on `replaceItemLocations` was expected and confirmed by grep — nothing in `mobile/src/lib/__tests__` references it.) The plan's Task 5 code block (the wrapper) was re-diffed programmatically against `mobile/src/lib/supabase/inventory.ts` and found byte-identical, the same check used for Tasks 1, 3, and 4.
