...
- Live Postgres access (no DB password exists on this machine): mint a login-role pooler URL by reading the output of `supabase db dump --dry-run`. The minted `cli_login_postgres.<ref>` role has no direct table grants — after connecting, run `SET ROLE postgres;` in-session or table reads hit permission-denied. pg_dump via the script uses `--role postgres` for the same reason. Read-only use unless the task explicitly authorizes writes.
