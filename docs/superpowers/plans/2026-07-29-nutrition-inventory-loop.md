...
- **Assessed and deliberately left alone.** A single-location save with no location picked writes `location: null` to `food_inventory` (the item row keeps its existing `storageType === 'single-location' ? location : null` expression, unchanged per Step 1) while the location row it creates falls back to `"pantry"` — the plan's own `location ?? "pantry"` spelling. The two disagree, but `food_inventory.location` is display-only and Task 11 is the comment/dead-copy sweep that should decide whether it survives at all; changing it here would be out of Task 6's scope. Also unchanged: the `parseFloat` validation / `parseInt` write split, which predates this task.
