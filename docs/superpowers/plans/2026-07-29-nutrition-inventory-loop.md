...
1. **Phase 3 must be merged to `main` before execution starts.** Tasks 9–10 modify `src/lib/eatNext.ts`, `src/lib/__tests__/eatNext.test.ts`, and `src/hooks/useEatNext.ts`, which Phase 3 creates. Create a fresh `nutrition-os/inventory-loop` branch from post-Phase-3 `main`; run `cd mobile && npm test` and record the green baseline.
2. **Re-read Phase 3's "⚠️ Execution amendments"** (`docs/superpowers/plans/2026-07-29-nutrition-recommender.md`) and reconcile: this plan's Task 9 assumes `rank()` compares `roleRank` → `score.raw` → prep → name and that `EatNextInput`/`Candidate` are shaped as that plan wrote them. If amendments moved them, adapt Task 9's diff and record it below.
...
`Candidate` gains `stockRank: number` (0 in-stock, 1 unknown, 2 known-missing) and `expiringRank: number` (0 uses-expiring, 1 not). `candidate()` takes the optional map, computes both ranks, and appends stock reasons to `extraReasons`:
