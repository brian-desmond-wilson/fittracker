...
- [x] **Step 1:** Grep for leftovers of the old projection and views:
...
*(Executed. Both greps came back as expected — **Step 1 found nothing to fix**. Grep 1 returns three hits, all in `ViewFoodDetailsScreen.tsx` (the "Storage Type" label, the single-location "Location" row, and which threshold row to render) — presentation only, no quantity math. Line numbers as measured pre-commit were `:194/:195/:257`; the commit's own edits to that file move them to `:204/:205/:267`. The expectation text above names EditFoodScreen and `stockState.ts`, which the `grep -v stockState` and the local-variable spelling `storageType === 'single-location'` exclude; `ViewFoodDetailsScreen` was the file it should have named. Grep 2 returns only the drop migration. The task's real work was the amendment backlog — see "⚠️ Execution amendments → Task 11".)*

- [x] **Step 2:** `cd mobile && npx tsc --noEmit && npm test` → green. Commit any fixes:

```bash
git add -A mobile/src mobile/app
git commit -m "chore(nutrition-os): retire the legacy quantity mirrors and sweep the comment backlog"
```

(Skip the commit if Step 1 found nothing — do not create an empty commit.)

> **Commit message amended during execution, and this is a deliberate deviation from the message written above.** The original read `remove last storage_type quantity branches`. Step 1 found **zero** such branches, so that message would have described work that did not happen while omitting all the work that did — twenty files of mirror deletion, parser unification and comment repair drawn from the amendment backlog. Per house rule #4 the fence is corrected in place rather than the history being made misleading; the sweep is not a "skip the commit" case because the backlog, not the greps, was its scope.
...
>
> **⚠️ Widened again by Task 10 — the same root cause, now on the two highest-traffic surfaces.** Task 10 wires the stock map into `useEatNext`, so the "zero-row item projects 0" consequence stops being confined to the Meal Library modal and reaches the **Home card and the Meals screen**, which render unprompted. Pre-gate they show **"missing N ingredients" on essentially every Eat Next recommendation and no expiring-rescue reasons at all**. **This is presentation only, and unlike the Task 8 entry above it is neither a data hazard nor a functional loss:** ordering is unaffected (a universal `stockRank: 2` ties every candidate on that term, so `rank()` falls through to `raw` and Phase 3 order is preserved exactly), nothing is written, and no decrement behavior changes — Task 8's entry above already accounts for all of that. The distinguishing fact is the **surface**, not the cause. It adds nothing to the pre-gate cost list; it is here because it is what the owner will actually *see* first, and because a reader comparing the app to the spec before the apply would otherwise reasonably conclude the recommender is broken. Closes wholesale with section A. Full reasoning in "⚠️ Execution amendments → Task 10".
>
...
- [x] **Step 1: Pre-flight (read-only).** `npx supabase migration list` → exactly `20260730100000_inventory_locations_truth` pending (Phase 3's `20260729110000` must be APPLIED — else the merge-order precondition was violated; stop). Then run every query block in 1a–1c below **verbatim**, in order, and capture their full output.
...
- [x] **Step 2: Snapshot before applying (owner-run — this step WRITES).** Section A issues a hard `DELETE` against production. The plan must not silently rely on Supabase PITR being enabled, so take an explicit in-database snapshot first — this is the recovery path if post-verify finds something wrong:
...
- [x] **Step 3: Apply.** `npx supabase db push --yes`.
  ...
  - **✅ RESOLVED AT EXECUTION (2026-07-30) — the CLI *does* relay NOTICEs.** ~~**⚠️ Absent notice output is NOT a failure — and this is unverified.** The Supabase CLI applies migrations through a `pgx.SendBatch` pipeline, and pgx only relays server `NOTICE` messages when an `OnNotice` handler is configured; whether the CLI installs one **could not be confirmed**. So a silent, successful push is expected-possible and proves nothing either way.~~ The apply printed the per-item notices and the closing counts (`canonicalized: 15, legacy caches resynced: 7`), so the `OnNotice` hedge is retired: on this CLI version, **missing notices would in fact be a signal worth investigating**, not an expected-possible non-event. The operational rule is unchanged regardless — **Step 4 is the real gate**; do not treat seen-notices as verification. See "⚠️ Execution amendments → Task 12".
...
- [x] **Step 1:** `cd mobile && npx tsc --noEmit && npm test` — all suites green (stockState + extended eatNext + all prior). **✅ PASSED (2026-07-30, run post-apply): `tsc --noEmit` → 0 errors; `npm test` → 9 suites / 257 tests, no test file touched.** Re-run after Step 2b's edits and still green (comment-only changes, so this is a regression check, not new coverage).
- [ ] **Step 2 (owner, on device — Metro reload, free `--port`):** list/detail/edit show identical quantities for the same item; restock (both "from store" and location→location) updates instantly and survives refresh with both strata equal; "Use from pantry" now offered on the canary item; multi→single edit leaves exactly one location row (verify via detail view); expiring section lists the right items; Meal Library "In stock" badges + filter; MealDetail missing list; Eat Next top pick prefers an in-stock meal and names an expiring ingredient when applicable; log a meal consuming the canary → quantities drop coherently everywhere.
  ...
  **Plus four checks added by Task 11's quality review.** None of the four components Task 11 changed are testable in this repo (React screens; the Jest run covers pure TS libs only), so the device pass is carrying that weight: **(a)** edit a multi-location item, flip it to single-location, save, then re-open — confirm the hidden Ready/Total thresholds still hold their old values (Task 11 stopped writing them from a possibly-stale snapshot, so they must survive untouched); **(b)** on an item holding **two location rows in the same location**, open Restock — confirm both appear as separate selectable rows with their own quantities, and that the one tapped is the one debited; **(c)** type in the search box to filter the "Expiring soon" list and tap a row **with the keyboard still up** — it must navigate on the first tap, not just dismiss the keyboard; **(d)** pick one item with an expiration date and confirm the **list, detail and edit** screens all show the same calendar day.
...
- [ ] **Step 2a (added by Task 10): confirm the Eat Next surfaces recovered post-apply.** Pre-gate, the Home card and Meals screen render "missing N ingredients" on essentially every recommendation and no expiring-rescue reasons, because nearly every meal projects out-of-stock (see "⚠️ Execution amendments → Task 10", and the Task 10 paragraph in Task 12's banner). It is presentation-only and ordering is unaffected, so it needs no action before the apply — but it must be confirmed **gone** after it. Check on device: recommendations show **"in stock"** where the pantry actually holds the ingredients, at least one **"uses X — expires in Nd"** rescue line appears if any matched item is inside the 7-day window, and no meal with a stocked pantry still reads "missing N ingredients". If the copy has not recovered, the reconcile did not seed the rows this task's map reads — investigate before dropping the Step 3 snapshots.

  **Plus three checks added by Task 14** (the surfaces now *render* this state, so it is finally observable on device rather than only in `EatNextResult`): **(a)** the Home card and each "Suggested now" chip carry a stock badge — green **In stock** where the pantry actually holds the ingredients, amber **Missing N** otherwise, and **nothing at all** for a meal the engine knows nothing about. Nothing is the correct rendering of unknown; an always-present badge would mean the map is not reaching that surface. **(b)** Trigger an **emergency** context (past dinner, ≥400 cal behind) and confirm the red card reads coherently: §5.3.4 ranks emergency by rescue size and deliberately ignores availability, so the card can legitimately show an amber `Missing 5` while an assemblable alternative sits below it off-screen. That is by design and is a **new user-visible composition nobody has seen** — confirm it looks like a deliberate trade-off rather than a bug. **(c)** The expiring-rescue line (`Uses X — expires in Nd`). **⚠️ Expect to see NOTHING here on current data** — the owner's inventory has nothing in the 0–7 day window (everything is long-expired or 49+ days out), so its absence is correct, not a regression. To exercise it, set an expiration inside the window on an item one of the meals resolves to; the line must read exactly as `MealDetail` renders the same fact, and day 0 must read "expires today", never "expires in 0d".
- [x] **Step 2b: Remove the pre-migration hedges (code, one commit).** **Four** comments (three from Task 7, one added by Task 8 — the fourth site is listed after the original three) describe the *pre-reconcile* world and become false the moment Task 12 applies. **Task 11's sweep runs BEFORE Task 12, so it must NOT remove them** (they are still true when it runs) — that is why they land here instead. Delete the "before Task 12's reconcile / zero-row item projects 0" qualifiers, keeping the surrounding statements, at exactly these four sites:
...
```
  ...
  **⚠️ AS EXECUTED, THIS SQL IS A NO-OP — the tables were never created.** Task 12 Step 2 was satisfied by a **local JSON export** of both tables instead of the in-DB `create table as` (the operator had REST but not direct SQL access). `drop table if exists` will simply report nothing. **The equivalent cleanup is deleting the local export file** once the on-device checklist passes — same rule, same timing, same reason. Full detail in "⚠️ Execution amendments → Task 12".

- [ ] **Step 4:** Stop. Merge/push are the owner's calls.

---

### Task 14: Show the stock state on the Eat Next surfaces — ⚠️ added after acceptance testing, not in the original plan

**Why this task exists.** Task 13 Step 2a's on-device pass found the defect Tasks 9 and 10 could not: the engine computes `"in stock"` / `"missing N ingredients"` / `"uses X — expires in Nd"` and **neither Eat Next surface renders any of it.** `EatNextHomeCard.tsx:158` draws `top.reasons[0]` and nothing else — and `reasons` is `[contextReason, ...prepReason, ...stockReasons]`, so index 0 is *always* the context reason. `EatNextRow.tsx` (the "Suggested now" strip) renders no reasons at all. Spec §8 says Eat Next "carries" these strings; carrying them without ever displaying them does not deliver that intent. Live consequence: with only 2 of 22 inventory items concept-linked, zero meals are assemblable, and the Home card recommends a meal the owner lacks every ingredient for with nothing on screen saying so. **Availability stays a soft signal (spec §9) — the meal is still recommended, it just has to say it can't be made.**

**The shape of the fix — a TYPED field, not a smarter read of `reasons`.** `reasons` is a flat `string[]` whose positions shift (`prepReason` is conditional), so index-guessing or substring-matching is one conditional away from silently wrong. The verdict goes on the recommendation as data.

- [x] **Step 1 — `mobile/src/lib/eatNext.ts` (additive only).**
  - `Candidate` derives `stockRank`/`expiringRank` from the looked-up `EatNextStockInfo` and then discards it. Carry it: `stock?: EatNextStockInfo`, set in `candidate()` from **the same** `stockByMealId?.get(m.meal.id)` that already computes the ranks — one lookup, so the badge and the ranking tier cannot disagree.
  - Add `stock?: EatNextStockInfo` to `EatNextRecommendation`. `undefined` = **unknown**, matching the map's existing "absent entry = unknown" semantics; documented on the field, with no second way to be unknown.
  - Populate it at **every** construction site. There are exactly two, and the second is easy to miss: `toRecs` and the **inline** `goal_hit` protein-short site, which bypasses `candidate()`/`toRecs` entirely. Enumerate them by grep, not by memory — a missed site renders no badge in that context and nothing else notices.
  - **Do not touch** `stockReasons`, the `rank()` comparator, `stockRank`/`expiringRank`, or any existing reason string.
  - Add `eatNextStockBadge(stock)` — the one definition of the badge's copy and its green/amber split, so the two surfaces cannot word the same verdict differently. `null` = unknown → render nothing.
- [x] **Step 2 — `mobile/src/components/EatNextHomeCard.tsx`.** Badge beside the meal name in `headerRow`: `undefined` → nothing; `assemblable` → green **`In stock`**; otherwise → amber `Missing ${n}`. Colors in `StyleSheet.create` (file idiom), reusing `track/meals/library/styles.ts`'s `badge`/`inStockBadge` treatment (`#22C55E` green, `#F59E0B` amber). The title already has `flexShrink: 1` + `numberOfLines={1}`; the badge must not be squeezed to zero width by a long name.
- [x] **Step 3 — `mobile/src/components/track/meals/EatNextRow.tsx`.** Same badge on each of the ≤2 chips, one notch denser (this surface stacks chips under the pace lines).
- [x] **Step 4 — tests.** `eatNext.ts` is a pure lib; the components are **not** testable here (`testEnvironment: "node"`, no React testing deps — do not add any). Pin: the field is populated for a meal present in the map (both verdicts); `undefined` with no map; `undefined` for a meal absent from a *present* map; populated at the `goal_hit` inline site; and the badge helper's three states plus the `Missing 0` degradation. **Mutation-test every new assignment** (drop it, hardcode `undefined`, flip `assemblable`) and confirm a test dies for each.
- [x] **Step 5 — gates.** `cd mobile && npx tsc --noEmit` → 0 errors; `npm test` green; every pre-existing test passing **unmodified** (test-file diff additions-only). Baseline into this task: 9 suites / 257 tests.

```bash
cd mobile && npx tsc --noEmit && npm test
git add src/lib/eatNext.ts src/lib/__tests__/eatNext.test.ts \
        src/components/EatNextHomeCard.tsx src/components/track/meals/EatNextRow.tsx \
        ../docs/superpowers/plans/2026-07-29-nutrition-inventory-loop.md
git commit -m "feat(nutrition-os): show stock state on the Eat Next surfaces"
```

**Not in scope, and stated so it is not mistaken for an oversight:** the `goal_hit` inline site still carries no stock *reason strings* (Task 9's recorded gap). This task gives it the badge; closing the reason gap means unifying that site with `toRecs`, which is a different change.
...
- **The honest failure mode.** Under rank 1 a broken meal can surface above a real meal the user genuinely can't assemble — and surfacing it is the *recovery path*, since `updateMeal`'s comment notes item-less meals are "visible in the UI and recoverable by re-editing". Under rank 2 it is buried in the bottom tier behind copy (`missing 0 ingredients`) that any user reads as a rendering bug and ignores.
- **⚠️ CORRECTION — the containment argument this amendment first shipped was self-contradicting and half false.** It read: *"an item-less meal has 0 calories, so it is band-filtered out of `catch_up` entirely and sinks on `raw` elsewhere. Rank 1 does not put it anywhere near the top of a real recommendation set."* The `catch_up` half is true and re-verified: `CATCH_UP_BAND = 0.35` and the filter is `Math.abs(totals.calories - gap) <= gap * CATCH_UP_BAND` (`eatNext.ts:376`), which is false at `calories = 0` for every `gap > 0` — and since the nudge's in-band pick shares `catchUpCandidates`, it is excluded there too. **"Sinks on `raw` elsewhere" is false where it matters.** `rank()` compares `a.stockRank - b.stockRank` (`:313`) strictly *before* `b.score.raw - a.score.raw` (`:318`), so against any known-missing meal the item-less meal wins before `raw` is ever consulted. This amendment's own new test asserts exactly that: `itemLess` (score 70) ranks **above** `knownMissing` (score 99). In `next_meal` the pool filters on category only (`:550`), so **if every real meal in the category is out of stock, the item-less meal is recommendation #1** — carrying no reason string saying anything is wrong. The nudge's out-of-band fallback (`:630`) has the same exposure and can name a 0-calorie corrupt meal in a push body reading "fixes it in N min". The sentence immediately above already named this failure mode honestly; the parenthetical then denied it. It is struck.
- **🚨 And the window where that bites is not rare — it is the pre-Task-12 norm.** Until the reconcile runs, most single-location items have zero location rows (Task 6's Finding A), so `fetchMealLibrary` projects them at `totalQuantity: 0` and **most meals assess as non-assemblable** (the `⚠️ Until the Phase 4 reconcile runs` note in `mealLibrary.ts` says exactly this). "Every real dinner is out of stock" is therefore the expected state in that window, not an edge case, and an item-less meal reaching #1 is likely rather than hypothetical. It closes wholesale when section A seeds the rows at Task 12. **The DECISION still stands** — an item-less meal requires the specific non-atomic `updateMeal` failure to exist at all, so the expected count for this single user is zero, and rank 2 would be a false claim in every window, not just this one. But the containment has to be stated honestly: what bounds the damage before Task 12 is the rarity of item-less meals, **not** the ranking.
- **The option not weighed in the first draft, and why omission still wins.** A third design was available: keep `stockRank: 2` and gate `stockReasons` on `missingCount > 0`. That buries the corrupt record *and* drops the nonsense copy — strictly better than doing nothing, and it would have removed the `next_meal` #1 exposure above. It is rejected because it keeps the false half of the claim: rank 2 still asserts "we positively established you can't make this" about a meal whose ingredients we never saw, and it re-splits the verdict from the copy so the two can drift apart again (the exact shape of the Task 8 FIX 1 / Task 9 divergence this task exists to close). Honesty about what is known beats burial of a record that is rare by construction — but the trade is real and the alternative deserved recording rather than silence.
...
**Mutation table — RE-MEASURED.** The first draft of this table mislabeled its Before column: it read "the tree with the plan's Step 1 only (89 tests)", but plan-Step-1-only is **86** tests in `eatNext.test.ts` (measured by restoring the file from `1b76d3a` and running it against the landed source), and 89 was the tree with three of the four new tests already added. Rather than fix the caption, the whole Before column was re-run against the true 86-test baseline — which turns out to be the stronger finding. Every mutant applied individually to `eatNext.ts`, run, then reverted; `git status --porcelain` confirmed free of residue before each commit.

**"Before" = the plan's Step 1 exactly as written: hook wired, not one test added (86 tests). "After" = final (90).**

| # | Mutant | Before (86) | After (90) |
|---|---|---|---|
| A | **write key** `map.set(meal.id, …)` → `map.set(meal.items[0].saved_food_id, …)` | **SURVIVED** | KILLED (3) |
| B | **read key** `.get(m.meal.id)` → `.get(m.meal.slug)` | KILLED (14) | KILLED (17) |
| C | **delete the `items.length === 0` skip** (the DECISION) | **SURVIVED** | KILLED (1) |
| D | `missingCount: a.missing.length` → `0` | **SURVIVED** | KILLED (2) |
| E | `expiringItemName` dropped to `null` | **SURVIVED** | KILLED (1) |
| K | `expiringDaysLeft` dropped to `null` | **SURVIVED** | KILLED (1) |
| J | skip condition inverted (`> 0`) | **SURVIVED** | KILLED (3) |
...
| L | **`expiringItemName: a.assemblable ? null : a.expiringItemName`** | **SURVIVED** | KILLED (1) |
| M | **`barcode: it.savedFood.barcode` → `null`** | **SURVIVED** | KILLED (1) |
| N | `barcode: it.savedFood.barcode` → `""` (falsy-guard variant of M) | **SURVIVED** | KILLED (1) |
| O | `name: it.savedFood.name` → `""` | SURVIVED | **SURVIVED — provably equivalent** |

**🚨 Read the Before column as a whole: NINE of the ten mutants survived the plan's Step 1 as written, and the one that died (B) is Task 9's, not this task's.** Every mutant touching the code Task 10 adds — the write key, the item-less skip, and all four field mappings — was invisible. That is the same finding Task 9 round 2 reported one level up (a call site is not covered because some test exercises its context) applied one level down again: **the builder was not covered because the recommender it feeds is.** It is also the concrete cost of the plan defect above — had the block stayed inside the hook, that entire column would still read SURVIVED and there would be no way to change it.

**Round 2 (quality review) — rows L, M and N were added after review found two ordinary production paths that round 1's fixtures never executed.** Both survived the **full 116-test** run across `eatNext.test.ts` + `stockState.test.ts`. Round 1's mutants were all aimed at code it had just written; these two are aimed at code the fixtures never reached, which is the harder class to see. Each row re-measured here, not transcribed from the review.

- **L — the flagship §9 rescue was never produced by the builder.** `expiringRank` bites **only inside `stockRank: 0`**: `rank()` compares stock first, and its own comment scopes the tiebreak to "the meals you can make" (`eatNext.ts:311-312`). So `{assemblable: true, expiringItemName: "…"}` is the *only* combination that makes the rescue feature do anything — and the round-1 field-mapping test drove only the `assemblable: false` branch (2 of its 3 items missing). Task 9's hand-written-map tests pin how the recommender *handles* that combination; nothing pinned the builder ever *emitting* it. The new test asserts the four-field verdict **and** that the rescue beats a 35-point raw deficit between two in-stock meals, with the exact reason arrays on both.
- **M/N — the barcode passthrough is the dominant production path and was entirely unexecuted.** Every round-1 fixture set `barcode: null` on both meal items and inventory rows. Barcode is **terminal regardless of stock** and outranks concept matching (`inventoryResolution.ts:33-45`), so dropping it silently downgrades every barcoded saved food to the concept route — which under-matches by design (2+ candidates → skip). Saved foods scanned via OpenFoodFacts all carry one, so this is the common case, not an edge. The new test gives the item and the row `conceptIds: []` so **only** the barcode can resolve it. N is included because `resolveInventoryMatches`' guard is `it.barcode ?` (truthy, not `!= null`), so `""` is a distinct mutant from `null`; both now die.
- **O — `name` is a provable equivalent mutant. Do not log it as a gap.** `ResolutionItem` has no `name` field (`inventoryResolution.ts:10-14`), so resolution cannot read it; `expiringItemName` is taken from the **inventory row's** `name`, not the item's; and the builder keeps only `a.missing.length`, discarding the array whose contents are the one thing `name` feeds. It is therefore unobservable *through the builder*. `assessAssemblability` itself **is** name-sensitive and `stockState.test.ts` pins that directly (`missing: ["Korean BBQ Sauce"]`), so the behavior is covered where it is observable.

- **B is the seam mutation the task demanded, and it confirms the two sides agree.** `.get(m.meal.slug)` killed **17** tests on the final tree — the **14** Task 9 round 2 established (re-measured against the restored `1b76d3a` test file, matching that amendment exactly) plus **3** added here (the counterfactual test fails under it too), i.e. the new builder tests independently re-pin the read key. A alone proves the write key: without it, keying by `saved_food_id` would pass, because the fixture deliberately names saved foods `sf-*` while meals are `m{n}` — the same fixture-collision that let `.get(m.meal.slug)` survive 81 tests in round 1.
...
**Deliberately routed to Task 11 (quality-review findings, none of them defects):**

- **#6 — `buildStockByMealId`'s JSDoc documents the DECISION and not the CONTRACT.** ~40 lines on *why* item-less meals are omitted, zero on *what to pass*. Three things are unstated and unvalidated, all of them correct-as-designed but invisible: a `conceptIdsBySavedFoodId` miss defaults to `[]` (so the item resolves to nothing and counts as missing — the intended under-claiming, but a caller passing a partial map gets silent wrong answers); `inventory` is expected to be the **whole** inventory, not pre-filtered per meal (`assessAssemblability` indexes it by id for the expiring scan); and a `meals`/`inventory` inconsistency is absorbed as "missing" rather than rejected. Trusting input is the right call for a single-caller pure function — a three-line `@param` block is the fix, not validation.
- **#7 — the not-shared/cadence rationale is now in three places near-verbatim** (`useEatNext.ts:285-310`, the `eatNext.ts` JSDoc, and this amendment): 26 comment lines above a one-line call. The problem is drift, not length — and the "not shared with `MealLibraryModal`" argument sits beside only **one** of the two computations it compares, so an edit to the modal's `assemblabilityById` will not surface it. Fix is to leave the pointer at the call site and hold the reasoning in one place. **Also a correction to my own wording:** "deliberately inside `load` rather than a `useMemo`" overstates the choice. `library` is a function-local awaited value that never enters state, so `useMemo` is not an available alternative without restructuring the hook. The conclusion (per-fetch is the right cadence) is right; the framing implies a decision that was never on the table.
- **#5 — `expect([...stockByMealId.keys()]).toEqual([...])` in the key-seam test is redundant** with the ordering assertions that follow, and couples the test to fixture order for no gain; `size` plus two `has()` would say the same thing. (The identical assertion in the item-less test **earns its place** — there it is the assertion that proves omission, not a restatement.)

**⚠️ RESIDUAL GAP, measured rather than assumed — the hook's own wiring is still unpinned, and no BEHAVIORAL test in this repo can pin it.** Deleting *both* the `buildStockByMealId(library)` line and the `stockByMealId,` argument from `useEatNext.ts` leaves `tsc` at **0 errors** (the field is optional; `noUnusedLocals` is off, so even the orphaned import passes) and **255/255 tests green**. Verified by applying it, not inferred. The constraint is structural: `jest.config.js` is `preset: "ts-jest"`, `testEnvironment: "node"`, `testMatch: **/__tests__/**/*.test.ts`, and the project has **no** `@testing-library/*`, `react-test-renderer` or equivalent (checked against `package.json`, not assumed) — so a hook cannot be rendered. Extraction shrank the exposure — the untested surface went from ~25 lines of inline logic to a two-token call site whose absence is obvious on sight — but it did not remove it. **Two pins do exist and were considered, both rejected:**
>   1. **A source-text guard** — `fs.readFileSync("src/hooks/useEatNext.ts")` asserted to contain both `buildStockByMealId(library)` and `stockByMealId,`. Genuinely reachable: `roots: ["<rootDir>/src"]` already covers the file, and reading it as text imports no React Native. Rejected as a test of spelling rather than behavior — it passes for a call whose result is discarded, and it breaks on a reformat, which trains readers to delete it.
>   2. **Extracting the whole `recommendEatNext` input assembly** into a pure `buildEatNextInput(...)`, leaving the hook a one-line delegation. This *would* pin the composition, including the argument. Rejected as disproportionate: it rewrites Task 9-era and Phase 3-era code (the clock, the pace calls, the profile mapping) that this task has no mandate over, and Task 10 already carries one forced deviation. It is the right shape for whatever future task unifies that assembly — noted there rather than done here.
>
> Recorded so Task 13's sweep can eyeball those two lines rather than trust the suite.
...
- **The plan's fence compiles as written.** Checked explicitly, because `new Map(arr.map(() => [k, v]))` is a classic place for TS to infer `(string | T)[]` instead of a tuple: pasted verbatim into a scratch file with **no annotation at all** and its result passed straight to `recommendEatNext`, it type-checks — `new Map(...)` contextually types the callback's array literal as a tuple, so the inferred `Map<string, {…}>` is assignable to `Map<string, EatNextStockInfo>`. Sanity-checked by injecting a deliberate error into the same scratch file (`missingCount: a.missing`), which `tsc` duly reported, proving the file was being checked rather than silently skipped. (An earlier draft of this amendment recorded the weaker annotated form of the check; the annotation is not what the fence says, and the unannotated check is the one that supports the claim.) The container change is not a bug fix.
- **🚨 SEQUENCING CONSTRAINT this task creates — do not ship this bundle to the device before Task 12 applies.** The *cause* is already stated in `fetchMealLibrary` (`mealLibrary.ts:112-118`) and is not new: pre-reconcile, most single-location items have zero location rows, project `totalQuantity: 0`, and therefore make nearly every meal `assemblable: false`. What Task 10 changes is **where that lands**. Before this commit it was confined to the Meal Library modal (Task 8's surfaces, entered deliberately). After it, the **Home card and the Meals screen** — the app's highest-traffic surfaces, rendered unprompted — show **"missing N ingredients" on essentially every recommendation, with no rescue reasons at all**, because `expiringRank`/the "uses X — expires in Nd" line only fire for meals that resolve. **Ranking degrades gracefully** and this is worth stating precisely: a universal `stockRank: 2` leaves every candidate tied on the new term, so `rank()` falls through to `raw` and Phase 3 ordering is preserved exactly. It is **copy damage, not ordering damage**, and nothing is written or lost. But it is the phase's most *visible* pre-gate symptom, and it closes wholesale when section A seeds the rows. **Distinct from the Task 8 entry in Task 12's banner set** — that one records a *functional* loss (no decrement path survives, and the resulting count drift is silent and un-auditable); this one records a *presentation* change on a different, unavoidable surface. Same root cause, different blast radius. Routed to **Task 13's sweep** as an on-device check, and cross-referenced from Task 12's banner below.
...
The mechanical half (Step 1's two greps) found **nothing** — recorded at the step itself above, including the one inaccuracy in its own expectation text. The task's real scope was the backlog seven earlier tasks routed here, re-derived by grepping the amendment set for "Task 11" rather than by reading the Task 11 intro paragraph, which enumerates only the Task 5/6/7 lists and silently omits everything Tasks 4, 8, 9 and 10 added. **Nineteen items** were derived; sixteen fixed, three declined. Gates on the final tree: `tsc --noEmit` **0 errors**, **9 Jest suites / 257 tests**, **no test file touched** (`git diff --stat -- mobile/src/lib/__tests__/` is empty). No database-connecting command was run at any point.

**⛔ BOUNDARY HELD — the four pre-migration hedges Task 13 Step 2b owns were found and left byte-untouched.** The callout at the top of this task names them; all four were located and verified present after the sweep: `foodInventoryMatchService.ts` `findInventoryMatchByBarcode`'s "Pre-Task-12 note" paragraph; `foodInventoryMatchService.ts` `consumeOneInventoryUnit`'s "Where the two still differ …" sentences; `MealsScreen.tsx`'s refund-arming "Not yet identical, though …" sentences; and `mealLibrary.ts` `fetchMealLibrary`'s `⚠️ Until the Phase 4 reconcile runs (Task 12) …` paragraph. **Verified mechanically, not by eye:** `git diff -U0` over those three files was filtered for deletions matching the hedges' distinguishing phrases and returned zero, and the complete deletion list for those files is seven lines, every one of them an intended edit. They are still true today and only become false at the apply.

**🔴 The headline item — `inventoryResolution.ts`'s false docstring — is fixed, and fixed by stating the invariant rather than deleting the clause,** as the hand-off required. "Sum of location quantities, or the legacy quantity for location-less rows" is replaced by a paragraph that says Σ location rows is the only truth, that a zero-row item is genuinely 0 rather than a cue to consult the cache, and that restoring a `locations.length > 0 ? … : quantity` arm re-arms the consume RPC's legacy branch. It names both producers (`mealLibrary.ts`, `foodInventoryMatchService.ts`) so a future reader can check the claim.

**Two dead payload fields dropped from queries, each with the grep that proves nothing reads it.**

- **`storage_type` out of `InventoryMatchSummary` and out of the select.** `grep -rn "\.storage_type" ` over `FoodPreviewModal.tsx` and `MealsScreen.tsx` — the only two consumers of the type — returns **zero hits**. The `Omit<InventoryMatchSummary, "quantity">` cast's column-for-column property that Task 7 established was re-checked **programmatically** after the edit (parse the select string, parse the interface, compare the sets): selected `{barcode, brand, id, name, unit}` vs `Omit` requires `{barcode, brand, id, name, unit}` — MATCH.
- **`quantity` (the legacy cache) out of `fetchMealLibrary`'s select and out of `InventoryRowRaw`.** `grep -n "\.quantity" mobile/src/lib/supabase/mealLibrary.ts` returns exactly one hit and it is inside a **comment** (the "the `r.quantity` fallback this replaced is gone deliberately" line). The Task 8 amendment recorded a counter-argument to dropping it; on inspection that argument points the *other* way and is why this was taken — an absent column means the removed fallback cannot be restored without also editing the query, which is one more deliberate step between a future reader and the divergence this phase closed. The reason is now stated at the select.
- **`is_ready_to_consume` at `mealLibrary.ts` — KEPT, and it is not dead payload.** Spec §9 prescribes selecting it, which the hand-off said to check first; but the stronger reason is that the hand-off's premise is wrong. `projectItemStock`'s `locations` param is `StockQuantityRow = Pick<StockLocationRow, "quantity" | "is_ready_to_consume">`, so the field is **required** by the call three lines below — dropping it from the select would either break the call or force widening the projection's input type. Nothing downstream of the projection reads the ready/storage split, which is what made it look dead. Documented in place so the next sweep does not re-open it.

**The legacy mirrors are gone, and the compiler enumerated the readers rather than a list doing it.** `total_quantity` / `ready_quantity` / `storage_quantity` are deleted from `InventoryItemWithState` and from `fetchInventoryWithState`'s row construction. Readers found and moved: `ViewFoodDetailsScreen.tsx:187,190,191` → `item.state.*`; `RestockModal.tsx` ×4 → `item.state.totalQuantity`; the two synthetic literals (`add.tsx:57-59`, `preview.tsx:69-71`) simply deleted, self-resolving exactly as Task 5 predicted; `FoodInventoryScreen.tsx`'s optimistic restock row stops assigning them. **The one non-obvious consequence was surfaced by `tsc`, not by grep:** with the grep-found readers already moved, deleting the fields produced one further error — `EditFoodScreen.tsx(134,21)`, because `useFoodImages` demanded a whole `FoodInventoryItemWithCategories` while reading only four `image_*_url` fields. Its param is now a `Pick<>` of exactly what it reads, the same "make the type state what it consumes" property Tasks 7 and 8 argued for.

*(Evidence claim narrowed after review, because the first draft overstated it. It read "deleting the fields produced **exactly one** error … that single error is the evidence the sweep was complete." That is not true as stated: the edits were not applied in that order. `ViewFoodDetailsScreen.tsx:187,190,191` were already typed `InventoryItemWithState` at `c91e8c1` and would have produced errors too had they not been moved first, on the strength of the grep. So the honest statement is the weaker one — **grep found the readers and `tsc` proved the list complete afterwards**, rather than `tsc` finding them unaided. The conclusion survives: all seven pre-commit mirror readers are accounted for, and once the fields are gone any missed reader is a compile error by construction. Recorded rather than quietly reworded, because this amendment set's standard is that verification claims be literally true — including the ones whose conclusion happens to hold.)*

- **Consequence taken rather than left dangling: `FoodInventoryItemWithLocations` and `FoodInventoryItemWithCategories` are deleted from `types/track.ts`.** With `RestockModal` retyped to `InventoryItemWithState` and `useFoodImages` narrowed, a repeat grep showed both types referenced **only by each other** — zero production importers. They were the client mirror of a view this phase drops, which makes them the most literal reading of this task's title. A comment records what replaced them; it deliberately does **not** spell the view's name, because doing so put a permanent false positive into this task's own Step 1 grep 2 (caught by re-running the grep after the edit, and reworded).

**The four sibling numeric fields: validated, not just re-parsed.** The hand-off called the fix "a straight reuse of the existing helper at four call sites". Reuse alone is not enough — `parseQuantityInput` returns `null` for both "empty" and "unwritable", so swapping it in for `parseInt` would have converted a silently-cleared threshold into a *differently* silently-cleared threshold. `restock_threshold`, `fridge_restock_threshold`, `total_restock_threshold` and `calories` are now **validated** in `handleSave` before the write (non-empty must parse, else a named alert and the owning section expands), and the write uses the same helper, so validation and write cannot disagree — the property Task 6 FIX 1 established for `quantity`. All four are `INTEGER` in the schema, verified by grep before choosing the acceptance rule: `calories` (`20250206_tracking_tables.sql:54`), `restock_threshold` (`20250209_extend_food_inventory.sql:19`), the two multi-location thresholds (`20250217000003:57-58`).

**The acceptance table was produced by extracting the shipped helper and running it, not by reasoning about it** — and it found the old behaviour to be worse than the hand-off described. The hand-off said the failure mode was a silently cleared threshold. It is that, plus three more:

| input | old (`parseInt` write) | new |
|---|---|---|
| `" "` | `null` — **CLEARED** | `null` (treated as empty/not set) |
| `".5"` | `null` — **CLEARED** | rejected with an alert |
| `"1.5"` | `1` — silently truncated | rejected with an alert |
| `"abc"` | `null` — **CLEARED** | rejected with an alert |
| `"-1"` | `-1` — 23514 on three of the four; **silently stored on `restock_threshold`** | rejected with an alert |
| `"1e3"` | `1` — **1000× under-write** | `1000` |
| `"99999999999"` | overflows int4 (22003) | rejected with an alert |
| `"0"` / `"5"` / `"2147483647"` | correct | unchanged |

*(The `"-1"` row was corrected after review: the first draft said it violated `CHECK (>= 0)` on all four. Three carry that CHECK — `calories` `20250206_tracking_tables.sql:54`, `fridge_restock_threshold` / `total_restock_threshold` `20250217000003:57-58` — but **`restock_threshold` is `INTEGER DEFAULT 1` with no CHECK** (`20250209_extend_food_inventory.sql:19`), so `-1` was written silently there rather than rejected. Worse than the row originally claimed, on the one field it got wrong. Also recorded, per review: whitespace-only `restockThreshold` now writes **`0`** where the old code wrote `null` — the `" "` row above describes the three nullable siblings, not this one. Kept as `0` deliberately: that column is the only non-nullable one of the four in the TS type (`restock_threshold: number`) and `0` preserves the historical "empty means 0" intent, whereas `null` contradicted the declared type. Stated at the write site.)*

**`formatExpirationDate`'s UTC parse was a live, user-visible defect for this owner, confirmed by execution.** A new `parseLocalDate` in `src/lib/dates.ts` replaces `new Date("YYYY-MM-DD")`. Run in the machine's own zone (`America/Los_Angeles`): `2026-08-15` rendered **"Aug 14, 2026"** before and "Aug 15, 2026" after; `2026-01-01` rendered "Dec 31, **2025**", i.e. the year was wrong too. Every stored `DATE` this app formats for display must go through the new helper.

**The `src/lib/**` → `src/components/**` import is closed, with a deliberately minimal blast radius.** The claim was verified before acting rather than inherited: `grep -rn 'from "@/src/components' mobile/src/lib/` returns exactly one hit, `mealLibrary.ts:6`. `getLocalDateString`'s definition moved to the new `src/lib/dates.ts`; `mealsHelpers.ts` imports and re-exports it so the **thirteen** existing component/route call sites are untouched; `mealLibrary.ts` now imports from `../dates`. The hand-off's "five call sites to move" would have meant editing fourteen files across the workout, schedule and profile trees for zero behavioural gain — churn the sweep's own ground rules warn against. The layering violation, which is the actual defect, is gone. (Note the re-export needs a local `import` as well: `export { x } from "y"` creates no local binding, and `mealsHelpers` calls the function six times itself.)

**The "Expiring soon" section is bounded by scroll, not by a cap — and the cap the hand-off suggested would have made it worse.** The `"expired"` band has no lower bound, so a six-month-expired in-stock item stays pinned; and because the section is fed by a list sorted soonest-first with expired days **negative**, those stale rows sort to the **top**. A "first N + k more" cap therefore fills every visible slot with the least actionable rows and hides the item expiring tomorrow. A `maxHeight` with internal scroll bounds the chrome above the grid (the stated problem — "~25 rows of chrome") while keeping every row reachable and requiring no product decision about when an expired item stops mattering. The header gains a count. `ScrollView` was already imported into that file and unused; it is now used.

**The duplicate-location restock rows: fixed properly, by the contract change Task 5 called "too wide for a review round".** `RestockModal`'s `onConfirm` now hands back a **location row id** (or `null` for "from store") instead of a `FoodLocation | "store"` name, and its source list is built from the rows themselves keyed by id. This was taken rather than deferred because it **removes** code: `handleRestockConfirm`'s name-based lookup, its `.id !== targetLocation.id` disambiguation and its "Could not find source location" fallback all delete, as do the two matching guards inside the modal and the three comments explaining why resolving by name is hazardous. Net effect: a second stocked row in the same location is now individually selectable instead of rendering a duplicate React key and being unreachable; the row the user tapped is by construction the row the RPC moves. Behaviour is otherwise identical — traced on both paths, including that `isValid`'s store arm is preserved by giving the store option `quantity: Infinity`, and that the preview's source row stays suppressed for the store.

**Smaller items, each fixed:**

- **`MealDetail.tsx:115` now gates on `expiringItemName != null`**, matching `eatNext.ts`'s `expiringRank`. Task 9's amendment is right that the two surfaces disagreed for `expiringItemName === ""`, and right about which way: under truthiness the detail line vanishes while the recommender still ranks the meal as a rescue — the recommender ordering on a signal the UI never states, which is the exact divergence Task 9's FIX 3 removed on the other side.
- **The three `?? 0` sites** (`FoodPreviewModal.tsx:86,396`, `MealsScreen.tsx:567`) are gone. `InventoryMatchSummary.quantity` is non-optional `number`; the coalesce was dead and `tsc` confirms.
- **The grid sort keys off `state.daysLeft`.** Ordering is provably identical for well-formed dates — `daysLeft` is `daysBetweenLocalDates(today, expiration_date)`, monotone in the date, against a single `todayLocalDate` sampled once per fetch — and strictly better for a malformed one, which the projection normalises to `null` and which now sorts to the end instead of producing `NaN` and an implementation-defined position. §8's letter, and one fewer source of date truth on the screen.
- **`location: null` vs `"pantry"`.** A single `const singleLocation = location ?? "pantry"` now feeds both the location row and `food_inventory.location`, so the display column cannot disagree with the row holding the stock. Matches the reconcile's own `coalesce(fi.location, 'pantry')`, so a pre-gate save and section A now agree on the same value.
- **`stockState.ts`'s file header** described only the projection; it now names both exports, since Task 2 added `assemblability` to the same module and Task 10 made `buildStockByMealId` a third consumer.
- **`buildStockByMealId`'s JSDoc gained the input contract** (#6): `conceptIdsBySavedFoodId` must be complete or items silently resolve by barcode only; `inventory` must be the whole inventory, not pre-filtered; inconsistencies are absorbed as "missing", never thrown.
- **The triplicated cadence rationale (#7) is down to one home.** `useEatNext.ts`'s 26-line block is now a 12-line pointer; the not-shared argument moved into `buildStockByMealId`'s doc; and — the half the amendment said was missing — `MealLibraryModal`'s `assemblabilityById` gained a four-line pointer, so an edit to *that* map now surfaces the comparison. The "deliberately inside `load` rather than a `useMemo`" overstatement is corrected to the true reason (`library` is a function-local awaited value that never enters state, so `useMemo` was never on the table).

**Declined, with reasons:**

- **#5 — the redundant `expect([...stockByMealId.keys()]).toEqual([...])` in the key-seam test.** Not touched. The gate for this task is "9 suites / 257 tests, **every existing test unmodified**", and removing an assertion is modifying one. The finding is also self-limiting: the amendment that raised it already establishes the assertion is harmless and that its twin in the item-less test earns its place. Deferring costs nothing; a gate violation for a cosmetic test edit is a bad trade. **Deferred to whichever future task next edits `eatNext.test.ts` for its own reasons.**
- **The int-vs-continuous-units schema question.** Recorded by Task 6 as explicitly **not** a Task 11 fix — a product decision needing a `numeric` migration and a second owner gate. Untouched, correctly. Note that this sweep *widens* its surface slightly: the four threshold/calorie fields now reject `"1.5"` the way `quantity` already did, so the same "int-only column, continuous units" tension is visible in four more places. That strengthens the case for the future decision rather than changing it.
- **`StockLocationRow`'s zero production importers.** Recorded by Task 8 as explicitly not a cleanup target. Untouched.
- **Pre-existing unused React Native imports** noticed in passing (`View`/`Text`/`TouchableOpacity`/`StatusBar`/`StyleSheet`/`ChevronLeft` in `add.tsx`, `ShoppingCart` in `FoodInventoryScreen.tsx`). Unrelated to this phase; deleting them would widen the diff into files this sweep otherwise has no business in. Recorded, not fixed. (`FoodInventoryItem`/`FoodLocation` in `FoodInventoryScreen.tsx` **were** removed — those were orphaned *by this commit*.)

**Two things this sweep could not verify and the next task should not assume it did.** `RestockModal`, `FoodInventoryScreen`, `ViewFoodDetailsScreen` and `EditFoodScreen` are React components: this repo's Jest run covers pure TS libs only, so the restock contract change, the expiring section's scroll behaviour, the new threshold alerts and the four moved quantity reads are **`tsc`-verified and reasoned, not executed**. They are named in Task 13 Step 2's on-device checklist territory and are the reason that checklist matters more after this commit than before it. Second: the `⚠️ RESIDUAL GAP` Task 10 recorded — the hook's two-token wiring of `buildStockByMealId` — is unchanged here and still needs Task 13's eyeball.

**Self-review finding, folded into the same commit — the UTC-parse fix was incomplete, and shipping only half of it would have CREATED a divergence.** After committing, a grep for other `new Date(<YYYY-MM-DD>)` display sites found `ViewFoodDetailsScreen.tsx:63-70`'s `formatDate`, which had the identical defect and was rendering `item.expiration_date` at `:287`. Fixing the grid alone would have left the list saying "Aug 15" and the detail screen saying "Aug 14" **for the same item** — precisely the one-item-two-answers class this phase exists to remove, and something Task 13 Step 2's "list/detail/edit show identical … for the same item" check would have caught on device at best. The subtlety that made it worth splitting rather than search-replacing: the same `formatDate` also served `created_at` and `updated_at`, which are **TIMESTAMPTZ** — full ISO instants that `new Date` resolves correctly and that `parseLocalDate` would corrupt. It is now two named functions, `formatCalendarDate` (DATE columns, via `parseLocalDate`) and `formatTimestamp` (TIMESTAMPTZ), each documented with which column type it is for. **The general lesson matches this amendment set's standard: a fix applied at the site the hand-off named is not the same as a fix applied to the defect.** The hand-off named one site; the defect had two, and the second was only found by grepping for the *shape* of the bug after fixing the first.

**Review-round fixes (second commit) — spec review verified the boundary, the mirror deletion, the type deletions, the `useFoodImages` narrowing, the `getLocalDateString` move, the `formatCalendarDate`/`formatTimestamp` split (including that `parseLocalDate` really would corrupt a TIMESTAMPTZ — `"2026-07-30T02:00:00+00:00"` is July 30 local under `parseLocalDate` and July 29 under `new Date`), the RestockModal id contract, and the `is_ready_to_consume` reasoning. Five issues; the first is the serious one because the first commit CREATED it.**

- **🚨 FIX 1 — there was a THIRD UTC-parse site, and fixing only two introduced the divergence the fix existed to remove.** `EditFoodScreen.tsx:116` seeded the picker with `new Date(item.expiration_date)`. Before `382db3c` all three surfaces were consistently *wrong*; after it the grid and detail read "Aug 15" while **edit still read "Aug 14" for the same item** — exactly the one-item-two-answers class invoked to justify widening past the hand-off in the first place, and a guaranteed failure of Task 13 Step 2's "list/detail/edit show identical … for the same item" check. **The previous round's claim "the hand-off named one site; the defect had two" was false — it had three**, and the third was in a file that commit already had open. Fixed at `:116` with `parseLocalDate`. Executed, not reasoned, in this machine's zone (`America/Los_Angeles`): for `2026-08-15` all three now render Aug 15 (was Aug 14 / Aug 14 / Aug 14 pre-`382db3c`, and Aug 15 / Aug 15 / **Aug 14** after it); for `2026-01-01` all three render Jan 1 2026 (was Dec 31 **2025** — the year was wrong too).
- **FIX 1b — the secondary write hazard at the same site was real, and the two fixes are COUPLED.** `:585` wrote `expirationDate.toISOString().split("T")[0]`. The iOS spinner is `value={expirationDate || new Date()}` and returns the picked date **carrying a time component**, so a first-ever pick made at 17:00 local converts to the next UTC day: picking Aug 15 wrote **`2026-08-16`**. Measured across four seeds. Fixed with `getLocalDateString(expirationDate)`, which formats the local calendar fields and is therefore independent of the time component entirely — strictly better than relying on the noon anchor, which only covers dates seeded from an existing value. **Coupling worth naming: `getLocalDateString` alone would have REGRESSED the UTC-midnight seed to Aug 14** (measured), so `:116` and `:585` had to land together. A reviewer taking only the second half of this bullet would have shipped a new bug.
- **FIX 2 — the new threshold validation could dead-end a save on an invisible field. Scoped to the active storage type, and the write scoped with it.** "Restock Threshold" renders single-location-only and "Ready"/"Total" multi-location-only, so validating all four unconditionally meant: type garbage into Ready Threshold, flip to single-location, and the save is blocked by an alert naming a field that no longer renders, with `setExpandedSection("storage")` opening a section that does not contain it. **Chosen fix: scope, not clear.** Clearing the inactive fields on flip destroys data on an innocent round-trip (flip to look at something, flip back, thresholds gone), and it treats a display toggle as a mutation. Scoping states the rule the UI already implies — *you can only be blocked by what you can edit*. The write is scoped the same way, so validation and the write still agree on exactly one set of fields: the inactive pair carries `item.*` through untouched rather than re-parsing hidden text, which means a save can no longer change a threshold the user could not see. That is better than either option the review offered — the hidden field is **preserved** (measured: stored `3` survives the flip) rather than silently nulled. Walked concretely: the dead-end scenario now saves successfully, every "bad visible field" case still blocks with the field's real on-screen label in the section that contains it, and both all-valid flows are byte-identical to before. Alert copy also now matches the on-screen labels ("Ready Threshold", not "Fridge restock threshold").
- **FIX 3 — the four new `errors.add(key)` calls were inert; wired rather than dropped.** `validationErrors` was consumed only for `name`/`quantity`/`locationEntries`, so the four keys were written and never read — state that reads as if it highlights a field and does not. Wiring was chosen over deleting because the state was the honest half and the missing binding was the defect: the four inputs now take `styles.inputError`, and the storage/nutrition `SectionHeader`s take the matching `hasError`. (The nutrition header was hardcoded `hasError={false}`.)
- **FIX 4 — two accuracy defects in the previous round's own evidence, both corrected in place above** with the correction recorded rather than the text quietly rewritten: the `"-1"` row over-generalised a `CHECK (>= 0)` that `restock_threshold` does not carry, and "deleting the fields produced exactly one error" was true only because the grep-found readers had already been moved.
- **FIX 5 — the whitespace-only `restockThreshold` micro-delta is recorded** (old `null`, new `0`), with the reason it is kept, at both the write site and the acceptance table.

**Observed and deliberately NOT fixed — the same UTC-parse bug class exists outside this phase.** `WeightScreen.tsx:335`, `ProgressPhotosScreen.tsx:320` and `MeasurementsScreen.tsx:156` each do `new Date(dateStr)` on what the schema declares `date DATE NOT NULL` (`20250206_tracking_tables.sql:51,95,131,168,209`), so they render a day early in negative offsets exactly as the three inventory sites did. Different tables, different features, no relation to the inventory loop — fixing them here would push a Phase 4 cleanup commit into the weight/photos/measurements surfaces. Recorded so it is not lost, and `parseLocalDate` now exists in `src/lib/dates.ts` for whoever takes it.

**Quality-review fixes (third commit) — the risky parts held under tracing (mirror deletion: no stale-object path across all four components; the date fix: all five round trips walked, DST- and year-end-immune because neither helper touches UTC; the RestockModal id contract, which also silently retired a latent `LOCATION_LABELS[null]` render; the sort-key equivalence; exactly one `ScrollView` in the file, so no nested-scroll trap). Two Important, one Minor, one judgement call, three recordings.**

- **🚨 FIX 1 — the inactive thresholds were REWRITTEN from a stale snapshot, and the previous round's comment claiming otherwise was false.** Round 2 wrote `isSingle ? … : item.fridge_restock_threshold`, described as carrying the value "through untouched". It does not: the column stays in the UPDATE payload, filled from the `item` prop — equivalent to untouched only if `item` is fresh, and the update path **deliberately makes it stale**. When `replaceItemLocations` fails, the "Stock Not Saved — Tap Save again" branch returns without `onSave()` and keeps the screen mounted, by which point the `food_inventory` UPDATE has already committed. Walked concretely: DB `fridge_restock_threshold = 2` → user sets Ready Threshold to `5` and saves → UPDATE commits `{fridge_restock_threshold: 5}` → `replaceItemLocations` throws → screen stays open with `item` still holding `2` → user flips to single-location and re-saves **as the copy instructs** → the old payload was `{restock_threshold: 1, fridge_restock_threshold: 2, total_restock_threshold: 10}`, silently reverting the 5. **Fix:** `itemData` now omits the inactive keys entirely via a conditional spread, so the payload is `{restock_threshold: 1}` and the column is genuinely not touched — measured, `hasOwnProperty("fridge_restock_threshold") === false`. Absence has no stale-snapshot failure mode; a value read from a prop always does. **Create path confirmed unaffected and handled explicitly:** an INSERT must supply every column it wants set, so the three thresholds are re-supplied at the `.insert()` call site from `item.*` — which there is the synthetic literal in `add.tsx`/`preview.tsx`, built fresh each render and structurally incapable of being stale — with `...itemData` spread *after* them so the active threshold still wins. Without that the INSERT would have silently taken the DB default (`restock_threshold INTEGER DEFAULT 1`) instead of the intended 0 for a new multi-location item.
- **🚨 FIX 2 — the sweep left half the pattern four lines below its own fix.** `protein`/`carbs`/`fats`/`sugars` still ran `parseFloat` behind a truthiness check. The review is right that Task 6's "cosmetic" classification of these neighbours is **superseded by this commit's own reasoning**: post-round-2 the same `itemData` literal behaved two ways, `calories` rejecting `"abc"` with a named alert while `protein` silently deleted the column and reported "Item updated successfully". **Fix:** a `parseDecimalInput` sibling — `Number`, finite, `0 <= v <= 99999999.99` — run through the same visible-fields loop. **Deliberately NOT `parseQuantityInput`:** these are `DECIMAL(10, 2) CHECK (>= 0)` (`20250206_tracking_tables.sql:55-58`, re-declared `20250209:12-15`) and grams are genuinely continuous, so `"1.5"` is real data and rejecting it would be a regression — the opposite call from the integer siblings, for a reason in the schema rather than in taste. Enumerated by extracting the shipped helper and running it:

| input | old (`protein ? parseFloat(protein) : null`) | new |
|---|---|---|
| `""` | null (not set) | null (not set) |
| `"0"` | 0 | 0 |
| `"1.5"` | 1.5 | 1.5 |
| `"1,5"` | **1** — silent 1.5 → 1 truncation | rejected with an alert |
| `"abc"` | **null — silently CLEARS the column** | rejected with an alert |
| `"-1"` | −1 → 23514 CHECK violation | rejected with an alert |
| `"1e3"` | 1000 | 1000 |
| `"999999999999"` | 22003 numeric overflow | rejected with an alert |

- **FIX 3 — `keyboardShouldPersistTaps="handled"` on the expiring list.** `ScrollView` defaults to `"never"`, and the section is filtered by `searchQuery`, so search-then-tap is the natural flow and the first tap was being spent dismissing the keyboard.
- **FIX 4 (judgement call) — TAKEN: the section now sorts rescue-first internally.** The review is right that the argument against a "+k more" cap applies equally to the scroll — the section inherited `filteredItems`' soonest-first order, expired days are negative, so three long-expired items fill the visible five and push tomorrow's expiry below the fold. Sorting *within the section* (still-rescuable `daysLeft >= 0` ascending, then expired most-recent-first) fixes it without touching the grid order. One rule: nearest to today wins, and the future beats the past. Taken rather than declined because leaving it would have meant the file embodying an objection the previous round raised and then ignored.
- **FIX 5 (#5) — DECISION: the five non-component callers were moved.** `useEatNext.ts` and the four `app/(tabs)/track/food-inventory/*` routes now import `getLocalDateString` from `@/src/lib/dates` directly. The strict layering defect (`src/lib/** → src/components/**`) was already closed, but `dates.ts` advised non-component callers to import from the new module while five of them did not — the "state that lies" class this whole sweep exists to remove, and all five are inside the surface this task owns. Five sole-import lines, zero risk. After it, **every remaining `mealsHelpers` importer is under `src/components/**`**, so the re-export shim now serves exactly the population its comment claims. `app/workout/[id].tsx` is untouched and is *not* an exception: its `getLocalDateString` is a different local definition in `workout-session/helpers.ts`. (Two further duplicate definitions exist in the schedule modals; consolidating all three is unrelated to this phase and is not done here.)
- **#6 — recorded: `4d448b9`'s subject is incomplete.** "third UTC date site and the hidden-threshold save dead-end" names two of its three changes and omits FIX 3, the wiring of the four inert validation keys, which is roughly 40% of that diff. The amendment above records it correctly; the subject does not. Noted rather than rewritten, since the commit is already in history.
- **#7 — cosmetics taken.** `isSingle` is now bound once, above the storage-type validation branch, and the duplicate `storageType === 'single-location'` spelling is gone (one binding, `:488`). `DATE_FORMAT`/`formatTimestamp`/`formatCalendarDate` are hoisted to module scope in `ViewFoodDetailsScreen.tsx` so they stop being re-created every render.

**Consistency note carried forward from FIX 3.** Now that the four macros are validated, their inputs and the nutrition `SectionHeader` were given the same `styles.inputError` / `hasError` wiring the integer fields got, so all nine numeric fields on the screen behave identically: named alert, section expands, offending field highlighted.

### Task 12

The owner opened the gate on **2026-07-30** and the migration `20260730100000_inventory_locations_truth` is **applied to production**. Steps 1–4 all ran; the apply succeeded on the first attempt; assertion D passed. Two things about *how* it ran deviate from the plan as written and are recorded below rather than quietly absorbed. The headline finding is the first bullet.

- **🚨 THE DESTROY-GUARD FIRED ON A REAL PRODUCTION ROW. This is the single most consequential outcome of the entire review process.** Step 1b's classifier, run over 22 items / 17 location rows, returned exactly one **`STOCK DESTROYED`** row: **"Bananas"** — `storage_type = single-location`, `item.location = cabinet`, legacy `quantity = 0`, and one location row holding **6 units in `pantry`**. Under §4's winner rule section A would have deleted that row and re-inserted one canonical row at `fi.quantity = 0` in `cabinet`: **6 units destroyed, irreversibly.** The guard added by Task 3's second review round (`raise` when `r.loc_total > r.quantity`, `20260730100000:51-53`) would have **aborted the entire apply** on that item. Both halves of that review round are therefore vindicated on live data, and it is worth stating exactly what each one bought: (i) the **classifier** (Step 1b's `section_a_effect` labelling — the previous query sorted `difference = 0` rows to the bottom and would have shown this row as unremarkable) is what surfaced it *before* the apply, turning an abort into a five-minute decision; (ii) the **in-migration guard** is what would have caught it had the pre-flight been skipped or misread. And the reason both were needed is the Task 3 amendment's `🚨 CRITICAL` finding: **assertion D cannot detect this class by construction.** Had the guard not existed, section A would have written the sole location row *from* `fi.quantity`, D would have compared that number against itself, reported `0 = 0`, and **certified the apply as successful while 6 units of real stock had just been deleted.** The plan's own post-verify would have agreed. Nothing else in the plan — no notice, no post-verify query, no on-device check — could have distinguished "destroyed 6 units" from "was already empty" after the fact.
  - Scale check on the rest of the classification: `STRATA COLLAPSED: 0`, `LOCATION MOVED: 0`. So the lossy-but-spec-prescribed classes were empty and Bananas was the only decision to make.
- **The Bananas resolution was the owner's call, and the evidence made it an easy one: trust the legacy `0`, delete the stray row.** The row's `notes` field read **`"Migrated from single-location"`** — the exact marker `migrate_single_location_items()` writes, which confirms it as a **backfill leftover** rather than genuine stock the user placed there. That is precisely the provenance the Task 3 analysis had *hypothesised* for this class without being able to check it; the hypothesis held. The row was deleted **by id, under guards that would have aborted on any mismatch** (rather than by a predicate that could have matched more than intended), and Step 1b was then **re-run**, returning `STOCK DESTROYED: 0` — i.e. the pre-flight was re-satisfied from scratch rather than assumed to be fixed. Recorded as an owner decision because it is one: the alternative reading (trust the location row, raise the legacy column to 6) was available and was declined on the strength of the `notes` evidence.
- **The plan's expected canary behaved exactly as predicted, which is the boring half of the result and worth recording as such.** "Propel Fitness Water" (legacy 0 / locations 60) is `multi-location`, so §4's winner rule hands it to the location rows and section A never touches it. The Task 3 amendment's inference — that the canary *must* be multi-location for both §4 and the post-verify to hold, argued from `EditFoodScreen.tsx:474` parking the legacy column at 0 for multi-location items — is **confirmed by measurement**. Post-verify: legacy 60 = Σ locations 60.
- **Step 1a's single-user preconditions all passed:** `auth_user_count: 1`, `items_outside_target: 0`, `cross_user_location_rows: 0`. Phase 3's `20260729110000` showed **APPLIED** and `20260730100000` was the **only** pending migration, so the merge-order precondition held. All five views were present pre-apply and `transfer_inventory_units` was absent — both as expected.
- **Task 6's Finding A is confirmed at scale, with numbers.** The pre-flight found **10 single-location items holding 74 units with zero location rows**: Chicken Crumbles 20, Ravioli 12, Sirloin 11, Spinach & Artichoke 10, Oats 8, Breakfast Sandwich 5, Guacamole 3, Oikos 2, Milk 2, Pesto 1. That is not a corner case — it is **the majority of the single-location inventory**, and it is the population the pre-gate banner's "saving one of these destroys its stock" warning was written for. Post-apply those items hold their real quantities in real rows (spot-checked: Chicken Crumbles 20 @ fridge, Ravioli 12 @ fridge, Sirloin 11 @ fridge), which is section A's seeding path working exactly as specified.
- **⚠️ DEVIATION — Step 2's snapshot was taken as a local JSON export, not the in-DB `create table as`.** The operator had REST access but not direct SQL access, so the two `zz_backup_*` tables were never created; instead both tables were exported in full to local JSON (**22 items × 29 columns, 17 location rows × 9 columns**). **Step 2 is satisfied, by different means** — the undo capability is equivalent (a complete row-level copy of everything section A can touch, taken before the apply), and it is arguably better positioned, since a local file cannot be reached through PostgREST at all. Consequences for the plan text, so a reader does not go looking for tables that do not exist: **(i) Task 13 Step 3's `drop table` block is a no-op** — there is nothing to drop; the equivalent cleanup is deleting the local export once the on-device checklist passes. **(ii) The RLS hardening the previous review round added to Step 2** (`enable row level security` + `revoke all … from anon, authenticated`, added because `create table as` produces RLS-disabled tables that Supabase's default grants expose to the anon key) **is moot for this execution.** It was correct and stays in the plan for anyone who runs Step 2 as written — but it defended a hazard this execution never created. **(iii) Step 2's restore block was never exercised** and remains untested SQL; restoring from the JSON export would be a hand-written operation, which is a real (if now-academic) downgrade from having the restore pre-composed. It was not needed: nothing in post-verify failed.
- **✅ The NOTICE-relay hedge is RESOLVED — the CLI does relay them.** Step 3 and the Task 3 amendment both recorded, explicitly as **unverified**, that the Supabase CLI applies through `pgx.SendBatch` and that pgx relays server `NOTICE` messages only with an `OnNotice` handler configured, so a silent successful push would prove nothing. **`npx supabase db push --yes` relayed the notices.** Both sites are marked resolved in place above rather than deleted, since the hedge was load-bearing when written. **The companion claim is NOT resolved and keeps its hedge:** the "a mid-apply abort leaves prod byte-identical because the file is one implicit transaction" inference rests on the same `pgx.SendBatch` mechanism, but the apply **succeeded on the first attempt**, so no abort was ever observed. Confirming one half of a two-part inference does not confirm the other, and the plan now says so at both sites.
- **Apply and post-verify results.** Closing counts: **`canonicalized: 15, legacy caches resynced: 7`**. No exception from section A's guard and none from the post-condition; **assertion D passed**. Step 4, read-only: the §6.1(4) post-condition query returned **0 violations** across **22 items / 29 location rows** (17 → 29 is section A/B seeding the 10 zero-row items plus the canonicalisation churn); the canary reads 60 = 60; Bananas reads 0 with one `cabinet` row at 0; **all five views are gone** (Step 1c query → 0 rows); `shopping_list` survives as a **table** (it was only ever the `shopping_list_active` *view* that was dropped — worth stating, because the two names are one character apart in the drop list); and `transfer_inventory_units` exists and rejected a `quantity: 0` probe with **`P0001 transfer quantity must be positive`**, which exercises its guard rather than merely confirming the function's existence.
- **📌 CORRECTION — Task 12's pre-gate cost enumeration never listed restock, and it should have.** The banner at the top of this task enumerates what breaks between a Task 5/6 bundle landing and the apply, and it was revised three times (Task 7 narrowing the barcode path, Task 8 narrowing the Meal Library path, Task 10 widening to the Eat Next surfaces). **It never mentioned "Restock Fridge".** `FoodInventoryScreen.tsx:282` calls `transferInventoryUnits` (`inventory.ts:75-88`), which invokes the `transfer_inventory_units` RPC — **created by this very migration**. For the entire pre-apply window that RPC did not exist, so every restock returned **PGRST202** and failed. **Non-destructive**, and this is the load-bearing detail: `transferInventoryUnits` does `if (error) throw error`, and the throw **precedes** the optimistic `setItems` call at `:290`, so the UI never shows a transfer that did not happen — the failure is loud and total, not silent and partial. It is **moot now** (the RPC exists and was probed in Step 4). It is recorded because the enumeration's *purpose* was to let the owner decide what to avoid touching, and an incomplete enumeration undermines that even when every omitted item turns out to be harmless. The general lesson matches this phase's pattern: **the banner was revised whenever a task narrowed or widened a path it already listed, and never re-derived from scratch against "what does the migration create that the client already calls?"** — which is the question that would have found this.
- **Gates after the apply:** `cd mobile && npx tsc --noEmit` → 0 errors; `npm test` → **9 suites / 257 tests** passing. No test file was touched.


### Task 13

Step 1 passed (`tsc --noEmit` 0; 9 suites / 257 tests). Step 2b executed below. Steps 2/2a/3/4 are the owner's.

- **Step 2b — the four hedges are retired, but two of them were REPLACED rather than deleted, which is a deliberate deviation from this step's own wording.** Step 2b says "delete the paragraph entirely" for the `findInventoryMatchByBarcode` and `fetchMealLibrary` sites. Deleting them outright would have been a net loss: at both sites the paragraph's *surrounding* claim is "there is no legacy-cache fallback here", and the deleted text was carrying the **reason a reader must not add one back**. This phase has repeatedly found that a comment stating the invariant is the thing that stops a future reader re-introducing the defect, so the temporal clause was removed and the **now-true statement of the invariant** written in its place. What each site got:
  - `foodInventoryMatchService.ts` — `findInventoryMatchByBarcode`'s "Pre-Task-12 note" paragraph is gone. **Replaced with:** every item now holds ≥1 location row (reconcile seeded them, write paths maintain them), so a projected 0 is a genuine out-of-stock, and re-adding a fallback would re-arm the divergence — "do not add one". The "Task 5 took the same direction for the grid" cross-reference was dropped as scaffolding, not reasoning.
  - `foodInventoryMatchService.ts` — `consumeOneInventoryUnit`'s "Where the two still differ … unreachable from the barcode path" sentences are gone, **and so is "the gate can only be more conservative than the RPC, never more optimistic"** — that sentence read as permanent but was not: it was the *summary of the residual gap* the deleted sentences described, and post-apply the gap it summarised no longer exists. In its place the docstring now states that the RPC's legacy-column branch is unreachable **in practice** (not merely from the barcode path) because every item holds a location row. **Kept verbatim and load-bearing:** "Callers MUST NOT infer that from their own pre-check", the TOCTOU reason (the gate is a separate, earlier read; stock can move between the two), the no-such-row / RLS-filtered case, "a false return means nothing moved — do not compensate for it with a refund", and "a 0 result is never an error". The TOCTOU reason is now explicitly flagged as *why the rule below stands*, since it is the only surviving justification once the divergence argument is retired.
  - `MealsScreen.tsx` — the refund-arming comment's "Not yet identical, though … which is what makes it safe" sentences are gone, replaced by the plain post-reconcile claim (gate and RPC read the same truth). **"Arm on outcome, never on intent" and all three of its reasons are kept verbatim** — the earlier-separate-read TOCTOU, the RPC's 0-for-no-such-row/RLS, and refund's unconditional credit. This is the rule Step 2b singles out as permanent and it is untouched.
  - `mealLibrary.ts` — the `⚠️ Until the Phase 4 reconcile runs (Task 12) …` paragraph is gone. **The two sentences above it are kept verbatim** ("Location rows are the ONLY quantity truth (spec §5.1) … restoring it would re-arm it"), plus one added line stating that the reconcile seeded a row for every item, so a 0 here is a genuine out-of-stock rather than a missing row.
- **Two further comments were falsified or made ambiguous by the apply and are fixed in the same commit, beyond the four Step 2b names.** Found by grepping the phase's hedge vocabulary (`Task 12`, `pre-apply`, `until the`, `not yet`, `reconcile runs`, `unapplied`) across `src/`:
  - `EditFoodScreen.tsx:861` read "the stale-cache case that would otherwise **block Task 12's reconcile**". The reconcile has run; there is nothing left to block. The *hazard* is not gone, though — a failed cache resync still leaves the item's two quantity sources disagreeing — so the clause was rewritten to state that consequence directly instead of pointing at a migration that no longer exists in the future tense. Note the class of the error: the sentence would have read as **still meaningful** to a future reader, just wrong about why, which is worse than an obviously stale reference.
  - `mealLibrary.ts:100` cites "**Task 12's** builder does `+ delta`". That is the **Meal Library plan's** Task 12 (`MealBuilder`, `setServings` — `2026-07-29-nutrition-meal-library.md:2296`), not this phase's migration apply — and it sits 20 lines above a paragraph that was, until this commit, about *this* phase's Task 12. Not falsified, but a genuine collision: disambiguated in place with the plan name and an explicit "not this phase's migration apply". **Recorded because bare cross-plan task numbers are an ongoing hazard in this repo's comments**, not just here.
- **One stale comment found and deliberately NOT changed.** `useEatNext.ts:157` says the Home-card / Meals-screen refetch behaviour is "per the plan (Tasks 8 and 10 — **not yet written**, so this is the plan's stated intent rather than verified code)". Tasks 8 and 10 have landed, so the parenthetical is stale — but it is stale from **Task 11's sweep**, not from the apply, and correcting it honestly means *verifying the refetch behaviour* rather than just deleting the hedge. Deleting the hedge without doing that work would upgrade an unverified claim to a verified-looking one, which is the opposite of what these comments are for. Left as-is and recorded here; it is a Phase 5 cleanup, and the stale-response guard it annotates is correct either way.

### Task 14

Added after Task 13's acceptance testing, so unlike Tasks 1–13 there was no plan fence to land against — the task text above was written from the on-device finding and is recorded in the plan body so this doc stays a complete record rather than one that stops at the last *planned* task. No database-connecting command was run at any point (the migration was already applied at Task 12; there was nothing to apply and nothing to verify by connecting). Gates on the final tree: `tsc --noEmit` **0 errors**; **9 Jest suites / 270 tests** (was 257 — **+13, all additive**). `git diff --numstat` on the test file is **`221 0`** and on `eatNext.ts` **`75 0`** — **zero deleted lines in either**, so every pre-existing test passes byte-unmodified and the lib change is strictly additive: `stockReasons`, the `rank()` comparator, `stockRank`/`expiringRank` and every existing reason string are untouched, as the task required.

**🚨 THE FINDING IS THE TASK: three review rounds across Tasks 9 and 10 verified that the stock strings were PRODUCED, and not one asked whether they were DISPLAYED.** Task 9 shipped 18 mutants over two rounds; Task 10 shipped 14 more, including a re-measured Before column proving nine of ten survived the plan as written. Between them they pinned the reason strings' content, their **order**, their pluralization, the day-0 special case, five call sites, and both sides of the lookup key. Every one of those tests asserts on `EatNextResult` — the engine's return value — and **`EatNextResult` was never rendered by anything under test.** `EatNextHomeCard.tsx:158` drew `top.reasons[0]`, which is *always* the context reason because `reasons` is built `[contextReason, ...prepReason, ...stockReasons]`; `EatNextRow.tsx` drew no reasons at all. So the app shipped a fully mutation-tested feature that was invisible on both of its surfaces.

The reason no review caught it is worth naming precisely, because it is a **generalisation of the lesson those same two tasks kept re-learning one level at a time.** Task 7's FIX 1: *a call site is not covered because some test exercises its context.* Task 9 round 2: *coverage of a context is not coverage of the wiring inside it.* Task 10: *the builder was not covered because the recommender it feeds is.* Task 14 is the last link in that chain and the one none of them reached — **the engine was not covered as a feature because its return value was covered as a value.** Each round moved the boundary of "what counts as wired" one hop closer to the user and then stopped at the edge of what Jest can execute; the surfaces sit one hop past that edge, so the question never got asked. Task 10 even *recorded* a "RESIDUAL GAP … no BEHAVIORAL test in this repo can pin it" for the hook's own wiring, and Task 13 Step 2a queued an on-device check for the exact copy at issue — the gap was visible from inside and still nobody asked what the card actually printed. **It took the owner looking at his phone.** The structural fix is not more tests; it is that a surface must branch on a typed field it cannot render wrong by accident, which is what this task installs.

**Both `EatNextRecommendation` construction sites were enumerated mechanically, by two independent greps, and each is now pinned by its own test.** `grep -rn "EatNextRecommendation" src app` returns the interface, the `EatNextResult` field, and `toRecs`' return type — no other file constructs one. `grep -rn "mealId:" src app` (the field no `EatNextRecommendation` literal can omit) returns exactly two object literals in `eatNext.ts`: **`toRecs` (`:358` pre-change)** and the **inline `goal_hit` protein-short site (`:464` pre-change)**. The second is the dangerous one — it bypasses `candidate()` and `toRecs` entirely, is terminal and single-pick, and every stock test in the file drives a *non*-terminal context, so a miss there would render no badge in `goal_hit` with nothing failing. Reading the grep was not treated as evidence: M5/M6/M7 below mutate that site specifically.

**Mutation table.** Every mutant applied to `eatNext.ts` individually by script, run against `npx jest eatNext.test.ts`, then reverted from a pristine backup; `git status --porcelain` confirmed free of residue before committing. The script also asserts each anchor string matches **exactly once** before substituting, so a silently-missed mutation cannot be mistaken for a survival.

| # | Mutant | Result |
|---|---|---|
| M1 | `toRecs`: **drop** `stock: c.stock` | KILLED (4) |
| M2 | `toRecs`: `stock: c.stock` → `undefined` | KILLED (4) |
| M3 | `candidate()`: **drop** `stock: info` | KILLED (4) |
| M4 | `candidate()`: flip `assemblable` on the carried info | KILLED (4) |
| M5 | **`goal_hit` inline site: drop `stock`** | KILLED (1) |
| M6 | **`goal_hit` inline site: `stock: undefined`** | KILLED (1) |
| M7 | **`goal_hit` inline site: read the map's FIRST entry, not this meal's** | KILLED (1) |
| M8 | `eatNextStockBadge`: unknown returns a badge instead of `null` | KILLED (2) |
| M9 | `eatNextStockBadge`: swap the `assemblable` branch | KILLED (6) |
| M10 | `eatNextStockBadge`: `missingCount > 0` → `>= 0` (prints "Missing 0") | KILLED (1) |
| M11 | `eatNextStockBadge`: drop the count from the label | KILLED (4) |

M7 is the one that needed inventing rather than deriving: M5/M6 die to any assertion that the field is populated at all, so on their own they would leave a site that populates the field from the *wrong* lookup indistinguishable from a correct one. The test that kills it hands in a present map keyed to a **different** meal and asserts the `goal_hit` pick reads `undefined` — the same shape of fixture-collision hazard Task 9 round 2's `scored()` `slug` fix existed to remove, applied to a construction site instead of a lookup key.

**⚠️ RESIDUAL GAP, measured rather than assumed — the render itself is still unpinned, and this repo structurally cannot pin it.** Deleting the entire badge JSX block from **both** components leaves `tsc --noEmit` at **0 errors** (the `stockBadge`/`badge` locals become unused, and `noUnusedLocals` is off) and **270/270 tests green**. Verified by applying it, not inferred. The constraint is the one Task 10 recorded: `jest.config.js` is `preset: "ts-jest"`, `testEnvironment: "node"`, `testMatch: **/__tests__/**/*.test.ts`, and the project has no `@testing-library/*` or `react-test-renderer` (checked in `package.json`, not assumed) — so a component cannot be rendered, and the task text explicitly forbids adding the dependency. **What the design does instead is shrink the untestable surface to something that cannot be silently wrong.** Each component reduces to `eatNextStockBadge(rec.stock)` plus a style lookup: the copy, the green/amber split, the three-state branch and the `Missing 0` degradation are all in a tested pure function, so the only thing left unpinned is *whether the JSX is present*, which is visible on sight in a four-line diff rather than buried in a ternary. That is the same trade Task 10 made when it extracted `buildStockByMealId` out of the hook, and it is stated here so the next reader does not mistake "270 green" for "the badge renders". **Confirming it renders is an on-device check** — appended to Task 13 Step 2a's list in spirit: the Home card and each "Suggested now" chip must carry a badge, green where the pantry has the ingredients and amber otherwise.

**DEVIATION — `eatNextStockBadge` is a new export the task text did not ask for.** The task specified the copy (`In stock` / `` `Missing ${stock.missingCount}` ``) and told each component to branch on `stock`; the literal reading is two hand-rolled ternaries, one per file. Rejected because **two copies of user-facing copy in the two surfaces of one feature is the same class of defect this task exists to fix** — a wording or color change made on the Home card and missed on the chip row is exactly "one surface under-reports, the other says something else", and nothing would fail. Putting it in `eatNext.ts` also makes it the only part of the change that Jest can execute: M8–M11 (four of eleven mutants, and the entire `Missing 0` defensive behavior the task asked for) exist *because* the decision is in a pure function. Additive, so it violates none of the "do not change" constraints. **⚠️ CORRECTION (round 2, flagged by both reviewers) — this paragraph originally justified the cost as "`eatNext.ts` now holds one presentation string, which is a boundary this file had otherwise kept clean". That is false, and false in my own favour.** `EMPTY_LIBRARY_MESSAGE` (`:30`) is user-facing copy in this file and is *exported precisely so a surface can compare against one source*; so are the context messages (`"Eating window closed for today."`, `"Target hit — nothing needed."`, `` `~${gap} cal to go before day's end` ``, `` `${gap} cal behind pace` ``), the nudge title and body, and every §9 reason string. The badge copy is **consistent with this file's long-standing role**, not a new boundary crossing — which makes the extraction *less* costly than claimed, not more. An amendment's authority rests on its measurements being measured; a cost invented to make a decision look hard-won is the same defect as a benefit invented to make it look easy. **The real reason the copy belongs here and not in the components' `styles.ts` is narrower and does transfer:** `styles.ts` imports `react-native`, so anything placed there is unreachable by `testEnvironment: "node"` — the pure decision has to live in a file Jest can load. **The `scoreBand` precedent cited alongside it only half-holds** and is corrected here too: `scoreBand` returns a *band*, not copy, so it is a precedent for the split (shared decision, per-surface palette) but not for the string. What actually transfers is the comment above `scoreBand`'s import, not its return type.

**DECISION — `missingCount === 0` on a non-assemblable meal renders `Missing items`, not `Missing 0`.** The task called for handling it "sanely rather than rendering 'Missing 0'". Three options were weighed: (a) render nothing — rejected, it makes an unmakeable meal indistinguishable from an unknown one on the surface that exists to tell them apart; (b) fall back to the green "In stock" badge — rejected outright, it is the rank-0 over-claim the Task 10 DECISION already argued against, made worse by being visible; (c) drop the count, keep the amber. (c) ships. It preserves the verdict (which is what the badge is *for*) and drops only the number, which is the part we cannot state. The state is not producible by `buildStockByMealId` — its item-less DECISION removed the one input that yielded it — but `EatNextStockInfo` is an **exported public input**, so the value stays constructible; this is the same "unproducible but constructible" framing as Task 10's `{assemblable: false, missingCount: 0}` counterfactual test, and it is tested from the same angle rather than left as a comment.

**Verified and deliberately left alone**, so a future reader does not "fix" them:

- **The `goal_hit` inline site gets the FIELD but still no stock REASON strings — Task 9's recorded gap is unchanged, and now pinned as unchanged.** The new test asserts `reasons` there matches neither `/missing/i` nor `/in stock/i`. That is a deliberate assertion of current behavior, not an endorsement: closing the reason gap means unifying that site with `toRecs`, which is a real change to a terminal context's copy and outside this task. Pinning it means a future task that *does* unify them has to update this assertion on purpose rather than discover the change afterwards.
- **The badge answers exactly one question — "can I make this now?" — and deliberately ignores `expiringItemName`/`expiringDaysLeft`.** A second badge (or a rescue-tinted first one) was considered and dropped: the expiring signal is already carried as reason copy (`uses X — expires in Nd`), it is *orthogonal* to assemblability (Task 9 FIX 3 made it append on **both** branches on purpose), and encoding two independent facts in one chip is how a surface starts lying about one of them. Pinned by a test that drives both verdicts with a rescue attached and asserts the badge does not move.
- **Nothing was done about `reasons[0]` still being the only reason the Home card renders.** The badge fixes the *availability* half of spec §8, which is what the owner found and what this task was scoped to. The prep-budget reason (`N min — over your prep budget`) and the expiring line remain invisible on both surfaces for the same structural reason. **This is a real, unfixed gap and it is recorded rather than quietly bundled**: fixing it is a copy/layout question (which reasons, how many lines, on which surface) that deserves its own decision, not a `.slice(0, 2)` added on the way past. Phase 5. **⚠️ SUPERSEDED IN PART by round 2 below — the expiring half shipped.** The reasoning above was sound *before* Step 1 and stopped being sound the moment Step 1 landed; see "Round 2, FIX 1". The prep-budget half stands.
- **`stock` is carried on `Candidate` and projected in `toRecs`, rather than re-looked-up in `toRecs`.** `toRecs` has no access to `stockByMealId` and giving it one would mean a second `.get(m.meal.id)` — a second read of the seam Task 9 round 2 proved fails silently, and the one way the badge could ever disagree with the tier the comparator used. The end-to-end test asserts `rec.stock` equals `stockByMealId.get(rec.mealId)` for **every** recommendation, driven through the real `buildStockByMealId`, which pins the agreement rather than the mechanism.
- **Layout with a long meal name.** On the Home card the badge sits in `headerRow` beside a title that already has `flexShrink: 1` + `numberOfLines={1}`, so the name truncates and the badge keeps its intrinsic width. `EatNextRow`'s chip previously had the name as a bare `Text`, so it gained a `chipHeader` row and the name gained `flexShrink: 1` (the only edit to an existing style line in either component). Both badges state `flexShrink: 0` **even though that is already RN's default for a `View`** — stated because a future `flexShrink: 1` added there would squeeze the badge toward zero width and silently undo this task, which is precisely the failure mode it exists to prevent, and a default is not a guard.

**Round 2 (second commit) — spec review returned "✅ compliant" (construction sites enumerated three ways, single-lookup confirmed, M4/M7/M9 independently re-run with matching kill counts, badge treatment verified byte-identical to `library/styles.ts`, §9 confirmed not violated); quality review found "no code defect" (layout measured across the whole ancestor chain — 40-char name, `Missing 12`, the emergency variant, Dynamic Type — and sound, because Yoga's `flexShrink` default is 0 and the title is the only shrinkable sibling). Both approved the `eatNextStockBadge` extraction. One Important finding, three Minor, four record-only.** Gates on the final tree: `tsc --noEmit` **0 errors**; **9 suites / 279 tests** (257 pre-task → 270 round 1 → 279 round 2, **+22 for the task**). Test-file diff against the pre-task tree (`7915700`) is **`351 0`** — **zero deleted lines**, so every one of the 257 pre-existing tests still passes byte-unmodified. No database-connecting command was run at any point.

**🚨 FIX 1 (Important) — the expiring line shipped. Round 1's deferral was right when it was written and stopped being right one commit later, which is the part worth recording.** Spec §10 (`…-design.md:144`) requires on device: *"Eat Next prefers an in-stock meal **and names an expiring ingredient**."* Round 1 shipped the first half and explicitly deferred the second, on the reasoning that *"fixing it is a copy/layout question … that deserves its own decision, not a `.slice(0, 2)` added on the way past."* That reasoning was about re-plumbing `reasons`, and it was correct **for a tree in which the expiring data was only reachable as a positional string**. Round 1's own Step 1 changed that tree: `rec.stock.expiringItemName` and `.expiringDaysLeft` became populated on **every** recommendation, read by **nothing**. So round 1 closed a "computed but never displayed" defect and reintroduced it one field over, in the same commit, while quoting its own finding as the justification. **The lesson is narrower and more useful than "we deferred too much":** a deferral is a claim about the cost of a change, and landing Step 1 *changed that cost* — from re-plumbing an array to one conditional reading two fields. Deferrals are stated against a tree; when the tree moves under them they have to be re-priced rather than inherited. The prep-budget reason was re-priced the same way and **stays deferred** — it still requires the `reasons` re-plumbing argument, because there is no typed field for it.

- **Copy is `MealDetail.tsx:120-125`'s to the character** — `Uses ${name} — expires today` / `Uses ${name} — expires in ${n}d`, em dash. That file was the app's *only* renderer of these two fields, one tap and one surface away inside the Meal Library modal. A second phrasing of one fact is the drift class this whole task exists to close, so the line states it the same way rather than better. Pinned by exact-string `toBe` assertions, not regexes: `/expires/` would pass for a rewording, which is exactly what must not happen silently. Mutants E3 (lowercase `uses`) and E5 (hyphen for em dash) exist because both are invisible to a regex and both die.
- **Gated on `expiringItemName != null`, never on the truthiness of `expiringDaysLeft`** (the Task 2 forward caution — `0` is the most urgent value and it is falsy). Mutant E1 applies that exact bug and dies on the day-0 test. `!= null` on the **name** rather than truthiness, matching `MealDetail` and `expiringRank`: an empty-string name still earns `expiringRank: 0`, so a truthiness gate would rank on a signal the UI declines to state. E6 applies it and dies.
- **DEVIATION beyond the fix as scoped — `expiryClause` was extracted, so `stockReasons` was touched after round 1 declared it off-limits.** Writing the new line would have created a **third** copy of the day-0 rule (`stockReasons`, the new line, and `MealDetail`'s JSX). Two of those three are in this file, eleven lines apart, and there is no argument for keeping them as duplicate ternaries while writing an amendment about copy drift. The extracted helper is shared by both; **both callers' output is byte-identical to what shipped before** — proven by the 257 pre-existing tests plus round 1's exact-reason-array `toEqual` staying green with no assertion touched, and by mutants E2/E10 killing in **3** and **7** tests respectively (i.e. the clause is pinned from both callers at once). `git diff 7915700 -- src/lib/eatNext.ts` deletes exactly **6** lines, all of them this extraction: three comment lines replaced by a fuller one, and the three-line ternary replaced by the shared call. **The lowercase `uses` in the reason fragment is NOT a third phrasing** and was deliberately kept per-caller: it is the same sentence as a list fragment rather than a standalone line, and it is the only thing that genuinely differs between the two.
- **`EatNextRow` gets the line too — the "does it fit" judgement the review asked to see stated.** It ships. The chip is already a full-width stacked block *because* a name plus a stats line will not fit a half-width pill (its own geometry comment says so), so the horizontal room exists; the line is `null` for every meal with no expiring ingredient, so in the common case the strip renders byte-identically to before; and at most two chips exist. `numberOfLines={1}` here against `2` on the Home card — a truncated rescue line still reads as a rescue and the full text is one tap away in `MealDetail`, whereas a wrapping one would push the second chip toward the fold. **The alternative (Home card only) was rejected on this task's own terms:** it would put the two Eat Next surfaces straight back into "one names the rescue, one silently does not", which is the defect this task exists to fix.
- **⚠️ It will not appear on the owner's device today, and that is not a failure.** His live inventory holds nothing in the 0–7 day window — everything is either long-expired or 49+ days out — so `expiringItemName` is `null` for every meal and the line correctly renders nothing. Recorded so its absence on the Task 13 Step 2 pass is not read as a regression. The way to see it is to set an expiration inside the window on an item a meal resolves to.

**FIX 2 (Minor) — a real survivor, closed.** `eatNextStockBadge`'s count was pinned only at `missingCount ∈ {0, 1, 3, 7}`, so a "cap the count at 10" edit (`Math.min(count, 10)`) passed all 270 tests. The badge is the one place a meal's shortfall is quantified and a silently clamped number is worse than none. `missingCount: 12` added to the existing assertion; mutant E9 now dies.

**FIX 3 (Minor) — `== null`, not `=== undefined`.** Both helpers now guard loosely, matching `stockReasons`' `if (!info)` for the same input type two functions away. Unreachable under `strict`, so a behavioural test needs an explicit `null as unknown as …` cast — added, and stated in the test as defensive-parity rather than a live path, because **without the cast the mutant is unkillable and the guard is decoration**. E7/E8 both die.

**FIX 4 (Minor) — two documentation inaccuracies corrected in place above,** both flagged by both reviewers. The numstat read `74 0`; actual was `75 0`. And the extraction's cost was justified as crossing "a boundary this file had otherwise kept clean" — **false, and false in my own favour**: `EMPTY_LIBRARY_MESSAGE`, four context messages, the nudge title/body and every §9 reason string are already user-facing copy in this file. The correction is written into that paragraph rather than appended here, along with the reviewers' sharper version of the argument (the copy belongs out of `styles.ts` because `styles.ts` imports `react-native` and is therefore unreachable by `testEnvironment: "node"`) and the note that the `scoreBand` precedent only half-holds — it returns a *band*, not copy, so it is a precedent for the split but not for the string.

**Round-2 mutation table.** Every mutant applied individually by script (which asserts its anchor matches exactly once before substituting, so a missed substitution cannot be misread as a survival), run against `npx jest eatNext.test.ts`, reverted from a pristine backup; `git status --porcelain` confirmed clean before committing. **26 mutants, 26 killed.** Round 1's eleven are re-run rather than transcribed, and the four the second reviewer designed are included — those four matter because **Task 14 changed what they test**: under Task 9 the map reaching those call sites only affected reason strings, and under Task 14 it decides whether a badge renders at all.

| # | Mutant | Result |
|---|---|---|
| M1–M11 | round 1's eleven, re-run on the round-2 tree | **all KILLED** (6/6/6/4/1/1/1/3/6/1/4) |
| F1 | **post_workout `candidate()` call site drops the map** | KILLED (1) |
| F2 | **emergency `candidate()` call site drops the map** | KILLED (1) |
| F3 | **`catch_up` context drops the map arg** | KILLED (1) |
| F4 | **nudge out-of-band fallback drops the map** | KILLED (1) |
| E1 | expiring line gated on truthiness of `expiringDaysLeft` | KILLED (2) |
| E2 | `expiryClause` drops the day-0 branch ("expires in 0d") | KILLED (3) |
| E3 | expiring line lowercased to `uses` (drift from `MealDetail`) | KILLED (6) |
| E4 | expiring line always returns `null` (never rendered) | KILLED (6) |
| E5 | em dash → hyphen | KILLED (6) |
| E6 | expiring line gated on **truthiness** of the name | KILLED (1) |
| E10 | `expiryClause` drops the `d` suffix | KILLED (7) |
| E9 | **badge caps the count at 10** (the FIX 2 survivor) | KILLED (1) |
| E7 | badge `== null` → `=== undefined` (FIX 3) | KILLED (1) |
| E8 | expiring line `== null` → `=== undefined` (FIX 3) | KILLED (1) |

E4 is worth a note on method: its **first** form (`if (true) return null;`) did not compile — it broke TS's narrowing rather than changing behavior — and a compile failure is **not** a kill, because it proves nothing about the tests. It was re-run as an equivalent that compiles (`void stock; return null;`) and then died on 6. A mutation run that silently counts compile errors as kills is measuring its own scaffolding; the script distinguishes the two and this row was re-measured by hand.

**Recorded, deliberately not fixed** (all four raised by the reviewers as record-only):

- **`rec.stock` ALIASES the live map entry — it is not a copy.** The same object is reachable from `stockByMealId`, from `Candidate.stock`, and from every recommendation for that meal. That is deliberate and is the property that makes the badge trustworthy (one lookup ⇒ badge and ranking tier cannot disagree), and it is read-only in practice — nothing in this file or either surface mutates it. But this file already documents exactly this hazard for `nudgeFireDate`'s `sourceDay` ("may be the exact object a caller is holding in state; mutating it would corrupt that state as a side effect of what looks like a pure read"), so the equivalent note is now on the `stock` field: treat it as frozen, copy before mutating.
- **The `emergency` context is a NEW user-visible composition that nobody has seen.** `toRecs` runs there over a calories-descending sort that deliberately ignores `stockRank` (spec §5.3.4 — a bigger rescue you must shop for beats a small one in the fridge), so the red emergency card can now show an amber `Missing 5` while an assemblable alternative ranks below it and off-screen. That is the spec working as designed and honesty beats silence — the previous behavior was to recommend the same meal while saying nothing. **Added as an on-device check** alongside Task 13 Step 2a: trigger an emergency (past dinner, ≥400 cal behind) and confirm the card reads coherently rather than looking like a bug.
- **A transient Eat Next ↔ Meal Library disagreement is now visible where it wasn't.** `MealRow`'s green-only "In stock" badge is computed from `MealLibraryModal`'s independently-fetched `assemblabilityById`, not from `useEatNext`'s map (the two-independent-lifetimes decision Task 10 argued for and this task did not revisit). Both are correct at their own fetch time, so an Eat Next chip and the library row behind it can briefly state different verdicts for one meal. Self-healing on the next refetch, and sharing them would trade a visible transient for a silent staleness bug — but before this task the Eat Next side displayed nothing, so the disagreement had nowhere to show. Noted rather than fixed.
- **`EatNextStockBadge.assemblable` reads as a domain fact when it is really a presentation variant.** `variant: "in_stock" | "missing"` would say what it means and would not invite a consumer to treat it as the engine's verdict. Both reviewers judged it not worth churning a just-shipped API for; recorded as an observation so the next person to touch this type has the argument already made.
