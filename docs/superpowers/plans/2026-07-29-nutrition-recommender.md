...
**Deviation 3, quoted with justification — the sync effect is NOT "near the other effects"; that placement is a runtime crash.** The plan's Step 3.6 said to "add, near the other effects" a `useEffect(…, [viewingToday, eatNext.result])`. Placed literally (with the effects at `:314-339`), a dependency **array is evaluated during render**, so `[viewingToday, …]` would read `viewingToday` before its `const` initializes at `:1149` — a TDZ `ReferenceError` that white-screens the Meals tab. This was caught by writing it that way first and reasoning about the dep array's evaluation timing before running anything, not by the type-checker (`tsc` does not flag TDZ on a later-declared `const` read from an earlier expression here) and not by the test suite (this screen has no test — see Gates). The effect therefore sits immediately after `proteinPace`'s `useMemo` (`:1234-1274`, guard and call at `:1270-1274`), i.e. at the end of the today-derived block whose `viewingToday` it reads, with the reason recorded in a comment so it isn't "tidied" back up later. The alternative — inlining `viewingDateStr === todayStr` to keep the plan's placement — was rejected as duplicating the expression `viewingToday` exists to name.
