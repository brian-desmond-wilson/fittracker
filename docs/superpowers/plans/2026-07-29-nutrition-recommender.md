...
- [ ] **Step 4: Post-apply wiring (ADDED during execution — see the Task 5 amendment).** Until this step runs, the nudge is dead code: Task 5 ships `nudgesEnabled: false` hardcoded because naming a nonexistent column makes PostgREST reject the whole select (42703 → HTTP 400). Only now is the column real. In `mobile/src/hooks/useEatNext.ts`, three edits:
  1. Append `, eat_nudges_enabled` to the `profiles` select string (the long single-line `"target_calories, … water_window_end"` argument).
  2. Add `eat_nudges_enabled: boolean;` to `interface ProfileRow` (replacing the `NOTE:` comment that marks its absence).
  3. Change the `recommendEatNext` argument `nudgesEnabled: false` to `nudgesEnabled: p.eat_nudges_enabled`, deleting the block comment above it that explains the hardcode.
