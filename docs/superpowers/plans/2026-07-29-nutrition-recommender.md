...
- [x] **Step 4: Post-apply wiring (ADDED during execution — see the Task 5 amendment; extended by the Task 6 amendment).** Until this step runs, the nudge is dead code: Task 5 ships `nudgesEnabled: false` hardcoded and Task 6 ships the Notifications-screen toggle seeded `useState(false)` with no DB read, because naming a nonexistent column makes PostgREST reject the whole select (42703 → HTTP 400). Only now is the column real. **Three edits** (not two — extended during Task 6's review):
...
Built `mobile/src/components/EatNextHomeCard.tsx` from the plan's Step 1 snippet with three pre-authorized corrections applied (A, B, C below) plus one additional deviation found during verification (the emergency-context message suppression). Wired into `mobile/app/(tabs)/home.tsx` exactly per Step 2 (import alongside the other card imports, section title + card inserted after `TodaysWorkoutCard` and before "Today's Summary" — matched on content, not the plan's stale line numbers, per Correction C).

**Correction A — `syncEatNudge(decision, sourceDay)` wiring.** The card reads `computedAt` (the hook's fifth field, added in Task 6's review) alongside `result` and calls `syncEatNudge(result.nudge, computedAt)` from a single `useEffect` keyed on `[result, computedAt]`, guarded by `if (!result || !computedAt) return;`. This cannot mis-schedule across midnight because it never constructs its own `Date`: `useEatNext.ts`'s `load()` sets `result` and `computedAt` together, synchronously, from the same local `now` on every successful run (confirmed by reading `load`'s body — `setResult(next); setComputedAt(now);` back-to-back, both inside the `runId`-guarded success path, never independently) and leaves both untouched on a failed refetch (stale-while-revalidate). So the two are a matched pair *by construction of the hook*, not by any discipline the card has to maintain — there is no code path in this component where the effect can observe a `result` from one load paired with a `computedAt` from another. The null branch (`computedAt` is `null` before the first load resolves) is the required compile-time decision Correction A calls out: correctly a no-op, since there is no decision yet to sync.

**Correction B — `EMPTY_LIBRARY_MESSAGE`.** Imported from `@/src/lib/eatNext` (exported there per Task 1's own amendment, which anticipated this exact fix) and compared with `result.message === EMPTY_LIBRARY_MESSAGE` instead of the plan's inline string literal.

**Correction C — line numbers.** `home.tsx`'s `TodaysWorkoutCard` render and `styles.sectionTitle` both exist with the names the plan assumes; matched on the `<TodaysWorkoutCard key={...} />` / "Today's Summary" content pair rather than the stale `:126-131` range, which had already drifted (actual insertion point was after line 128, before line 131 pre-edit).

**Verification 1 — every imported symbol checked against its actual shape.** `useEatNext`'s return type read in full (`useEatNext.ts:73-103`): five fields (`result`, `loading`, `error`, `refetch`, `computedAt`), matching the card's destructure. `syncEatNudge`'s signature read at its definition (`eatNudgeService.ts:167-172`): `(decision: EatNextNudge | null, sourceDay: Date): Promise<void>`, matching the call site. `EMPTY_LIBRARY_MESSAGE` confirmed exported at `eatNext.ts:26`. `EatNextResult`'s fields (`eatNext.ts:137-142`: `context`, `message`, `recommendations`, `nudge`) match every field the card reads. `UtensilsCrossed` confirmed as a real export via two independent checks: `node_modules/lucide-react-native/dist/esm/icons/utensils-crossed.js` exists, and `dist/esm/icons/index.js:1581` has `export { default as UtensilsCrossed } from './utensils-crossed.js';`. `useFocusEffect` and `router` both confirmed as genuine `expo-router` exports (`build/exports.d.ts:3` and `:21`) rather than assumed from the plan's snippet — `expo-router`'s own doc comment on `useFocusEffect` (`build/useFocusEffect.d.ts:15-19`) shows the identical `useFocusEffect(useCallback(...))` shape the card uses, so the plan's import line is correct as written (the house pattern of importing `useFocusEffect` from `@react-navigation/native` instead, seen in `MealsHomeCard.tsx`/`WaterIntakeHomeCard.tsx`, is an equally-valid alternate source for the same hook — not required here since `tsc` and the runtime behavior are identical either way).

**Verification 2 — navigation targets.** `/(tabs)/track/meals` confirmed as a real route (`mobile/app/(tabs)/track/meals.tsx` exists) and already the target of a working plain `router.push("/(tabs)/track/meals")` call in `MealsHomeCard.tsx:83` — used verbatim for the empty-library CTA. The object-form push with `params: { suggestMealId: top.mealId }` compiles under this repo's config (`npx expo-router` typed routes is not enabled — no `experiments.typedRoutes` in `app.json`, no generated `.expo/types` — so `Href` does not statically enumerate params per route the way it would under that experiment); confirmed empirically via `tsc --noEmit` returning 0 errors rather than assumed. Task 10 (not this task) is what makes the `suggestMealId` param do anything on the receiving end — `meals.tsx`/`MealLibraryModal` don't read it yet; sending it is Task 8's whole obligation here, and that part is satisfied.

**Verification 3 — double-load on mount: confirmed. ORIGINAL CONCLUSION ("no clean fix exists without touching `useEatNext.ts` or dropping the focus reload") WAS WRONG — SUPERSEDED, fixed in-card during review.** `useEatNext`'s internal `useEffect(() => { load(); }, [load, refreshKey])` fires `load()` on every mount, independent of focus. The card's `useFocusEffect(refetch)` also fires on the initial focus that accompanies a first mount. So a fresh mount of `EatNextHomeCard` issued two `load()` calls back-to-back-ish, both racing through the same five-way `Promise.all`. Correctness was never at risk — the `runIdRef` stale-response guard (`useEatNext.ts:162`) ensures only the run whose `runId` still matches `runIdRef.current` at completion may call `setResult`/`setError`/`setLoading(false)`. The error in the original pass was framing this as a binary choice ("touch `useEatNext.ts`" vs. "drop `useFocusEffect` entirely") and stopping there, missing the middle path: **skip only the ONE focus callback that coincides with the very first mount**, via a `useRef` flag local to the card, leaving every subsequent focus (and the `refreshKey` path, which never went through `useFocusEffect` to begin with) untouched. Implemented as a `firstFocus` ref: the focus callback checks and flips it before deciding whether to call `refetch()`, so the skip fires exactly once per component lifetime, and a remount creates a fresh ref (reset to `true`) in lockstep with `useEatNext`'s own mount effect firing again — so a remount still gets exactly one load from each source, same as today's steady state, not zero. This is entirely inside `EatNextHomeCard.tsx`; `useEatNext.ts` was not touched. Matters beyond the wasted request: since the nudge-resync effect is keyed on `[result, computedAt]`, two loads on mount previously meant two distinct `result` identities in quick succession, i.e. two full cancel-then-schedule `syncEatNudge` cycles (up to four native round trips) on every mount — halving the loads halves that too. **Correction to the record, not just the code: this was flagged by the coordinator's review as important beyond this one task, because Task 10 mounts a second surface (`EatNextRow`) on the same `useEatNext` hook and would have inherited the same false "no clean fix" reasoning if it had gone unchallenged.**

**Verification 4 — spec §7.1 completeness: a deliberate, reported scope reduction, not a silent one. The PROCEDURAL call (don't reach into `eatNext.ts` unauthorized) was correct and stands; the SCOPE RULING was overturned by the coordinator — see "Round 2" below, where the grant was extended and the gap closed at the engine.** Spec text: "Shows the top recommendation: name, first reason line, cal · protein · prep, score chip; red accent when context=`emergency`." The plan's Task 8 code (and this implementation) renders name, first reason line, and the emergency red accent — but **not** a cal/protein/prep line or a score chip. Traced why: `EatNextRecommendation` (`eatNext.ts:64-68`) carries exactly `{ mealId: string; name: string; reasons: string[] }` — no calories, protein, prep minutes, or score. Those values exist inside `recommendEatNext`'s internal `Candidate`/`ScoredMeal` types (`totals: MealTotals`, `score: BrianScoreResult`) but are never attached to the `EatNextRecommendation` the function returns; only the *reasons text* threads some of that data through as English (e.g. emergency's reasons[0] already embeds `${cal} cal, ${prep} min prep` — the numbers spec §7.1 wants are, coincidentally, already visible in the reason string for that one context, but not for `next_meal`/`post_workout`/`catch_up`/`goal_hit`, whose reasons don't state calories/protein/prep at all). Surfacing a standalone cal/protein/prep line or a score chip would require adding fields to `EatNextRecommendation` and populating them in every `toRecs`/return-site in `recommendEatNext` — an `eatNext.ts` change, which this task's constraints explicitly forbid ("Do NOT modify `eatNext.ts` … If you need something they don't expose, report it — don't reach in"). **Decision: implement exactly what the plan's code provides (name + first reason + message), and report the gap rather than choosing silently or reaching into the engine.** This mirrors Task 1's own precedent of flagging `EMPTY_LIBRARY_MESSAGE` as a needed engine export rather than having Task 8 improvise a workaround — the analogous fix here (adding cal/protein/prep/score fields to `EatNextRecommendation`) is a legitimate follow-up for whoever next touches `eatNext.ts` (Task 10 touches the engine's consumers but not the engine itself, per its own file list — so this is likely a small dedicated follow-up, not naturally absorbed by an existing remaining task). Filed as a follow-up below rather than expanding this task's scope.

**Verification 5 — all five render states traced against what the hook can actually publish; no reachable blank card.** `useEatNext`'s initial state is `{ result: null, loading: true, error: null, computedAt: null }` (`useState` initializers, `useEatNext.ts:151-154`); `loading` only ever transitions `true → false` once, in `load`'s `finally` (`:346-348`), and every exit from `load`'s `try`/`catch` either calls `setResult` (clearing `error`) or `setError` (leaving the previous `result`, possibly still `null` on a first-load failure) before that `finally` runs. Consequence: the state `{ loading: false, result: null, error: null }` requires a `load()` to have completed without ever calling `setResult` or `setError`, which the code makes structurally impossible — every branch of `try`/`catch` hits one or the other. So, walking the card's branches in order: (1) `loading && !result` — reachable only pre-first-resolution, renders the spinner; (2) `error && !result` — reachable only when the first load fails, renders error+retry; (3) `if (!result) return null` — dead code by the invariant just traced, kept only because TypeScript can't see across `useState`/`setState` call sites to know it, standard defensive narrowing, not a bug; (4)/(5) `!top` (contextual empty: `goal_hit` terminal, `after_window` terminal, or `next_meal` with no eligible candidates including the empty-library case) vs. `top` present (normal / `emergency`, distinguished by `result.context === "emergency"`). All five states the plan names are reachable and rendered; no combination the hook can actually publish falls through to a blank `<View>` or `null` beyond the structurally-unreachable branch (3).

**Also assess — `useFocusEffect(refetch)` + `refreshKey`: no odd interaction found.** `refetch` is `useEatNext`'s `load`, built with an empty dependency array (`useCallback(..., [])`, `useEatNext.ts:164-350`), so its identity never changes across renders — the card's `useFocusEffect(useCallback(() => refetch(), [refetch]))` therefore never re-fires from `refetch` identity churn. `refreshKey` reaching the hook triggers reload through a completely separate path: the hook's own `useEffect(() => { load(); }, [load, refreshKey])` fires whenever `refreshKey` changes, regardless of focus state. Since Home's `refreshKey` only bumps from its pull-to-refresh handler, which requires Home (and therefore this card) to already be mounted and focused, a `refreshKey` bump while off-focus isn't a reachable scenario in this single-tab layout. No double-fire, no missed-refresh case found.

**Also assess — `numberOfLines` truncation risk: low, not zero.** Enumerated every `reasons[0]` string the engine can produce (`eatNext.ts`): `next_meal` → `"next: breakfast"` / `"between meals"` (short); `post_workout` → `"post-workout meal"` or `"post-workout — N g protein"` (short); `goal_hit` protein-short branch → `"protein short by ~N g"` (short); `catch_up` → `` `closes the ~${gap} cal gap (${cal} cal, ${prep} min)` `` (longest observed form, roughly 45-55 characters for realistic gap/cal/prep values); `emergency` → `` `~${gap} cal to go — ${cal} cal, ${prep} min prep` `` (similar length). At 13pt in a card with 20pt padding on each side, even a narrow 320pt-wide device (iPhone SE) leaves roughly 280pt of text width — comfortably two lines for anything under ~90 characters combined, and the longest strings measured are well under half that. `title` at `numberOfLines={1}` is a user-authored meal name from the Meal Library with no length constraint at creation time; standard single-line truncation is the expected, accepted behavior for a title, same as every other card in this app. Risk judged low for the reason line and unremarkable for the title; not fixed (nothing to fix — this is expected `numberOfLines` behavior, not a defect), reported per the instruction to check rather than assume.

**Also assess — `result.message` below the reason line: real duplication for `emergency`, fixed; not for the others, left alone.** Compared the actual strings side by side per context. `emergency`: `message` = `` `~${gap} cal to go before day's end` ``, `reasons[0]` = `` `~${gap} cal to go — ${cal} cal, ${prep} min prep` `` — both begin with the identical clause `~N cal to go`, so rendering both stacks a literal repeated phrase on screen with no new information in the second line beyond "before day's end." Fixed by adding `showMessage = result.message && !emergency` and gating the message `<Text>` on it — the red border + icon already carry the emergency signal, so nothing is lost. `catch_up`: `message` = `` `${gap} cal behind pace` ``, `reasons[0]` = `` `closes the ~${gap} cal gap (${cal} cal, ${prep} min)` `` — both cite the same gap number but in different framings (current deficit vs. what this meal closes); judged complementary, not duplicated, left rendering both. `goal_hit` protein-short branch: `message` = `"Calorie target hit — protein still short."`, `reasons[0]` = `` `protein short by ~${gap} g` `` — different information (overall state vs. this meal's specific shortfall number), left rendering both. `next_meal` and `post_workout` never set a non-null `message` when `top` exists (`next_meal`'s message is only non-null in the `cands.length === 0` branch, which is exactly the `!top` case the card handles separately; `post_workout` hardcodes `message: null`), so no duplication risk in either.

**Other deviation, quoted with justification:** the `showMessage` conditional above is the only line of logic added beyond the plan's snippet plus Corrections A–C; everything else is the plan's code with those three corrections substituted in place.

**Gates:** `cd mobile && npx tsc --noEmit` → 0 errors. `cd mobile && npm test` → 8/8 suites, 203/203 tests, unchanged from baseline (this task adds no test-eligible code — `EatNextHomeCard.tsx` imports `react-native`, which this repo's `testEnvironment: node` Jest scope excludes, matching `eatNudgeService.ts`'s and `notificationService.ts`'s precedent). Files touched: `mobile/src/components/EatNextHomeCard.tsx` (new), `mobile/app/(tabs)/home.tsx`, this plan doc. No database-connecting command run at any point (no `supabase` CLI invoked).

**Follow-up filed in round 1 (not Phase 3 scope as currently written): surface cal/protein/prep/score on `EatNextRecommendation`.** To satisfy spec §7.1's "cal · protein · prep, score chip" literally, `recommendEatNext`'s return type needs those fields attached per-recommendation (from the `Candidate`/`ScoredMeal` data it already computes internally but doesn't expose), and `EatNextHomeCard` would then render a second line from them. Left for a dedicated follow-up rather than done here because it requires editing `eatNext.ts`, which this task's file list explicitly excludes; whoever picks it up should also decide whether to re-derive the numbers already embedded as English in some `reasons[0]` strings (risking two sources of the same fact) or leave those reason strings as-is alongside the new structured fields. **CLOSED in round 2 below** — the coordinator extended the grant to `eatNext.ts` for this exact item and ruled it in-scope rather than deferred.

---

## Round 2 (coordinator review of round 1)

Both reviews came back with three required changes and a set of polish items. Disposition of each, in the order the coordinator's message raised them.

**1. §7.1 completeness — engine grant extended, gap closed at the source.** The coordinator overturned round 1's scope ruling (procedural call to not reach into `eatNext.ts` was affirmed as correct; the decision to defer the gap instead of escalating for a wider grant was not) and extended this task's file list to `mobile/src/lib/eatNext.ts`, `mobile/src/lib/__tests__/eatNext.test.ts`, and the spec doc, for this one item only. `EatNextRecommendation` gained four fields — `calories: number; protein: number; prepMinutes: number; score: number` — populated at both of `recommendEatNext`'s projection sites (`toRecs`, and the `goal_hit` protein-short branch's inline return object; grepped `mealId:` in `eatNext.ts` first to confirm exactly two sites exist, so neither could be missed). `calories`/`protein` are `Math.round`ed, matching how the reason strings already render the same numbers. `score` is `c.score.score` / `top.score.score` — the /100 renormalized value, per spec §5.5's explicit "UI surfaces still display `score`/100" — not `c.score.raw`, which §5.5 reserves for ranking (`rank()`'s own comment: "Rank on the un-renormalized component sum … UI still DISPLAYS score/100"). This is the one field in the four where reading the wrong property compiles cleanly and silently ranks-vs-displays the wrong number, so it's the one the coordinator asked to be mutation-verified, not just tested.

Three new tests added to `eatNext.test.ts`, in a new `describe("recommendation structured fields", …)` block: (1) pins `calories`/`protein`/`prepMinutes` from a `scored()` fixture with fractional totals (512.6/33.6 → rounds to 513/34), confirming `Math.round` is applied and not, say, `Math.floor`; (2) the `score`-vs-`raw` discriminator, using the existing `scored()` fixture's independent `raw`/`score` override (already exercised by the pre-existing "ranks by raw, not the rounded score" ranking test, but that test never inspected the *output* — this one does: `raw: 50, score: 99` and asserts `recommendations[0].score === 99`, `!== 50`); (3) the same discriminator plus the rounding check against the `goal_hit` protein-short branch specifically, since that's a second, independently-written projection site that (1) and (2) don't exercise (`raw: 50, score: 88` on the bridge candidate, asserting the projected `score` is 88).

All three pass against the real code. `cd mobile && npm test` → 8/8 suites, **206/206 tests** (203 baseline + 3 new), `eatNext.test.ts` alone: 67/67 (64 baseline + 3 new). `cd mobile && npx tsc --noEmit` → 0 errors.

**Mutation table (hand-verified, `--no-cache` per the coordinator's note that ts-jest caching produced a false survivor in a tight loop on this branch):**

| Mutant | Applied via | Result |
|---|---|---|
| `score: c.score.score` → `score: c.score.raw` (`toRecs`) | `sed` in-place | ✅ killed — both new score-discriminator tests fail (`Expected: 99, Received: 50` and `Expected: 88, Received: 50`) |
| `score: top.score.score` → `score: top.score.raw` (goal_hit branch) | same `sed` command, same pass | ✅ killed — the goal_hit-specific test fails (`Received: 50`, its own `raw` value) |
| Reverted (`mv eatNext.ts.orig eatNext.ts`) | — | Full suite back to 67/67 (`eatNext.test.ts`), `tsc` 0 errors |

Both projection sites were mutated in the same pass (one `sed` with two patterns) and both discriminator tests failed in the same run, confirmed by test name in the failure output before reverting — not inferred.

**Byte-identical behavior confirmed for the six contexts, ranking, and `computeNudge`.** The diff against pre-round-2 `eatNext.ts` is additive only: four new interface fields, four new assignment lines at each of the two existing return sites, nothing removed or reordered, no new branch, no changed comparison operator anywhere outside those two hunks. All 64 pre-existing `eatNext.test.ts` tests — covering every context, the ranking order tests, `computeNudge`, and `nudgeFireDate` — pass unmodified (not edited, not re-asserted, literally the same file content for those `it(...)` blocks) both before and after the round-2 diff.

**Spec §5.2 amended, not §7.1.** §7.1 already asked for "cal · protein · prep, score chip" — it needed no change once the implementation caught up to it. §5.2's `recommendations: Array<{ mealId: string; reasons: string[] }>` was the section actually behind reality (missing `name`, which Task 1 added long before this task, in addition to missing the four fields this task adds) — a dated, coordinator-attributed amendment paragraph was appended after its code block, in the same style as the existing §6/§8.1 amendments (quoted the stale original, stated the corrected element type, explained why, pointed at this amendment for full reasoning). Full text is in `docs/superpowers/specs/2026-07-29-nutrition-recommender-design.md`, immediately following §5.2's code block.

**2. MUST FIX — uncaught `syncEatNudge` rejection.** Confirmed the trace the coordinator gave: `requestPermissions` (`notificationService.ts:33-67`) wraps none of its three awaited native calls in try/catch; `syncEatNudgeCore` awaits it outside its own try (which starts only at the `scheduleNotificationAsync` call); `serialize` returns the queued operation's promise directly, unwrapped. So a rejection from `requestPermissions` propagates all the way out of `syncEatNudge` to whoever called it — and `syncEatNudge`'s own doc comment says exactly that ("Does not surface permission-denial to the caller (stays `Promise<void>`)" describes the *denial* path, which is swallowed inside `syncEatNudgeCore` via `if (!granted) return;` — but a *thrown* rejection from `requestPermissions` itself, e.g. the native call itself failing rather than returning a denial, is a different path that isn't caught anywhere in the service). The card's effect had no catch, so that reaches nowhere: a dev yellow-box, silence in production. Fixed exactly as suggested: `void syncEatNudge(result.nudge, computedAt).catch((e) => console.error(...))`. Deliberately still not surfaced to the user (no `Alert`) — this runs on every background resync with no gesture to attach one to, consistent with how `syncEatNudgeCore` already treats permission *denial* (surfaced once, at the `NotificationsScreen.tsx` toggle gesture, never from a background resync — see the Task 6 amendment, MUST FIX 2).

**3. MUST FIX — double-load fix, and the record correction.** Implemented exactly as the coordinator specified: a `firstFocus` ref, checked and flipped inside the `useFocusEffect` callback before deciding whether to call `refetch()`. The round-1 amendment's claim ("no clean fix exists without touching `useEatNext.ts` or dropping the focus reload") is corrected in place above (Verification 3), marked superseded rather than silently rewritten, with the reasoning error named explicitly: the original pass posed this as a binary choice and stopped at the first two options without considering "skip only the first focus callback," which needed no change outside this file. Re-verified after the fix: `useEatNext`'s mount effect still fires once per mount (unchanged, that file wasn't touched); the card's focus effect now fires but no-ops on the very first focus and calls `refetch()` on every subsequent one; a remount instantiates a fresh `firstFocus` ref (back to `true`) in the same render pass as the hook's own state resetting, so the skip-once behavior re-arms correctly rather than staying permanently skipped or double-skipping. `refreshKey` changes were never routed through `useFocusEffect` (they go through `useEatNext`'s own `[load, refreshKey]` effect dependency) so this fix doesn't touch that path at all — confirmed by re-reading `useEatNext.ts:352-363` unchanged.

**Also apply — all four items, applied as specified:**
- `showMessage` changed from `result.message && !emergency` (typed `string | boolean | null`) to `!!result.message && !emergency` (typed `boolean`). Commented as unreachable-but-correct-to-guard, matching the coordinator's exact reasoning (an empty-string message would otherwise hit RN's "Text strings must be rendered within a `<Text>` component" — not reachable with today's engine, which never emits `""`, but the type shouldn't rely on that).
- `if (!result) return null` now carries a one-line comment pointing at Task 5's `setError(null)` relocation as the reason it's unreachable, using the same phrasing style as `computeNudge`'s own "defensive, currently unreachable" comments in `eatNext.ts`, per the coordinator's explicit ask for consistency with that precedent.
- The contextual-empty `<Text>` got `flex: 1` (`emptyMessageText` style), not `numberOfLines` — matches the coordinator's specific instruction and reasoning (it sits beside a fixed-width icon in a row with nothing else bounding its width; the 45-char worst case today doesn't need it, but nothing stops a longer message next to the icon without it).
- The file header comment's "refreshKey remount/reload" corrected to "refreshKey prop-driven reload (no remount — the same component instance re-fetches when refreshKey changes)."

**Deliberately NOT changed, per the coordinator's explicit ruling — recorded so neither reads as an oversight:**
- `router`/`useFocusEffect` imported from `expo-router` rather than `useRouter()`/`@react-navigation/native` (the pattern `MealsHomeCard.tsx`/`WaterIntakeHomeCard.tsx` use). Both are one-word-import changes and both compile/behave identically (Verification 1 in round 1 already confirmed `expo-router`'s own doc example uses this exact `useFocusEffect(useCallback(...))` shape) — the coordinator chose not to churn two imports purely for cross-file uniformity, and this note exists so the divergence isn't "fixed" by a future pass without checking here first.
- Card spacing/border vs. `TodaysWorkoutCard` immediately above it (`#1E293B`/`#374151`/16px padding/16px `marginBottom` there, vs. this card's `#111827`/`#1F2937`/20px/8px). Matches two of the three Home-card neighbors (`MealsHomeCard`, `WaterIntakeHomeCard` use the same `#111827`/`#1F2937`/20px), so it's on the right side of "match the file's local convention" — but two adjacent full-width cards (`TodaysWorkoutCard` directly above this one) will read as visibly different, and the 8px `marginBottom` here leaves only 16px before the "Today's Summary" title vs. 24px after the workout card. Not changed — the coordinator's ruling was to record this as an explicit item for the owner's on-device pass at Task 12, not guess at a fix now without seeing it rendered.

**Gates (round 2):** `cd mobile && npx tsc --noEmit` → 0 errors. `cd mobile && npm test` → 8/8 suites, **206/206 tests** (203 baseline + 3 new `eatNext.test.ts` cases; every other suite unchanged). Files touched this round: `mobile/src/lib/eatNext.ts`, `mobile/src/lib/__tests__/eatNext.test.ts`, `mobile/src/components/EatNextHomeCard.tsx`, `docs/superpowers/specs/2026-07-29-nutrition-recommender-design.md`, this plan doc. No database-connecting command run at any point (no `supabase` CLI invoked; the mutation test used `sed`/`mv` on a local file only). Commits: engine fields + tests, must-fixes (uncaught rejection + double-load), card rendering (stats row + score chip + polish), this docs/spec commit — four separate commits, in that order, matching the coordinator's requested split.

### Task 9

Built `mobile/src/components/RampHomeBanner.tsx` from the plan's Step 1 snippet with one structural deviation (state shape, below), mounted in `mobile/app/(tabs)/home.tsx` directly above the `{/* Today's Workout Section */}` comment per Step 2, and wired the `?modal=nutrition` deep link into `mobile/app/(tabs)/profile.tsx` per Step 3 with one import deviation (also below). All five Phase 1 APIs the banner calls were read at their actual definitions before use, not assumed from the plan's snippet.

**Verification 1 — every Phase 1 API checked against its real shape; all match the plan exactly, no defects found.** `fetchNutritionPreferences()` (`mobile/src/lib/supabase/nutritionPreferences.ts:19-44`) returns `NutritionPreferencesData` with `rampLevels: CalorieRampLevel[]` (line 16). `CalorieRampLevel` (`mobile/src/types/nutrition-preferences.ts:70-83`) has exactly `level: number`, `name: string`, `is_active: boolean`, `started_at: string | null` — all four names and types the banner reads. `fetchRecentWeighIns(sinceLocalDate: string): Promise<WeighIn[]>` (`nutritionPreferences.ts:131-141`) takes a plain date string via `.gte("date", sinceLocalDate)`, matching the `getLocalDateString(since)` call site. `assessRampProgress({ weighIns, levelStartedAt, today }: AssessRampProgressOpts)` (`mobile/src/lib/rampProgress.ts:51-141`) has exactly that three-field options shape; `RampRecommendation` (line 10) is `"advance" | "hold" | "insufficient_data"` — `"advance"` is a real member, and the banner's `=== "advance"` check needs no other case (the other two both correctly render nothing). `getLocalDateString(date: Date = new Date()): string` (`mobile/src/components/track/meals/mealsHelpers.ts:22-27`) has a defaulted, optional `Date` parameter, so both `getLocalDateString(since)` and the zero-arg `getLocalDateString()` call compile and behave as the plan's snippet assumes.

**Verification 2 — the 42-day window: checked, confirmed equal, not a defect.** `NutritionPreferencesScreen.tsx:44` — `const TREND_WINDOW_DAYS = 42; // 6 weeks of weigh-ins for the ramp assessment`. The plan's banner snippet's `TREND_WINDOW_DAYS = 42` matches exactly. Both files independently compute `since` the same way (`new Date(); since.setDate(since.getDate() - TREND_WINDOW_DAYS)`) and feed it through the identical `getLocalDateString(since)` → `fetchRecentWeighIns` → `assessRampProgress` pipeline, so the banner and the Preferences screen will always agree on whether an advance is due — the exact "two surfaces disagree" failure spec §7.3 warns against does not occur here. Left the constant duplicated (not extracted to a shared module — out of scope per the file-list constraint, which forbids touching `nutritionPreferences.ts` or any other Phase 1 file to create a shared export) but added a comment on the banner's copy naming the screen's copy as the value it must track, so a future change to one is more likely to be caught reviewing the other.

**Verification 3 — `?modal=nutrition` deep link: works as written, including `setParams({ modal: undefined })`, which is confirmed as the *documented* clearing mechanism for this expo-router version, not a guess.** `activeModal` state and its `"nutrition"` value both exist as the plan assumes (`profile.tsx:39` pre-edit, unchanged by this task; `ModalScreen` union at `profile.tsx:21-29` includes `"nutrition"`; `setActiveModal` is its setter). For clearing the param: read `expo-router@~6.0.24`'s own source rather than assuming. `setParams` (`node_modules/expo-router/build/global-state/routing.js:165-171`) delegates directly to React Navigation's `navigationRef.current.setParams(params)`, which shallow-merges — so a key set to `undefined` is **not removed** from the underlying route-params object, it's merged in as an explicit `undefined` value. The critical piece is on the *read* side: `useLocalSearchParams` (`node_modules/expo-router/build/hooks.js:149-169`) has its own comment explaining exactly this — *"React Navigation doesn't remove 'undefined' values from the params object, and you cannot remove them via navigation.setParams as it shallow merges. Hence, we hide them here"* — and its implementation explicitly special-cases `value === undefined` and returns `[key, undefined]` unmodified (skipping the `decodeURIComponent` call that runs for every other value). So `router.setParams({ modal: undefined })` is not a workaround or an assumption — it is the library's own documented, designed-for mechanism for this exact use case, and after it runs, `useLocalSearchParams()` genuinely reports `params.modal === undefined` on the next render, not the stale string `"nutrition"`. No prior code in this app calls `setParams` (grepped `app/` and `src/` — zero hits before this task), so there was no existing in-app precedent to match against; the expo-router source itself was the source of truth.

**Verification 4 — effect dependency correctness: traced, settles in one extra render, does not loop.** `useEffect(() => { if (params.modal === "nutrition") { setActiveModal("nutrition"); router.setParams({ modal: undefined }); } }, [params.modal])`. On the deep-link render, `params.modal === "nutrition"`: the effect fires, calls `setActiveModal("nutrition")` (schedules a re-render, but does not change `params.modal`) and `router.setParams({ modal: undefined })` (updates the navigation route's params, which — per Verification 3 — causes `useLocalSearchParams()` to report `undefined` on the next render). That change to `params.modal`'s *value* (not merely a new object identity — the primitive itself changes from `"nutrition"` to `undefined`) is what the effect's dependency array is keyed on, so the effect re-fires exactly once more; this time `params.modal === "nutrition"` is false, so the body is skipped and no further `setParams` call is made. No third re-fire follows, because nothing after that touches `params.modal` again. Confirmed by reading `useFocusEffect`-adjacent React Navigation update semantics is not needed here — this is a plain `useEffect` on a primitive dependency, which is the simpler and more standard case to reason about.

**Verification 5 — spec §7.3 completeness, confirmed against the read spec text (`docs/superpowers/specs/2026-07-29-nutrition-recommender-design.md:115-117`), all clauses satisfied.**
- "Above the Today's Workout section" — mounted at `home.tsx:128-129`, immediately before the `{/* Today's Workout Section */}` comment (see Verification 6 below for the exact placement check).
- "rendered only when `assessRampProgress` … returns `advance` and a next level exists" — `if (assessment.recommendation === "advance" && next) { setNextLevel(...) } else { setNextLevel(null) }`, and the component returns `null` whenever `nextLevel === null`.
- "reused verbatim" — `assessRampProgress` is imported and called with the exact same three-field options shape `NutritionPreferencesScreen.tsx` uses; no logic from `rampProgress.ts` is reimplemented or approximated in the banner.
- Text "Ready for Level N — {name}" — rendered verbatim as `` `Ready for Level ${nextLevel.number} — ${nextLevel.name}. Tap to review.` `` (the plan's own snippet appends ". Tap to review." beyond the spec's quoted fragment; kept, since it's additive affordance text, not a contradiction of the quoted string, and the spec doesn't forbid trailing text).
- "tap → `router.push("/(tabs)/profile?modal=nutrition")`" — implemented as the object form `router.push({ pathname: "/(tabs)/profile", params: { modal: "nutrition" } })`, which resolves to the identical URL; confirmed by the fact this compiles under `tsc --noEmit` with 0 errors and by Verification 3/4 tracing that `profile.tsx` receives `params.modal === "nutrition"` from it.
- "`app/(tabs)/profile.tsx` reads the `modal` search param on focus and auto-opens the Nutrition Preferences modal, then clears the param" — implemented as a plain mount-time/param-change `useEffect` rather than literally "on focus" (no `useFocusEffect` in `profile.tsx`); this is the correct reading because `router.push` to a screen that isn't already mounted runs the effect on that same mount, which is the only time this deep link is reachable in this app's navigation (there's no existing path that leaves `profile.tsx` mounted-but-unfocused with a new `modal` param arriving later) — so "on focus" and "on mount with this param" coincide here, and Verification 4 already confirmed the effect settles rather than looping.
- "never writes" — confirmed: `RampHomeBanner.tsx` calls only `fetchNutritionPreferences`, `fetchRecentWeighIns`, and `assessRampProgress` (all reads/pure functions), never `changeRampLevel` or any other mutating export from `nutritionPreferences.ts`. No `supabase.from(...).insert/update/delete/rpc` call appears anywhere in the new file.
- "Top-of-ramp (`advance` with no next level) stays a Preferences-only banner (Home shows nothing)" — when `active` is the highest level, `prefs.rampLevels.find((l) => l.level === active.level + 1)` returns `undefined`, so `next` is falsy, the `assessment.recommendation === "advance" && next` guard is false regardless of the assessment, and the `else` branch runs `setNextLevel(null)` — no exception, no fallthrough, falls cleanly into "renders nothing."

**Verification 6 — placement confirmed by content match, not line number.** `home.tsx`'s `{/* Today's Workout Section */}` comment (plan's assumed `~126`) is actually at line 131 pre-edit (drifted, as expected — Task 8 inserted the Eat Next section between it and "Today's Summary"). The banner is mounted at the correct point regardless: `home.tsx:128-129`, between the `MorningRoutineBanner` and the `{/* Today's Workout Section */}` comment, which places it above **both** `TodaysWorkoutCard` and the `EatNextHomeCard` Task 8 added directly below it — matching "mount above Today's Workout" from the Task 9 file list and the "above the Eat Next card" instruction in this task's verification checklist, since Eat Next already sits below Today's Workout in the tree Task 8 built.

**Deviation 1, quoted with justification — two `useState` calls collapsed into one.** The plan's snippet:
```tsx
const [nextLevelName, setNextLevelName] = useState<string | null>(null);
const [nextLevelNumber, setNextLevelNumber] = useState<number | null>(null);
```
was replaced with a single `useState<{ number: number; name: string } | null>(null)`, set and cleared together in the same call. The plan's two-variable form is set/cleared together everywhere it's touched, so in practice they never observably disagree — but the shape itself doesn't rule out a stale intermediate render seeing one updated and not the other (React batches same-tick `setState` calls, but that's an implementation detail of the scheduler, not a type-level guarantee), and the render guard (`nextLevelName === null`) checks only one of the two fields, so a hypothetical bug that set `nextLevelNumber` without `nextLevelName` (or vice versa) would silently render `undefined`/`NaN` inline in the banner text rather than being caught by the guard. One object makes "actionable" a single fact with one thing to check, at zero cost (both call sites already always set or clear both fields together, so this is a pure simplification, not a behavior change) — confirmed by `tsc`/tests both passing unchanged with the new shape.

**Deviation 2, quoted with justification — `profile.tsx` does not import a second `router` binding.** The plan's Step 3 said "Add imports `useLocalSearchParams, router` from `expo-router` (keep existing imports)," but `profile.tsx` already has `const router = useRouter();` (pre-existing, used by `loadUserData`/`handleSignOut`). Read `expo-router`'s `useRouter()` implementation (`node_modules/expo-router/build/hooks.js:94-100`): it returns `imperative_api_1.router` directly — the identical singleton the module-level `router` export resolves to (aside from the preview-mode `isPreview` branch, irrelevant on-device). Importing a second `router` binding from `expo-router` alongside the existing `useRouter()`-derived local would either shadow it (confusing, since both are the same object under a different name) or collide as a duplicate identifier if renamed carelessly. Used the existing local `router` for the `setParams` call instead; behavior is identical, one fewer import. Confirmed via `tsc --noEmit` (0 errors) that this compiles and via Verification 3/4's trace that the runtime call is exactly `setParams({ modal: undefined })` either way.

**Also assess — fail-silent on error: judged appropriate, and the inconsistency with `EatNextHomeCard` is justified, not an oversight.** `EatNextHomeCard` shows an error+retry state because it *is* Home's only surface for its feature — a failed load there means the user sees nothing where a full recommendation card should be, with no other path to the same information. `RampHomeBanner` is different in kind: it promotes a suggestion that already lives, and is always reachable, inside `NutritionPreferencesScreen`'s `RampCard` (`profile.tsx` → Nutrition Preferences, no deep link required) — a failed banner load simply means the promotion doesn't appear this time; the underlying feature (checking/confirming a ramp advance) is unaffected and unblocked. Rendering an error+retry chip for a promotional banner would also be visually heavier than the information warrants (a permanent-feeling error box for something that's supposed to disappear entirely most of the time anyway). Fail-silent-and-log matches the plan's own reasoning in the code comment and is the right call for a decoration/promotion surface, as distinct from a primary-content surface.

**Also assess — cost per focus: two Supabase queries added per Home focus; no cheap fix identified, none applied.** `RampHomeBanner` fires `fetchNutritionPreferences` (four parallel `select` calls internally) and `fetchRecentWeighIns` (one more) on every focus via `useFocusEffect`, on top of whatever `EatNextHomeCard`, `MealsHomeCard`, and `WaterIntakeHomeCard` already fire the same way — this is the established house pattern for every Home card (self-fetching, focus- and `refreshKey`-driven, no shared cache), not something this task introduced in isolation. A ramp level changes at most a few times a month and weigh-ins change at most daily, so the data this banner reads is far more static than the Eat Next engine's inputs (which change per meal logged) — that asymmetry means this banner is comparatively the *most* over-fetched card on the screen relative to how often its answer can change, but "don't build a caching layer" was an explicit instruction for this task, and the fix that would actually help (a shared query cache or a longer-lived focus-independent store) is a cross-cutting change to how every Home card fetches, out of scope for a single new component. Reported, not built.

**Also assess — `useFocusEffect` + `refreshKey` with `load`'s empty deps: confirmed correct, and `refreshKey` is genuinely needed, not vestigial.** Read `@react-navigation/core`'s `useFocusEffect` source directly (`node_modules/@react-navigation/core/src/useFocusEffect.tsx:32-112`) rather than assuming its behavior: the hook's outer `React.useEffect` has `[effect, navigation]` as its own dependency array, and inside, `if (navigation.isFocused()) { cleanup = callback(); isFocused = true; }` runs immediately whenever that outer effect re-runs — i.e., on *any* change to the `effect` callback's identity, not only on a genuine focus transition. Since `load` is `useCallback(..., [])` (stable forever) and the banner's `useFocusEffect(useCallback(() => load(), [load, refreshKey]))` callback's identity therefore changes only when `refreshKey` changes, a `refreshKey` bump reloads the banner immediately if Home is currently focused — which it always is when `refreshKey` bumps, since that only happens from Home's own pull-to-refresh handler (`home.tsx`'s `onRefresh`), which requires Home to be mounted and focused to fire at all. Without `refreshKey` in the inner `useCallback`'s deps, the banner would refresh only on genuine focus events (tab switch away and back) and would silently not update on pull-to-refresh, unlike its sibling cards (`MealsHomeCard`, `WaterIntakeHomeCard`) which do watch `refreshKey`. So the answer to "does the banner need `refreshKey` at all" is yes — it is not redundant with the focus-triggered reload, it covers a distinct trigger (manual refresh while already focused) that focus-only tracking would miss.

**Gates:** `cd mobile && npx tsc --noEmit` → 0 errors. `cd mobile && npm test` → 8/8 suites, 206/206 tests, unchanged from baseline (this task adds no test-eligible code — `RampHomeBanner.tsx` imports `react-native`, excluded from this repo's `testEnvironment: node` Jest scope, matching `EatNextHomeCard.tsx`'s precedent from Task 8). Files touched: `mobile/src/components/RampHomeBanner.tsx` (new), `mobile/app/(tabs)/home.tsx`, `mobile/app/(tabs)/profile.tsx`, this plan doc. No database-connecting command run at any point (no `supabase` CLI invoked). The banner performs no writes — verified above (Verification 5) and independently by grepping the new file for `.insert(`, `.update(`, `.delete(`, `.rpc(` — zero matches.

#### Task 9 — review round 1

Both reviews came back spec-compliant with no must-fix and quality-approved ("strongest task in the branch so far"), confirming (among other things) the `useLocalSearchParams`-vs-`useGlobalSearchParams` choice as the single load-bearing line in the deep link — only `useLocalSearchParams` applies expo-router's undefined-hiding, per its own source comment quoted verbatim in Verification 3 above; `useGlobalSearchParams` reads `useRouteInfo().params` raw with no such mapping, which would have made the clear invisible and re-opened the modal on every return to Profile. Two fixes required, both applied; one file-list extension granted.

**Fix 1 — narrowed the banner's fetch from `fetchNutritionPreferences()` (4 queries) to a new, additive `fetchRampLevels()` (1 query).** `fetchNutritionPreferences` (`nutritionPreferences.ts:19-25`, unchanged) runs `food_concepts.select("*")`, `nutrition_constraints.select("*").maybeSingle()`, `nutrition_vendors.select("*")`, and `calorie_ramp_levels.select("*")` in parallel — the banner only ever read the fourth, so the other three (concepts being the largest) were fetched and discarded on every Home focus, on top of `fetchRecentWeighIns`, on top of whatever `EatNextHomeCard` already fires on the same focus. Added `fetchRampLevels(): Promise<RampLevelSummary[]>` to `nutritionPreferences.ts` (`:143-160`), selecting exactly `level, name, is_active, started_at` — the four columns the banner's active/next-level derivation and `assessRampProgress` call actually read, ordered by `level` to match `fetchNutritionPreferences`'s existing `calorie_ramp_levels` query. Follows this module's single-query idiom (`fetchRecentWeighIns`'s shape: one `supabase.from(...).select(...)`, `if (error) throw error`, `return data ?? []`), not the multi-query `Promise.all` idiom, since it's one table. Returns a new `RampLevelSummary` interface (not `CalorieRampLevel`) because the projection is intentionally narrower than the full row type — returning `CalorieRampLevel[]` from a query that doesn't select `id`/`user_id`/`target_calories`/etc. would be a type lie. The banner now imports `fetchRampLevels` in place of `fetchNutritionPreferences` and reads `rampLevels` directly instead of `prefs.rampLevels`; no other change to its load logic. **`NutritionPreferencesScreen` was not touched and does not use the new export** — confirmed by grep (`fetchRampLevels` has exactly one call site, in `RampHomeBanner.tsx`) — its full-dataset fetch is correct as-is per the coordinator's explicit instruction, not duplication. **`fetchNutritionPreferences` and every other existing export in `nutritionPreferences.ts` are behaviorally unchanged**: `git diff` on the file shows a pure append (25 lines added, zero removed, zero modified) after the pre-existing `fetchRecentWeighIns` function; every other function's body, signature, and position is byte-identical to before this change.

Recorded per the coordinator's note for the record: the assess *sequence* (42-day offset → fetch weigh-ins → derive active level → call `assessRampProgress`) is genuinely duplicated between `RampHomeBanner.tsx` and `NutritionPreferencesScreen.tsx`'s `load()`, synchronized only by the `TREND_WINDOW_DAYS` cross-file comment — the fetch itself was the false half of that framing (the screen needs the other three datasets; the banner now correctly doesn't fetch them), but the assess sequence is real duplication. **Follow-up filed, not built:** a shared `assessRampFromServer()` helper (fetch ramp levels + recent weigh-ins + derive active + call `assessRampProgress`, returning `{ assessment, active, next }` or similar) that both call sites could use, closing the duplication at its actual location instead of leaving it synchronized by comment.

**Fix 2 — stopped a transient refetch failure from wiping a visible banner.** The original `catch` block called `setNextLevel(null)` alongside `console.error`, which meant: if the banner was already showing (from a prior successful load) and a later refocus fetch failed on a network blip, the banner would vanish silently and only return on the next successful focus — discarding known-good state for a failure the user has no way to notice or act on, the inverse of the stale-while-revalidate rule `useEatNext.ts` deliberately follows for the same reason. Deleted the `setNextLevel(null)` line; the `catch` is now log-only (`console.error("RampHomeBanner load:", error)`), same distinct prefix as before. The banner is now sticky across a failed refetch: once shown, it stays showing its last-known-good `nextLevel` until either a successful load changes it or the underlying advance is genuinely no longer due. The coordinator's endorsement of fail-silent-on-first-load (Deviation review, "Also assess" section above) stands unchanged — this fix only changes what happens to *already-earned* state on a *later* failure, not the first-load behavior.

**Also applied — moved the `useLocalSearchParams`/`useEffect` deep-link block below `formData`'s `useState` in `profile.tsx`.** It previously sat between `activeModal`'s `useState` and `formData`'s `useState`, splitting the file's state-declaration block; moved to sit directly above the pre-existing `useEffect(() => { loadUserData(); }, [])` at (pre-edit) `profile.tsx:85`, so all `useState` calls are grouped together and the file's two effects are adjacent, matching the rest of the file's structure. No logic changed — same effect body, same dependency array, same comments.

**Rulings recorded, not changed:** the ". Tap to review." suffix beyond §7.3's quoted banner text (additive affordance copy, required string present verbatim); the object-form `router.push({ pathname, params })` over the spec's literal query-string form (equivalent, consistent with Task 8's navigation calls); the param-keyed `useEffect` over §7.3's literal "on focus" wording, ruled *strictly safer* — a `useFocusEffect` would re-run on every focus and could re-open a dismissed modal if the param-clear ever regressed, where the current mount/param-change effect degrades no worse than today; no loading placeholder for the banner (a skeleton flashing on every focus for a usually-absent banner would be worse than the one-time layout shift when it does appear); and the effect's dependency list omitting the stable `router`/`setActiveModal` (consistent with the sibling `loadUserData` effect immediately below it, no ESLint exhaustive-deps rule configured in this repo).

**Gates (round 1):** `cd mobile && npx tsc --noEmit` → 0 errors. `cd mobile && npm test` → 8/8 suites, **206/206 tests**, unchanged. Files touched this round: `mobile/src/lib/supabase/nutritionPreferences.ts` (additive-only, file-list extension granted for this item), `mobile/src/components/RampHomeBanner.tsx`, `mobile/app/(tabs)/profile.tsx`, this plan doc. No database-connecting command run at any point (no `supabase` CLI invoked). Commits: additive `fetchRampLevels()` export, banner fetch-narrowing + sticky-state fix, profile.tsx hook reordering, this docs commit — four separate commits, in that order.

### Task 10

Created `mobile/src/components/track/meals/EatNextRow.tsx`, added the optional `initialMealId` prop to `mobile/src/components/track/meals/library/MealLibraryModal.tsx`, and wired all three pieces (row, deep link, nudge resync + write-refetch) into `mobile/src/components/track/MealsScreen.tsx`. Every anchor was located by content, as instructed — all four of the plan's line-number references had drifted (`~:1149` pace `useMemo`s → `:1147-1175`; `~:1407-1412` pace-lines block → `:1411-1419`; library-modal block → `:1605-1620`; `MealLibraryModal`'s visibility effect → `:69-74`). One of the plan's factual claims about the file was **wrong** and is corrected below (Deviation 1: `router` was *not* already imported).

**Correction A — `sourceDay` wired from the hook's `computedAt`, never a constructed `Date`.** The resync is `void syncEatNudge(eatNext.result.nudge, eatNext.computedAt).catch(…)` inside `useEffect(…, [viewingToday, eatNext.result, eatNext.computedAt])`, guarded by `if (!viewingToday || !eatNext.result || !eatNext.computedAt) return;`. Both values come from the same `useEatNext` object read in the same render, and the hook only ever publishes them together (`useEatNext.ts:338-340` — `setError(null); setResult(next); setComputedAt(now);` in one block, all after the `runId !== runIdRef.current` stale-guard, with no other `setComputedAt` call site in the file), so the pair can never straddle two loads: a failed refetch takes the `catch` path and touches neither (`:341-344`), and a superseded in-flight load returns before both. `computedAt` is `Date | null`, hence the third clause of the guard — pre-first-load there is no decision to sync, so it is a deliberate no-op rather than a case to paper over. **Why it cannot mis-schedule:** `nudgeFireDate(fireAtMinutes, sourceDay, now)` (`eatNext.ts:124-145`) returns `null` unless `sourceDay` and the service's own `new Date()` are the same local calendar day, so the only two outcomes are "scheduled on the day the decision was computed" or "not scheduled at all" — the cross-midnight case (decision computed at 23:58, effect runs at 00:01) resolves to `null` and cancels rather than firing ~24h late. Passing the hook's `Date` object straight through, never mutating it (`nudgeFireDate` constructs a fresh `Date` rather than calling `.setHours` on it, per its own doc comment), keeps the hook's state uncorrupted. Note for the record that with `nudgesEnabled` still hardcoded `false` (`useEatNext.ts:328`, until Task 11 applies the migration), `result.nudge` is always `null` today, so this resync currently only ever *cancels* — no nudge can fire from either surface until Task 11 lands, and nothing here was verified against a real fired notification.

**Correction B — floated promise has a `.catch`.** `void syncEatNudge(...).catch((e) => { console.error("MealsScreen nudge resync:", e); })`, same shape and same reasoning as `EatNextHomeCard.tsx:68-70`, with a distinct log prefix so the two resync points are distinguishable in the console. Re-verified the rejection path is real rather than inherited from the plan's assertion: `requestPermissions` (`notificationService.ts`) has no try/catch around its `getPermissionsAsync`/`requestPermissionsAsync` calls, `syncEatNudgeCore` awaits it outside its own try (`eatNudgeService.ts:113`, with a comment saying the rejection propagates deliberately), and `serialize` returns the op's promise to the caller (`:57-62`) — so an uncaught rejection here would be a dev yellow-box and silence in prod. Not surfaced to the user: this is a background effect with no gesture to attach an alert to, matching how the whole family handles it.

**Correction C — `useLocalSearchParams` kept, not "improved".** Re-derived the finding from the installed source rather than trusting Task 9's note: `useGlobalSearchParams` (`node_modules/expo-router/build/hooks.js:147-149`) is `return useRouteInfo().params` — raw, no mapping; `useLocalSearchParams` (`:150-169`) maps entries and special-cases `if (value === undefined) return [key, undefined]` at `:156-158`, directly above the library's own comment at `:154-155` ("React Navigation doesn't remove 'undefined' values from the params object, and you cannot remove them via navigation.setParams as it shallow merges. Hence, we hide them here"). expo-router is `6.0.24` (`node_modules/expo-router/package.json:3`), matching the plan's `~6.0.24`. Also confirmed `router.setParams` exists on the module-level `router` export (`build/imperative-api.js` — `setParams: routing_1.setParams`), so the imperative singleton is usable here without `useRouter()`. The choice is recorded in a comment on the import line so a future reader can't "simplify" it back.

**Correction D — §7.2 chip copy: my reading, and what I built.** Spec §7.2's example is *"Korean Beef Bowl · 450 cal behind"*. **"450 cal behind" is unambiguously the pace GAP, not the meal's calories** — read as English it is `catch_up`'s own message string (`eatNext.ts:371` produces exactly `` `${gap} cal behind pace` ``), and no per-recommendation field is ever described as "behind". But the gap is a **context-level** number that `EatNextResult` does not expose numerically at all: `message` is prose, `recommendations[]` carries no gap field, and the only numeric gap in the codebase is `caloriePace.catchUpAmount`, which on this screen is computed by MealsScreen's *own* `computeMealPace` call (`MealsScreen.tsx:1154-1164`) off a different clock instant and a different totals read than `useEatNext`'s. Rendering a number from that source next to a recommendation from the engine would be exactly the "two surfaces disagree" failure spec §6 exists to prevent, and the row's contract ("a dumb renderer — the hook/engine owns all logic") forbids it. The gap is also meaningless in 4 of the 6 contexts: `next_meal` and `post_workout` have `message: null` and no gap at all, `goal_hit`/`after_window` have no gap either — so a per-chip gap has nothing to render outside `catch_up`/`emergency`.

So I split the example into the two places its two halves actually belong: **the gap appears once, at strip level, in the engine's own words** (`result.message`, rendered on its own line under the "SUGGESTED NOW" label — its own `Text`, not folded into the label, because uppercased with `letterSpacing` the longest co-occurring message would overflow the strip width), and **each chip carries its own numbers** — `{calories} cal · {prepMinutes} min`, both from `EatNextRecommendation`, which Task 8 added *precisely* so §7.1 and §7.2 could render numbers instead of prose (see §7.2's own citation in the §5.2 amendment: "§7.2 needs the identical numbers for its own chip row"). `prepMinutes` rather than `protein` because the strip is specified as slim and prep is the "can I actually eat this now" gate the engine's own prep budget cares about; `calories` because it is the figure §7.2's example centers on. Worth noting that in `catch_up` — the context the spec's example is drawn from — the two numbers nearly coincide by construction: candidates are filtered to within ±35% of the gap (`CATCH_UP_BAND`, `eatNext.ts:227`), so a 450 cal gap surfaces ~450 cal meals, which is very likely why the example reads ambiguously in the first place. Net effect on screen for the spec's own scenario, **as revised in review round 1** (the strip-level message line was removed — see the Task 10 round 1 section below; the shipped strip is `SUGGESTED NOW / [Korean Beef Bowl · 430 cal · 5 min]`). **Deliberately dropped the plan's `rec.reasons[0]` second line** — it predates the display fields, and for `catch_up` it reads "closes the ~450 cal gap (430 cal, 5 min)", which restates both the strip-level gap and the chip's own numbers as prose in a chip the spec asks to keep slim. The full reasoning is recorded in the row file's header comment, not just here.

**Verification 1 — anchors, all found by content; one plan claim disproved.** `viewingToday` is declared at `MealsScreen.tsx:1149` (pre-edit) as `viewingDateStr === todayStr`, and the pace block is `{viewingToday && (<View style={{ marginHorizontal: 20, marginBottom: 12 }}><MealsPaceLines … /></View>)}` at `:1412-1419` — the row went *inside* that `View`, directly after `MealsPaceLines`, so it inherits both the guard and the 20pt gutter. State name is `libraryVisible` (`:147`, `useState(false)`). The `MealLibraryModal` render block is `:1606-1620` with exactly the props the plan's preconditions claimed — `visible`, `savedFoods`, `todayDate`, `onClose`, `onLogged` — and it has exactly one consumer in the app (grepped: `MealsScreen.tsx` only), so the new optional prop reaches no other call site. **`router` was NOT already imported** (the plan's parenthetical "`router` is already imported in this file; if not, import from `expo-router`" — the file had no `expo-router` import at all pre-edit, verified by reading the full import block `:1-84`); added `import { router, useLocalSearchParams } from "expo-router"`.

**Verification 1b — the shared post-write refetch path: there is exactly ONE funnel, and it is `fetchMealsForDate(date, force)`.** This was the riskiest claim to check, so it was enumerated exhaustively rather than sampled. Every write handler in the file follows the identical two-step shape — invalidate the day's cache entry, then call `fetchMealsForDate(viewingDate, true)` — and there are **nine** such forced call sites, all mapped to their enclosing handler (post-edit line numbers): `handleRefresh` `:360`, `handleUndoLastLog` `:476`, `handleLogMealFromPreview` `:629`, `handleManualSaveAndLog` `:859`, `handleAddMeal` `:1017`, `handleQuickAdjustment` `:1069`, `handleSaveMealEdit` `:1159`, `handleDeleteMeal` `:1186`, and the `MealLibraryModal`'s `onLogged` `:1733`. The only unforced call is the `viewingDate` navigation effect (`:315`). So the hook-in point is the `if (force)` branch inside `fetchMealsForDate` — the same branch the pre-existing `refreshHistory()` call already occupies for the identical reason ("a forced fetch means the day was just written to") — and one line covers all nine writers, including the library modal's, with no write handler touched. **Two honest caveats.** (i) `handleAddMeal` refetches only `if (mealDate === viewingDateStr)` (`:1016`), so logging via the add form to a *different* day than the viewed one reaches no refetch at all; when that other day is today (viewing a past day, dating the entry today), today's totals change and the recommender is not told. That is a pre-existing gap in the screen's own refresh logic — the day cache is invalidated but nothing refetches — and closing it means editing a write handler, out of scope here; the Home card's focus refetch covers it on the next Home visit. Reported, not fixed. (ii) I added `&& dateStr === todayStr` to the new line, because the recommender always computes for **today** regardless of the viewed date, so a write to a past day cannot change its answer; this only suppresses provably-pointless network work and, per (i), suppresses nothing the unguarded form would have caught.

**Verification 2 — `MealLibraryModal`'s current shape, and the `detailMeal === undefined` fallback: it renders the LIST, not a blank body, so the plan's optional guard was NOT added.** Pre-edit shape confirmed: props exactly as the plan claimed (`:31-37`); state machine `type View_ = { mode: "list" } | { mode: "detail"; mealId: string } | { mode: "builder"; mealId: string | null }` (`:26-29`) — so the detail mode's shape is `{ mode: "detail", mealId }`, which is what the new effect constructs; visibility effect `useEffect(() => { if (visible) { setView({ mode: "list" }); load(); } }, [visible, load])` (`:69-74`), replaced with the plan's `initialMealId`-branching version and `initialMealId` added to the deps. The fallback claim was then traced through the body chain (`:308-379` pre-edit) in order: `!data && loadFailed` → retry body; `!data` → spinner; `view.mode === "detail" && detailMeal` → `MealDetail`; `view.mode === "builder"` → `MealBuilder`; **`else` → the `SectionList`**. With `view.mode === "detail"`, `data` non-null and `detailMeal` `undefined` (stale or deleted id), clauses 3 and 4 are both false and control reaches the `else` — **the list renders**. The plan's conditional instruction ("if it renders nothing, add … guard logic") therefore does not apply and no guard effect was added. One cosmetic consequence, stated rather than fixed: the header still reads from `view`, so it shows `detailMeal?.name ?? "Meal"` → "Meal" with a "‹ Library" back action above a list body until the user taps that action (which sets `{ mode: "list" }` and reconciles it). Self-healing in one tap, reachable only via a deep link to a since-deleted meal, and fixing it would mean adding a `setView` inside a render-phase-derived condition — worse than the symptom. Also confirmed the new dep doesn't misfire: `initialMealId` changes only when MealsScreen sets or clears it, and its clear is batched with `visible → false`, where the effect's `if (visible)` guard makes the re-run a no-op; and logging from a deep-linked detail leaves `initialMealId` untouched, so `handleLog`'s `setView({ mode: "list" })` (`:188`) stands rather than being bounced back to detail.

**Verification 3 — no double-load, and nothing to compound.** `MealsScreen.tsx` has **no** `useFocusEffect` and no other focus-driven reload (grepped `app/` + `src/`: the only `useFocusEffect` call sites in the app are `MealsHomeCard`, `WaterIntakeHomeCard`, `RampHomeBanner`, `EatNextHomeCard` — all Home components), and it is rendered from exactly one place, `app/(tabs)/track/meals.tsx:9`, with no other mount point. So `useEatNext()` with no `refreshKey` gives exactly one load per mount (the hook's own mount effect, `useEatNext.ts:352-363`) and Task 8's `firstFocus` ref has no analogue to fix here — the deliberate absence of a focus reload is recorded in a comment at the hook call site. Subsequent reloads come only from the write funnel. Note the two surfaces do overlap by design (Home card focus load + this screen's write refetch), which is precisely the overlap `useEatNext`'s `runIdRef` stale-guard and `eatNudgeService`'s `serialize` queue were built for — both pre-existing, neither newly stressed by this task beyond what their own comments already anticipate ("MealsScreen after every meal-log write").

**Verification 4 — `suggestMealId` traced end to end.** Param name matches exactly: `EatNextHomeCard.tsx:140-144` pushes `{ pathname: "/(tabs)/track/meals", params: { suggestMealId: top.mealId } }`; the route file `app/(tabs)/track/meals.tsx` exists and renders `MealsScreen`, so the pathname resolves. `useLocalSearchParams<{ suggestMealId?: string }>()` is read inside `MealsScreen` rather than the route file (the file list permits only three files, and `libraryVisible` lives here); this works because the hook reads `LocalRouteParamsContext`, which the enclosing `Route` provides to descendants — and MealsScreen having exactly one mount point means there is no second route whose params it could read or clear by accident. The effect sets `libraryInitialMealId` and `libraryVisible`, so the modal's visibility effect fires with a target and lands on `{ mode: "detail", mealId }`. Consumed once: `router.setParams({ suggestMealId: undefined })` runs in the same effect body, and per Correction C the next `useLocalSearchParams()` read reports `undefined`, so the effect re-fires exactly once with a falsy param and the guard stops it — one extra render, no loop, the lifecycle Task 9 traced for `?modal=nutrition`. Navigating back and forth cannot re-open it: `onClose` pops the route (`meals.tsx:9`), unmounting the screen and discarding `libraryInitialMealId`/`libraryVisible`, and a later arrival via the "Meal Library" button pushes the route with no params, so the effect's guard is false on that fresh mount. If instead the card is tapped while the route is already in the stack, the param arriving anew is the correct trigger and re-opening at that meal is the intended behavior.

**Verification 5 — spec §7.2 clause by clause.** "Slim strip rendered directly under `MealsPaceLines`" — inside the same wrapper `View`, immediately after that element, first child after it. "today view only, same `viewingToday` guard" — literally the same `{viewingToday && …}` block, no second condition of its own; note the enclosing block is also `!showAddForm && activeTab === "today"`, i.e. the Today tab, which is where the pace lines live, so the row can never appear on Insights or over the add form. "top 2 recommendations as tappable chips" — `result.recommendations.slice(0, 2)`, each a `TouchableOpacity` (the engine returns up to `TOP_N = 3`, so this slice is the §7.2 narrowing, not a re-ranking). "tap opens the library modal at that detail" — `onMealPress` sets `libraryInitialMealId` then `libraryVisible`, the same pair the deep link sets, landing on `{ mode: "detail" }`. "Refreshed by the screen's existing write-refetch path, so logging a meal immediately updates or clears the suggestion" — Verification 1b's single funnel. Chip copy per Correction D.

**Also assess — is the plan's "stale in this closure" reasoning right, and does the effect fire when it should without firing spuriously? Yes, and yes.** `refetch` is `load`, an async `useCallback` that resolves state later, so a handler that called `syncEatNudge(eatNext.result.nudge, …)` right after `eatNext.refetch()` would pass the **pre-write** decision — sync the very state the write invalidated. The effect form is correct. It fires when it should because `recommendEatNext` returns a freshly-constructed object on every successful load, so `setResult` always changes identity and the effect re-runs after each post-write reload as well as after the initial load. It does *not* fire spuriously: on a failed refetch the hook takes its `catch` and leaves `result`/`computedAt` identities untouched (stale-while-revalidate), so the dep array is unchanged and nothing re-syncs a decision the user is no longer looking at. The one redundancy left is deliberate and named in the plan: this screen's effect and the Home card's effect can both sync the same decision, which is harmless because the service is a serialized cancel-then-schedule (`eatNudgeService.ts:47-63`) and re-scheduling the same `fireAtMinutes` on the same `sourceDay` is idempotent.

**Also assess — `viewingToday` gating the sync effect: switching to a past day and back neither cancels nor staleness-traps the nudge.** The gate narrows only *when* the effect runs, never what it computes: `useEatNext` always evaluates today, whatever date the screen is showing, so nothing about `result.nudge` becomes wrong while a past day is on screen. Switching away runs no cancel (the effect's early `return` calls nothing — this is the specific thing worth stating, since a gate that *did* cancel would silently delete a valid pending nudge every time the user browsed yesterday); the pending nudge simply stands, which is correct because it is still today's decision. Switching back re-runs the effect with the same `result`/`computedAt` pair — the guard passes and the service cancels-then-reschedules the identical instant. The only cost of the gate is a skipped resync while a past day is being viewed, recoverable at the next one, matching the tolerance the service already documents for a failed cancel. Combined with Verification 1b's `dateStr === todayStr` narrowing on the refetch, past-day browsing and past-day editing are both completely inert with respect to the recommender — which is the intended reading of §7.2's "today view only".

**Also assess — the clear case works.** `if (!result || result.recommendations.length === 0) return null` in the row, plus the write-path refetch, gives §7.2's "immediately … clear the suggestion": logging enough calories flips the engine to `goal_hit`, whose terminal branch returns `recommendations: []` (`eatNext.ts:299`) — and whose protein-short exception returns exactly one recommendation, so the strip narrows to a single chip rather than vanishing, which is the more useful behavior and needs no special-casing. `after_window` (`:257`) and the empty-library / no-fit `next_meal` messages (`:400-405`) also produce zero recommendations and thus no strip; those context messages are §7.1's job ("never a blank card"), which is why the row renders nothing rather than a message-only strip — recorded in the row's own comment. The refetch → new `result` → re-render chain is what makes it "immediate"; there is no separate invalidation to get wrong. Not verified on device (no Metro/simulator run in this task): this is a code-path trace, not an observed clear.

**Also assess — chip copy fits within `numberOfLines={1}`.** Chips are full-width in the pace block's 20pt gutter (~335pt on a 375pt screen, less 12pt horizontal padding each side ≈ 311pt of text width). Line 2 is the bounded one and always fits: the longest realistic content is 4-digit calories with 3-digit prep, `"1200 cal · 120 min"` — 18 chars at 12pt ≈ 110pt, a third of the width (the engine's own prep hard cap is `maxPrepMinutes × 2`, so 3-digit prep needs a ≥50 min budget; either way it fits). Line 1 (`rec.name`, 14pt semibold) is the only string that can truncate — roughly 40 chars fit, and `meals.name` has no length constraint, so a long user-authored name like "Korean Beef Bowl with Rice and Extra Vegetables" (47) will ellipsize. Accepted: truncating a name the user wrote themselves (and can see in full one tap later, on the detail the chip opens) is the right trade for a slim strip, and it is the same `numberOfLines={1}` treatment §7.1's card gives its title. The strip-level message line is the reason it is not folded into the uppercase label: the longest message that can co-occur with a recommendation is `"Calorie target hit — protein still short."` (40 chars) — fine at 12pt on its own line, but appended to "SUGGESTED NOW · " with `textTransform: "uppercase"` and `letterSpacing: 0.5` it would exceed the width.

**Deviation 1, quoted with justification — `router` imported, contradicting the plan.** The plan's Step 3 said "(`router` is already imported in this file; if not, import from `expo-router`.)" It was not: pre-edit `MealsScreen.tsx` imported nothing from `expo-router` (full import block read, `:1-84`). Took the plan's own fallback and added `import { router, useLocalSearchParams } from "expo-router"`, using the module-level singleton rather than `useRouter()` — this file has no existing router local to reuse (unlike `profile.tsx`, where Task 9 correctly avoided a second binding), and `setParams` is present on that singleton (Correction C).

**Deviation 2, quoted with justification — hook declared with the state block, not "near the pace `useMemo`s".** The plan's Step 3.2 said "Hook: near the pace `useMemo`s (~:1149): `const eatNext = useEatNext();`". Declared it at `:171` instead, with the other hooks, because `fetchMealsForDate` (`:255`) now reads `eatNext.refetch` — the same relationship `useHistoricalMeals`'s `refreshHistory` (`:194`) already has with that function, so this matches the file's existing convention rather than inventing one. Declaring it at `:1149` would have compiled and, in fact, worked (the reference is inside a function body that only ever runs post-render, after every `const` is initialized), but it would leave a 900-line-later declaration read by a function defined earlier — fragile in exactly the way the *next* deviation shows can bite.

**Deviation 3, quoted with justification — the sync effect is NOT "near the other effects"; that placement is a runtime crash.** The plan's Step 3.6 said to "add, near the other effects" a `useEffect(…, [viewingToday, eatNext.result])`. Placed literally (with the effects at `:314-339`), a dependency **array is evaluated during render**, so `[viewingToday, …]` would read `viewingToday` before its `const` initializes at `:1206` — a TDZ `ReferenceError` on every render of the Meals tab. **Correction, applied in review round 1: my original wording here claimed this was "invisible to `tsc`". That is false, and the false version taught a wrong lesson about the gate.** Reproduced the plan's literal placement in the real file and ran the project's own `npx tsc --noEmit`, which fails with two diagnostics: `error TS2448: Block-scoped variable 'viewingToday' used before its declaration.` and `error TS2454: Variable 'viewingToday' is used before being assigned.` Both are reported at **line 344, column 7 — the `viewingToday` token inside the dependency array**, not the reference in the effect body two lines above it (which `tsc` correctly leaves alone, since a function body only runs post-render). So the type-checker pinpoints exactly the render-phase half of the mechanism, and the defect would have failed the gate every task in this branch runs before it could ever execute. The finding stands and the relocation is still correct — the plan's placement *is* wrong — but it was catchable by the existing gate, not silent. The effect therefore sits immediately after `proteinPace`'s `useMemo` (`:1234-1274`, guard and call at `:1270-1274`), i.e. at the end of the today-derived block whose `viewingToday` it reads, with the reason recorded in a comment so it isn't "tidied" back up later. The alternative — inlining `viewingDateStr === todayStr` to keep the plan's placement — was rejected as duplicating the expression `viewingToday` exists to name.

**Deviation 4, quoted with justification — the write-path refetch is not `await`ed, and carries a day guard.** The plan's Step 3.6 said to "append `await eatNext.refetch();`" to the shared path. Written as `if (force && dateStr === todayStr) eatNext.refetch();`, immediately after the pre-existing `if (force) refreshHistory();`. No `await`: `refetch`'s declared type is `() => void` (`UseEatNextValue`, `useEatNext.ts:87`), so `await` on it would be a no-op that *reads* as ordering guarantee — and there is genuinely nothing to order against, since the nudge sync deliberately happens in the effect instead (which is the plan's own conclusion two lines later, `// eatNext.result is now stale in this closure`). Not awaiting also matches the `refreshHistory()` line it sits beside. The `dateStr === todayStr` guard is justified in Verification 1b.

**Deviation 5 — chip copy, per Correction D** (`rec.calories`/`rec.prepMinutes` + strip-level `result.message`, replacing the plan's `rec.reasons[0]`). Pre-authorized by Correction D; full reasoning there and in the row file's header comment.

**Gates:** `cd mobile && npx tsc --noEmit` → **0 errors**. `cd mobile && npm test` → **8/8 suites, 206/206 tests**, unchanged from baseline. No new tests: both new/changed UI files import `react-native` (and `MealsScreen` now transitively imports `expo-notifications` via `eatNudgeService`), which cannot load under this repo's `testEnvironment: node` Jest config — the same exclusion recorded for Tasks 6, 8 and 9, and the reason no RN import was dragged into the node scope. No `as any`, no `@ts-expect-error`, `StyleSheet.create` used, styles follow the palette the plan's snippet and `EatNextHomeCard` share. Files touched: `mobile/src/components/track/meals/EatNextRow.tsx` (new), `mobile/src/components/track/meals/library/MealLibraryModal.tsx`, `mobile/src/components/track/MealsScreen.tsx`, this plan doc — no Phase 1/2/3 engine, hook, or service file modified (`git diff --stat` confirms). **No database-connecting command run at any point** (no `supabase` CLI, no Metro/dev server, no simulator). Four commits, in order: `EatNextRow`, the `initialMealId` prop, the MealsScreen wiring, this docs commit.

#### Task 10 — review round 1

Quality review approved as ready to merge; spec review approved the §7.2 chip-copy judgment with one must-fix defect and two documentation corrections. The verifications that mattered most were run *against the file* rather than accepted: no component-level early return exists anywhere in `MealsScreen` (every `return` at ≤4-space indent is inside a nested handler or callback; the sole component-level return is the JSX), so the three new hooks cannot shift hook order on any path — the single largest blast-radius risk in this task, and it did not materialize. `todayStr` (`:224`) predates `fetchMealsForDate` (`:255`), so the new line's reference is a plain closure with no ordering subtlety, and `eatNext.refetch()` is internally try/caught, so it cannot throw into `fetchMealsForDate`'s catch and produce a spurious "Failed to load meals" alert. The write-path enumeration (Verification 1b) was independently confirmed complete at ten call sites — nine forced plus the unforced nav effect — with every `meal_logs` mutation in the file followed by a forced fetch. Three fixes applied, plus a spec amendment; four items recorded as observations for Task 12.

**Fix 1 (must-fix defect) — removed the strip-level `result.message` line from `EatNextRow`.** My Correction D reasoning was right about *provenance* (use the engine's words; never re-derive the gap from the screen's `catchUpAmount`) and wrong about *placement*: the screen already owns that fact one line up. `EatNextRow` renders at `MealsScreen.tsx:1520`, directly beneath `MealsPaceLines` at `:1513` in the same `viewingToday` block, and that component's `behind` branch (`MealsPaceLines.tsx:56-62`) already renders `Calories: {delta}cal behind · eat {catchUpAmount}cal by {catchUpLabel}` — the same gap, stated *more* precisely (it adds the catch-up target and the milestone label). Read the component to confirm rather than taking the review's word: verified that branch, and verified the `goal_hit` case too (`:39-43` renders `goal hit ✓` on the calorie line while the protein line takes the same `behind` branch, so the protein shortfall the `goal_hit` protein-short message states is likewise already on screen). All three contexts where a message can co-occur with a recommendation — `catch_up`, `emergency`, `goal_hit` protein-short — are therefore pure duplication. The second problem is the serious one: `useEatNext` and MealsScreen fetch `meal_logs` separately and sample `new Date()` separately, and after a write their settles are asynchronous and independent, so there is a real window in which those two adjacent lines display **different numbers for the same gap** — the on-screen form of exactly the disagreement §6's single-assembly-point rule exists to prevent. Removed the conditional, the `<Text>`, and the now-unused `message` style; the removal and its surface-specific reasoning are recorded in the row file's header comment, including the explicit note that this does **not** apply to Task 8's Home card, which has no pace lines beside it and correctly still renders `message`.

**Fix 2 (documentation) — spec §7.2 amended in place.** The spec had been left untouched, so anyone diffing §7.2's quoted `"Korean Beef Bowl · 450 cal behind"` against the shipped screen would read it as a regression. Added a dated, coordinator-attributed amendment in the §5.2/§5.6/§6/§8.1 style, in two paragraphs: the first records that "450 cal behind" is the gap rather than a meal property, that the gap is context-level and prose-only so a per-chip rendering would mean either repeating one number across both chips or re-deriving it against §6, that it is absent entirely in `post_workout`/`next_meal` where `message` is `null`, and that the chips therefore carry `{calories} cal · {prepMinutes} min` from the Task 8 display fields. The second records Fix 1 — that the gap is not rendered at strip level either, because `MealsPaceLines` already renders it one line above from independently computed state — and scopes that to §7.2 only. It closes by telling a future engineer who adds a numeric gap field to `EatNextResult` that the per-chip half becomes implementable and should be revisited, while the strip-level half should not, since it turns on the adjacency of `MealsPaceLines` rather than on the data model.

**Fix 3 (documentation) — the TDZ claim's characterization corrected in Deviation 3 above**, where the false "invisible to `tsc`" sentence lived. Verified the correction myself rather than accepting it: reproduced the plan's literal placement in the real file, ran `npx tsc --noEmit`, captured both diagnostics (`TS2448`, `TS2454`), and restored the file via `git checkout` to a verified-clean diff. The reproduction added one detail worth keeping — both errors are reported at **column 7 of the dependency-array line**, the `viewingToday` token inside `[…]`, and *not* at the reference in the effect body two lines earlier. That is the mechanism confirmed precisely: `tsc` flags the render-phase evaluation and correctly ignores the post-render one.

**Also applied — `colors.*` in place of hardcoded hex.** `EatNextRow` had inherited `#111827`/`#1F2937`/`#FFFFFF`/`#9CA3AF` from the plan's snippet, but `RecentFoodChips.tsx:90-115` — a chip row two files away in the same directory, which can be on screen at the same time — uses `colors.card`/`colors.border`/`colors.foreground`/`colors.mutedForeground`. Since `colors.card` is `#1E293B` (`colors.ts:12`), the two chip rows were rendering visibly different backgrounds. Switched all four. Kept the local `StyleSheet` rather than moving into the shared `mealsScreenStyles`, per the ruling and matching `MealsPaceLines`/`RecentFoodChips`.

**Also applied — the stale-`initialMealId` guard, placed after the load.** This closes the cosmetic gap Verification 2 above reported and declined to fix. The placement is the point: the guard cannot live in the visibility effect, because `load()` has not resolved there and `data` is null, so there is nothing to validate the id against. It is a separate effect keyed on `[data, view]`, resetting `detail` → `list` only when `data` is present and no meal matches `view.mealId`. Cannot loop (it only ever moves `detail` → `list`, and its own `view.mode === "detail"` test is false on the next run) and cannot fire on a live target. Reachability remains narrow — a meal deleted between the recommendation load and the tap, in one session — but the pre-fix behavior left the header reading "Meal" with a `‹ Library` action over the library list, where that action looks broken precisely because the list is already what's on screen.

**Observations for the Task 12 checklist — recorded, not fixed.**
- **Chip geometry vs `RecentFoodChips`**: `borderRadius: 10` against its pill `20`, and full-width stacked blocks against its `flexDirection: "row", flexWrap: "wrap", gap: 8, maxWidth: "48%"`. Ruled a legitimate judgment call (meal names plus a stats line truncate badly in a 48% pill), but the strip still runs label + two two-line chips, so **flag for the owner's on-device pass**: confirm it still reads as "slim" per §7.2. Fix 1 removed one line from it.
- **One-frame stale view on modal re-open**: `onClose` does not reset `view` and the visibility effect is deferred, so the first painted frame of a re-open can show the previous view. Pre-existing in kind, lands inside the slide-in animation, likely imperceptible — device pass only, do not fix.
- **`handleAddMeal`'s conditional refetch** (Verification 1b, caveat i) confirmed pre-existing and correctly left alone: when it skips, `fetchMealsForDate` is not called at all, so the day list is equally stale — an inherited bug, not one this task added. And because §7.2's row renders only in the today view, the user cannot be looking at a stale row while viewing another day.
- **`nudgesEnabled` is still hardcoded `false`** (`useEatNext.ts:328`), so `result.nudge` is always `null` and both resync points can currently only ever *cancel*. The nudge half of §7.2/§8.1 therefore **cannot be verified end to end until Task 11's Step 4 wiring lands after the owner gate** — nothing in Tasks 8 or 10 was checked against a notification that actually fired. This belongs on the Task 12 checklist.

**Gates (round 1):** `cd mobile && npx tsc --noEmit` → **0 errors**. `cd mobile && npm test` → **8/8 suites, 206/206 tests**, unchanged. Files touched this round: `mobile/src/components/track/meals/EatNextRow.tsx`, `mobile/src/components/track/meals/library/MealLibraryModal.tsx`, `docs/superpowers/specs/2026-07-29-nutrition-recommender-design.md` (§7.2 amendment, item 2's granted scope extension), this plan doc. `MealsScreen.tsx` needed no change this round — the TDZ item was a doc correction, and the file was restored to a verified-clean diff after the `tsc` reproduction. No database-connecting command run at any point (no `supabase` CLI, no Metro, no simulator). Three commits this round plus this docs commit: the spec amendment, the `EatNextRow` fix (message removal + `colors.*`), the `MealLibraryModal` stale-id guard.

### Task 11

**Steps 1–3 (the apply itself) were executed outside this session, with the owner's explicit go-ahead.** Recorded here for continuity, per the coordinator's account of the post-verify: `profiles.eat_nudges_enabled` now exists as `boolean not null default false` and was the *only* line added to the entire `public` schema; both dead views (`daily_nutrition_summary`, `daily_water_summary`) are gone, taking the view count from 11 to 9; the table count (82) and its sorted name list are identical before and after; and `npx supabase migration list` records `local == remote`. This session ran **no database-connecting command** — no `supabase` CLI, no `db push`, no `migration list`, no dump, no psql — per its own constraints; Steps 1–3's checkboxes above are left unticked here since this session did not perform or independently re-verify them, only Step 4.

**Step 4 — post-apply wiring — DONE. All three edits applied exactly as the plan's text specified:**

1. **`mobile/src/hooks/useEatNext.ts`** — `interface ProfileRow` gained `eat_nudges_enabled: boolean;` (`:41`), replacing the `NOTE:` comment that marked its absence. `PROFILE_COLUMNS` gained the matching `eat_nudges_enabled: true,` (`:68`). `PROFILE_SELECT` is derived from `PROFILE_COLUMNS`, so the column is now in the actual select string with no separate edit needed.
2. **Same file** — the `recommendEatNext` call's `nudgesEnabled: false` argument (with its 15-line hardcode comment) is now `nudgesEnabled: p.eat_nudges_enabled` (`:313`), comment deleted.
3. **`mobile/src/components/profile/NotificationsScreen.tsx`** — `eat_nudges_enabled` added to the profile `.select(...)` string (`:104`, appended after `meal_reminder_types`, matching the plan's specified position). The block comment above `const [eatNudges, setEatNudges] = useState(false)` (`:93` pre-edit, naming the migration/42703/deferred-edit list) is deleted; the `useState(false)` declaration itself is unchanged (still the correct initial value pending the fetch). Inside the existing profile-load `useEffect`'s `if (data)` block, added `setEatNudges(!!data.eat_nudges_enabled)` (`:116`), placed directly after `setMealEnabled(...)` — grouped with the meal-settings seed rather than the water one, since it reads more naturally beside the other boolean-toggle seed than interleaved into the water-times block. Same `!!` coercion idiom as `enabled`/`mealEnabled`.

**Verification 1 — column name confirmed against the applied schema, not just intent.** `grep -n "eat_nudges_enabled" supabase/migrations/20260729110000_*.sql` → line 6 (a comment) and line 26: `add column if not exists eat_nudges_enabled boolean not null default false;`. Matches the coordinator's post-apply account exactly: `eat_nudges_enabled`, boolean, not null, default false.

**Verification 2 — the `PROFILE_COLUMNS` bidirectional check, actually fired, not assumed.** Two isolated trials, each restored to a byte-identical file before the next (confirmed via `diff` against a saved-off copy):
- Removed `eat_nudges_enabled: boolean;` from `ProfileRow` only (kept the `PROFILE_COLUMNS` key and the `p.eat_nudges_enabled` use site). `npx tsc --noEmit` produced **two** errors: `TS2353: Object literal may only specify known properties, and 'eat_nudges_enabled' does not exist in type 'Record<keyof ProfileRow, true>'` at the `PROFILE_COLUMNS` literal, and `TS2339: Property 'eat_nudges_enabled' does not exist on type 'ProfileRow'` at the `p.eat_nudges_enabled` call-site read (an extra diagnostic beyond what the plan called out, since this trial also broke the read site — expected, and it does not undermine the check).
- Removed `eat_nudges_enabled: true,` from `PROFILE_COLUMNS` only (kept the `ProfileRow` field). `npx tsc --noEmit` produced exactly the plan's predicted diagnostic: `TS1360: Type '{...}' does not satisfy the expected type 'Record<keyof ProfileRow, true>'. Property 'eat_nudges_enabled' is missing in type '{...}' but required in type 'Record<keyof ProfileRow, true>'`.
- Both trials restored via `cp` from a pre-edit backup; `diff` confirmed byte-identical to the correct post-edit file before re-running `tsc` clean. The mechanism the plan relies on for Step 4 is real, not asserted.

**Verification 3 — the nudge path traced end to end.** `computeNudge` (`mobile/src/lib/eatNext.ts:433-438`) opens with `if (!nudgesEnabled) return null;` — the only gate this migration's wiring touches; nothing else in the engine reads `eat_nudges_enabled` or changes shape based on it. The owner's profile row reads `eat_nudges_enabled = false` (the backfilled default — no row has ever set it `true`), so `p.eat_nudges_enabled` is `false`, `nudgesEnabled` is `false`, and `computeNudge` returns `null` on every call — `EatNextResult.nudge` is always `null` today, exactly as before Step 4, just now sourced from a real column instead of a hardcode. Traced the consumers: both call sites (`EatNextHomeCard.tsx:68`, `MealsScreen.tsx:1271`) call `syncEatNudge(result.nudge, computedAt)`, and `syncEatNudgeCore` (`eatNudgeService.ts:98-114`) unconditionally cancels first, then does `if (!decision) return;` — so with `decision === null`, every resync degenerates to cancel-only, confirmed exactly as expected. No other engine behavior changes when the flag is read live versus hardcoded `false`: `computeNudge`'s only reference to `nudgesEnabled` is that single early return (spec §5.6's gate), and `recommendEatNext`'s other five context branches never read it at all (grepped `nudgesEnabled` in `eatNext.ts`: two hits, the interface field and this one guard).

**Verification 4 — the Notifications screen toggle round-trips.** Read `handleEatNudgesToggle` (`NotificationsScreen.tsx:392-442`, unmodified by this task) against the now-real column: the `.update({ eat_nudges_enabled: value }).eq("id", user.id)` write (`:404`) previously 42703'd on every call; the same RLS/update pattern the water and meal toggles already use against `profiles` applies here, so the write now succeeds. The fetch effect's new `.select(...)` + `setEatNudges(!!data.eat_nudges_enabled)` means the switch is seeded from the persisted row on every mount, so leaving and re-entering the screen no longer resets it to `false` — the specific symptom the plan's on-device check (`toggle Pace nudges off and back on, confirm it survives leaving and re-entering the screen`) exists to catch. Toggle-off still calls `cancelAllEatNudges()` unconditionally (`:435`, untouched). **The ON-path permission request (added in Task 6's review round) now behaves correctly with a real write behind it**: previously the `.update` 42703'd, the toggle's `if (error)` branch fired every time, and the `requestPermissions()` branch below it was unreachable in practice pre-migration; now the write can succeed, so `requestPermissions()` actually runs at the gesture, and on denial the existing "Notifications Disabled" alert + settings-write-back-to-`false` + `setEatNudges(false)` revert (`:417-431`) is live for the first time. Not verified on a physical device or simulator in this session (no Metro/simulator run, per the task's constraints) — this is a code-path trace confirming the logic is now reachable and correct, not an observed on-device toggle.

**Verification 5 — no other deferral placeholders remain.** `grep -rn "eat_nudges_enabled" mobile/src` → all eight hits are live code (the interface field, the `PROFILE_COLUMNS` key and its doc-comment mention, the `nudgesEnabled: p.eat_nudges_enabled` call, and the screen's select/seed/two `.update(...)` calls) — none are hardcodes or placeholders. `grep -rn -i "42703\|Task 11\|deferr" mobile/src`: the `42703` hits left in `useEatNext.ts` are the general `toError()` doc comment describing PostgREST's error shape (not specific to this deferral); the `Task 11` hits are `useEatNext.ts`'s comment on why `PROFILE_COLUMNS` is bidirectionally checked (still accurate — that mechanism is exactly what Step 4 exercised) plus two hits in `RampCard.tsx`/`mealScore.ts` that reference an unrelated "Task 11" from a different plan's numbering (meal-library / meal-score UI, not this migration). `grep -rn "Correction A\|Seeded false\|TO WIRE UP\|Hardcoded until" mobile/src`: the only hits are two references to `eatNext.ts`'s own unrelated "Correction A" (Task 2's finding about `computeNudge`'s defensive invariants being mutation-provably unreachable — nothing to do with this migration). Nothing is still hardcoded.

**What this unblocks for Task 12.** Before this step, `nudgesEnabled` was hardcoded `false` everywhere, so no engine or UI code path involving an actual scheduled nudge had ever run — Task 10's amendment explicitly flagged this ("the nudge half of §7.2/§8.1 therefore cannot be verified end to end until Task 11's Step 4 wiring lands"). That gate is now open: an owner who flips the Notifications screen's Pace-nudges toggle ON will, for the first time, exercise a real `eat_nudges_enabled = true` row, a live `computeNudge` call that can return a non-null decision, and an actual `scheduleNotificationAsync` call in `syncEatNudgeCore`. Task 12's on-device checklist item ("with nudges enabled and a deliberately-behind day, a nudge schedules... and clears after catching up") is the first point in this plan where that claim is checkable at all.

**Gates:** `cd mobile && npx tsc --noEmit` → **0 errors**. `cd mobile && npm test` → **8/8 suites, 206/206 tests**, unchanged from baseline. Files touched: `mobile/src/hooks/useEatNext.ts`, `mobile/src/components/profile/NotificationsScreen.tsx`, this plan doc. No `eatNext.ts`, `eatNudgeService.ts`, or UI surface file touched. No `as any` / `@ts-expect-error`. **No database-connecting command run at any point in this session** — the apply and its post-verify were performed and confirmed by the coordinator before this session began; this session's own verification (Verifications 1–5 above) was entirely static: `grep` against the migration file and the `mobile/` tree, and `tsc` runs against local edits. Three commits, in order: the `useEatNext.ts` wiring, the `NotificationsScreen.tsx` wiring, this docs commit.

**Concerns for the owner to watch on device now that nudges are live.** (1) Turning the toggle ON for the first time will trigger a real `requestPermissions()` call at that gesture — if notifications were previously denied for FitTracker, the owner will see the "Notifications Disabled" alert with an Open Settings action, exactly as designed, but this is the first time that path has actually executed end to end. (2) A nudge only ever schedules when calorie pace is genuinely `behind` by at least `NUDGE_MIN_GAP_CAL` before `windowEndMinutes`, per `computeNudge`'s existing (unmodified) gates — an owner testing this by simply flipping the toggle on a day that is on-pace or already past the eating window will correctly see nothing scheduled, which is expected behavior, not a bug. (3) `cancelAllEatNudges`/`syncEatNudge` are unchanged by this task and were already reviewed and mutation-tested in Task 6 for the ≤1-pending invariant and the calendar-day guard — this step only made the input to that machinery real; if anything looks off in scheduling behavior itself, the relevant code (and its extensive amendment history) is in the Task 6 section, not here.
