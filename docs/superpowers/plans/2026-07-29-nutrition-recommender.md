...
A follow-up code-quality review (verdict: "Ready to merge with fixes," no Critical issues, purity and the reason-generation seam praised) found four further untested guards via its own mutation pass, plus three smaller correctness/hygiene issues. None change §5 behavior; spec compliance had already been re-verified and was explicitly out of scope for this pass.
