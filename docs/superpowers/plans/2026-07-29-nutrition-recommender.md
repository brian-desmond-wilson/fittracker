...
- [ ] **Step 4: Post-apply wiring (ADDED during execution — see the Task 5 amendment).** Until this step runs, the nudge is dead code: Task 5 ships `nudgesEnabled: false` hardcoded because naming a nonexistent column makes PostgREST reject the whole select (42703 → HTTP 400). Only now is the column real. In `mobile/src/hooks/useEatNext.ts`, two edits (the select list is derived from the interface, so this can no longer be half-done — see the Task 5 amendment):
  1. Add `eat_nudges_enabled: boolean;` to `interface ProfileRow`, replacing the `NOTE:` comment that marks its absence. **This alone fails to compile** (`Property 'eat_nudges_enabled' is missing in type … but required in type 'Record<keyof ProfileRow, true>'`) until you add the matching `eat_nudges_enabled: true,` entry to the `PROFILE_COLUMNS` object just below it, which is what puts the column in the select string.
  2. Change the `recommendEatNext` argument `nudgesEnabled: false` to `nudgesEnabled: p.eat_nudges_enabled`, deleting the block comment above it that explains the hardcode.

  Then `cd mobile && npx tsc --noEmit && npm test` (both must stay clean — no test covers the hook, so this is a compile + regression check only), and verify on device that the Notifications toggle from Task 6 now actually gates nudge scheduling. Commit separately from the apply.
...
**`### Task 11` `Step 4: post-apply wiring` extended, not renumbered.** Appended a `NotificationsScreen.tsx` sub-item to the existing Step 4 (added by Task 5) after its two `useEatNext.ts` edits, without touching either of them:
...
Then re-run `cd mobile && npx tsc --noEmit && npm test` as Step 4 already directs (both files together, since both changed), and add "toggle the Pace-nudges switch and confirm the write no longer fails" to the on-device checklist Step 4 already specifies for the Notifications toggle.
...
**Verification 6 — midnight edge in `syncEatNudge`: assessed as a latent-but-currently-unreachable gap; code NOT changed, comment expanded.** `EatNextNudge.fireAtMinutes`'s doc comment (`eatNext.ts:71-73`) is explicit: minutes-since-local-midnight on the SAME local day `nowMinutes` was computed on, and "Consumers must resolve it against that same day, not a fresh `new Date()`" — which is exactly what the plan's `syncEatNudge` does (`fireDate = new Date(); fireDate.setHours(...)`). Traced the failure mode precisely: if a decision is computed shortly before local midnight (requires `windowEndMinutes` itself to be within minutes of midnight, since `fireAtMinutes <= windowEndMinutes` is an engine invariant) and `syncEatNudge` is not invoked until after local midnight, `new Date()` resolves to the new calendar day, and `fireDate.setHours(...)` reconstructs a time on that new day — up to ~24h later than the decision intended, silently, no error. The existing `fireDate.getTime() <= Date.now()` guard does not catch this: it only detects "the target clock time already passed today," not "today is a different day than the one the decision was computed for." **Concluded not genuinely unsafe today**, and left unchanged, because no call path exists yet that could trigger it: `useEatNext`'s `load()` computes `now`/`nowMinutes` and the resulting decision in one synchronous pass (Task 5 amendment, "Deviation: one clock for the whole assembly"), and nothing in the codebase as it stands persists an `EatNextResult` across a background/foreground cycle before calling `syncEatNudge` — that wiring is Task 8/10, not yet written. This is exactly the reachability discipline Task 5's review round 1 established (state what's verified against actual code, not what a future task might do) applied here, one task earlier than usual, because the caller doesn't exist yet to check. The `syncEatNudge` doc comment was expanded (not the logic) to spell out the exact failure mode, why it's inert today, and a load-bearing instruction for whoever wires Task 8/10: call `syncEatNudge` synchronously off a freshly-computed `result`, never a decision cached from a prior day. **Flagged for re-verification at Task 8/10 review**, since that is where the guarantee this reasoning depends on either holds or breaks.
...
**Assess 3 — no test coverage possible; confirmed, not worked around.** `jest.config.js`: `testEnvironment: "node"`, `roots: ["<rootDir>/src"]`, `testMatch: ["**/__tests__/**/*.test.ts"]` — no React Native mocks configured. `eatNudgeService.ts` imports `expo-notifications` at module scope, so importing it at all in the node test environment would fail before any test body runs; confirmed by precedent — neither `mealReminderService.ts` nor `waterReminderService.ts`, both years-old and in the same family, has a test file. No test was added, matching that precedent exactly. The one piece of logic worth extracting as pure and testable — the `fireAtMinutes` → `Date` conversion (the exact midnight-edge logic in Verification 6) — was considered and **not extracted**, purely because of this task's explicit file-touch list (`eatNudgeService.ts`, `NotificationsScreen.tsx`, this plan doc only); extracting it would require a new module, following the `mealScoreInput.ts` precedent from Task 5. Flagged as a follow-up below rather than done out-of-scope.

**Follow-up (queued, not in Task 6 scope):** extract the `fireAtMinutes` → `Date` conversion out of `eatNudgeService.ts` into a small pure, type-only-import module (mirroring `mealScoreInput.ts`'s zero-runtime-import trick), so it can enter the node-Jest suite and the midnight-edge case from Verification 6 can be asserted directly rather than only reasoned about. Natural moment: whenever Task 8/10's wiring is written and the reachability argument in Verification 6 needs re-checking anyway.
