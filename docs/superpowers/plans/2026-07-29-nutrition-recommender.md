...
**Verification 3 — double-load on mount: confirmed, not fixed, matches the hook's own stated design.** `useEatNext`'s internal `useEffect(() => { load(); }, [load, refreshKey])` fires `load()` on every mount, independent of focus. The card's `useFocusEffect(refetch)` also fires on the initial focus that accompanies a first mount. So a fresh mount of `EatNextHomeCard` issues two `load()` calls back-to-back-ish, both racing through the same five-way `Promise.all`. Correctness is protected by the `runIdRef` stale-response guard (`useEatNext.ts:162`), confirmed by reading it: only the run whose `runId` still matches `runIdRef.current` at completion is allowed to call `setResult`/`setError`/`setLoading(false)`, so the two calls can't produce a torn state — worst case is the slower one's network round trip being wasted (its result, if it lands after the faster one, is simply discarded by the guard; if it lands first, the faster/`runId`-current one still wins on completion since guards compare against `runIdRef.current`, which was bumped again by the second call). **Not fixed**, because there is no clean fix available from this file alone: the mount-triggered load lives inside `useEatNext.ts` (explicitly off-limits — "Do NOT modify … `useEatNext.ts`"), and dropping the card's own `useFocusEffect` would break the spec §7.1 "useFocusEffect reload" requirement (a background→foreground cycle without an intervening unmount would then never refresh the card). `useEatNext.ts`'s own comment (`:155-161`) already names Task 8's focus-refetch as the reason the guard exists, i.e. this exact double-invocation was anticipated and paid for during Task 5, not stumbled into here. Wasted work is one extra five-query `Promise.all` per mount — judged acceptable given the alternative is touching a file outside this task's grant.

**Verification 4 — spec §7.1 completeness: a deliberate, reported scope reduction, not a silent one.** Spec text: "Shows the top recommendation: name, first reason line, cal · protein · prep, score chip; red accent when context=`emergency`." The plan's Task 8 code (and this implementation) renders name, first reason line, and the emergency red accent — but **not** a cal/protein/prep line or a score chip. Traced why: `EatNextRecommendation` (`eatNext.ts:64-68`) carries exactly `{ mealId: string; name: string; reasons: string[] }` — no calories, protein, prep minutes, or score. Those values exist inside `recommendEatNext`'s internal `Candidate`/`ScoredMeal` types (`totals: MealTotals`, `score: BrianScoreResult`) but are never attached to the `EatNextRecommendation` the function returns; only the *reasons text* threads some of that data through as English (e.g. emergency's reasons[0] already embeds `${cal} cal, ${prep} min prep` — the numbers spec §7.1 wants are, coincidentally, already visible in the reason string for that one context, but not for `next_meal`/`post_workout`/`catch_up`/`goal_hit`, whose reasons don't state calories/protein/prep at all). Surfacing a standalone cal/protein/prep line or a score chip would require adding fields to `EatNextRecommendation` and populating them in every `toRecs`/return-site in `recommendEatNext` — an `eatNext.ts` change, which this task's constraints explicitly forbid ("Do NOT modify `eatNext.ts` … If you need something they don't expose, report it — don't reach in"). **Decision: implement exactly what the plan's code provides (name + first reason + message), and report the gap rather than choosing silently or reaching into the engine.** This mirrors Task 1's own precedent of flagging `EMPTY_LIBRARY_MESSAGE` as a needed engine export rather than having Task 8 improvise a workaround — the analogous fix here (adding cal/protein/prep/score fields to `EatNextRecommendation`) is a legitimate follow-up for whoever next touches `eatNext.ts` (Task 10 touches the engine's consumers but not the engine itself, per its own file list — so this is likely a small dedicated follow-up, not naturally absorbed by an existing remaining task). Filed as a follow-up below rather than expanding this task's scope.
...
**Follow-up filed (not Phase 3 scope as currently written): surface cal/protein/prep/score on `EatNextRecommendation`.** To satisfy spec §7.1's "cal · protein · prep, score chip" literally, `recommendEatNext`'s return type needs those fields attached per-recommendation (from the `Candidate`/`ScoredMeal` data it already computes internally but doesn't expose), and `EatNextHomeCard` would then render a second line from them. Left for a dedicated follow-up rather than done here because it requires editing `eatNext.ts`, which this task's file list explicitly excludes; whoever picks it up should also decide whether to re-derive the numbers already embedded as English in some `reasons[0]` strings (risking two sources of the same fact) or leave those reason strings as-is alongside the new structured fields.
