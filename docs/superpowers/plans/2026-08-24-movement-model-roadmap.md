...
- [ ] Core skill/scoring curation pass (user directive 2026-09-09, from the Stage 5 exit gate): EVERY exercise/movement should carry a skill level and at least one scoring type when relevant — declared standard. Today 58/287 rows lack skill_level and most lack scoring junctions; cores included (Bench Press has neither). Two parts, in order: (1) curate skill_level + scoring types on all 48 cores so wizard inheritance supplies defaults to every new derivation; (2) then make the wizard require skill level and ≥1 scoring type on create (with a considered "when relevant" rule for Recovery/Mobility rows). Optionally backfill the remaining derivations from their cores where blank.
