...
- [ ] DEFERRED to the multi-user milestone (decision 2026-09-11, creator avatars): the `refresh-creator` action on `capture-post` lets any signed-in user create a `public.creators` row for any regex-valid handle and, for a stale Instagram creator, choose which platform-CDN image becomes the avatar everyone sees (host allow-list only, no ownership tie, no rate limit). Fix when accounts multiply: require `(platform, handle)` to exist in the caller's own `captured_sources`, and/or a per-user hourly cap. Spec: `docs/superpowers/specs/2026-09-10-creator-avatars-design.md` §5.2.
