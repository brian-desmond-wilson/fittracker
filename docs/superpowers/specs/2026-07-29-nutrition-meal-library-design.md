...
**`consume_inventory_units(p_inventory_ids uuid[]) returns table(inventory_id uuid, consumed int)`** — per id: decrement 1 from the `food_inventory_locations` row with stock (`ORDER BY is_ready_to_consume DESC, quantity DESC LIMIT 1 FOR UPDATE`); if the item has **no** location rows, fall back to a **guarded** UPDATE of the legacy column — `set quantity = fi.quantity - 1 where fi.id = v_id and fi.quantity > 0` (legacy-only items exist: 22 inventory rows, 17 location rows). After each decrement with locations, resync `food_inventory.quantity = sum(location quantities)` so legacy readers stay correct. `consumed` is 0 when no stock — never an error.

> **Amendment (2026-07-29, Task 6).** This clause originally specified the fallback as `food_inventory.quantity = greatest(0, quantity − 1)`. That form is **wrong**, for a non-obvious reason worth recording: with `greatest()` the UPDATE still **matches** the row when `quantity` is 0 — it just writes 0 over 0 — so `get diagnostics … row_count` returns 1 and the function reports `consumed = 1` for a decrement that took nothing. That directly contradicts this section's own "`consumed` is 0 when no stock," and it would silently defeat the Task 9 fix, which filters the RPC's returned rows on `consumed > 0` to decide what `undoMealLog` may refund; an untruthful 1 there makes Undo invent a unit of stock. Moving the condition into the `WHERE` clause makes `row_count` an honest report of whether anything moved, and `quantity >= 0` is still guaranteed because the row is only touched when it is already ≥ 1. The implementation in `20260729100100_inventory_consume_rpc.sql` is correct as written; the spec text was the defect.
