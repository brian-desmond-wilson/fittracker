...
- **Meal Library:** rows get an **"In stock"** badge; an "In stock only" filter toggle sits in a filter bar directly **below** the header, list mode only; MealDetail lists `missing` when not assemblable ("Missing: Korean BBQ Sauce"); MealBuilder shows per-item availability dots. The container computes one `assemblabilityByMealId` map (memoized like `scores`/`totalsById`) that the badge, the filter and MealDetail read; **MealBuilder's per-item dots cannot read that map** and instead receive the container's `inventory` array and call `assessAssemblability` once per item with a single-item `items` array.

*(Amended 2026-07-30 during Task 8 execution, ruled by the coordinator: the corrected text is folded into the bullet above rather than left stale beneath this note, per the convention this spec already uses at §7. Two divergences, both upheld in favor of the shipped code. **(1) "one `assemblabilityByMealId` map … that all of these read" was internally incoherent** where "these" included MealBuilder's per-item dots: the map is keyed by **meal id** and its values are whole-meal `MealAssemblability` objects, which carry no per-item resolution — `missing` is a list of display names, not a keyed structure — so nothing in it can answer "is *this* ingredient in stock", and MealBuilder is editing an unsaved draft that has no meal id at all when creating. The plan's resolution (pass `inventory`, call per item) is the only implementable one and is what shipped. It is also **provably equivalent** to what the spec intended: `resolveInventoryMatches` carries no cross-item state, so a one-item call's `assemblable` is exactly "this item is not in the whole-meal `missing` list". Cost is `O(inventory)` per item — ~250 comparisons at 25 rows × 10 items — so the map would have bought nothing but coupling. **(2) "header gains an 'In stock only' filter toggle"** — the toggle shipped in a filter bar **below** the header, not in it. `MealLibraryModal`'s header is a fixed three-slot `space-between` row (`＋ New` / title / `Done`) that a fourth child breaks, and the rejected alternative — a `ListHeaderComponent` inside the `SectionList` — would scroll out of view while still hiding rows, which is precisely the state that must stay visible. Recorded here because it was originally captured only in the plan. Full reasoning in the plan's "⚠️ Execution amendments → Task 8".)*
