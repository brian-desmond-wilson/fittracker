...
- `findInventoryMatchByBarcode` (existing service) now selects `locations:food_inventory_locations(quantity, is_ready_to_consume)` and returns `totalQuantity` from the shared projection; `InventoryMatchSummary.quantity` is redefined to carry that projected total. MealsScreen's `willUseInventory` gate needs no code change beyond the semantic fix flowing through — but its in-code divergence comments (MealsScreen ~:539-543, foodInventoryMatchService ~:64-70) are updated to say the divergence is closed.
