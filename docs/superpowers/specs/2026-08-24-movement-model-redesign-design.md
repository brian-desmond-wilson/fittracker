...
## Amendment — Stage 4 locks live (2026-09-08)

`20260909100000_turn_the_locks.sql` is LIVE. Duplicates are structurally impossible: UNIQUE constraint `exercises_fingerprint_key (core_movement_id, identity_fingerprint)` DEFERRABLE INITIALLY IMMEDIATE (curation flows may SET CONSTRAINTS DEFERRED for multi-row reshuffles; validate pre-commit with SET CONSTRAINTS ALL IMMEDIATE); plain fingerprint index dropped. Engine: recompute split into orchestrator + single-row worker; identity changes drain whole core families in one global (cardinality, id)-ordered pass (tier invariant proven; the review's diamond scenario is a permanent V10 fixture); locks acquired up front in the same order (residual overlapping-cascade 40P01 is documented retryable); session-variable guard replaced pg_trigger_depth; enforce_core_self_reference is SECURITY DEFINER with FOR KEY SHARE target validation + FOR UPDATE demote self-lock (both commit orders closed). Exit gate met on live. Bare zero-attribute children are impossible while cores stay attribute-free (they collide with the core's own '' slot).

## Amendment — Stage 5 app unification live (2026-09-09)

App code (merge 30df655) + migration `20260910100000_renormalize_and_policies.sql` are LIVE. The front-door module is the app's sole catalog writer (create/update, typed errors, duplicate UX); one wizard serves both tabs with a real edit path; reads use stored tier/generated names/junctions; filter pills are classification-driven; capture resolves alias-first with unknowns routed to the review queue (three resolutions incl. alias teaching) — the silent auto-create path is deleted. DB: alias re-normalization on dictionary change (collisions routed to review), DELETE-triggered family recompute closes the last drift gap, policies tightened (own-non-official writes; wild-alias INSERT carve-out; official-catalog minting blocked; definer workers sealed from RPC). Remaining Stage 5 exit gate: on-device add/edit/capture-match. Stage 6 hand-offs recorded on the roadmap (private-row hierarchy infiltration, alias scoping).
